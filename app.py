...
def _interaction_detail_html(interactions: list) -> str:
    ...
    for it in interactions:
        ...
        card = (
            ...
            f"<div style='font-size:13px;color:#d4b896;line-height:1.7;'>{detail['성립']}</div>"
            f"</div>"
            # 효과
            f"<div>"
            ...
            f"<div style='font-size:13px;color:#d4b896;line-height:1.7;'>{detail['효과']}</div>"
            f"</div>"
            f"</div>"
        )
...
def render_role_winner_hero(meal_result: Dict[str, object]) -> None:
    ...
    st.markdown(textwrap.dedent(f"""
    <div style="background:linear-gradient(135deg,#22101c 0%,#160a12 100%);border:1px solid rgba(230,164,184,.45);border-radius:24px;padding:1.05rem 1.15rem;margin:.75rem 0 1rem 0;text-align:center;box-shadow:0 14px 34px rgba(214,66,115,.10);">
        <div style="display:inline-block;background:#22101c;border:1px solid rgba(214,66,115,.22);border-radius:999px;padding:.22rem .72rem;color:#fde68a;font-weight:950;font-size:.88rem;">{emoji} {safe_title}</div>
        ...
        <div style="color:#d4b896;font-weight:760;line-height:1.55;margin-top:.35rem;">{safe_summary}</div>
    </div>
    """).strip(), unsafe_allow_html=True)
...
def render_rank_pyramid(title: str, rows: List[Dict[str, object]], caption: str = "") -> None:
    ...
    for idx, row in enumerate(rows[:6], start=1):
        ...
        st.markdown(f"""
        <div style="width:{width}%;margin:.28rem auto;padding:.55rem .75rem;border:1px solid rgba(230,164,184,.38);border-radius:16px;background:#22101c;box-shadow:0 8px 18px rgba(214,66,115,.08);text-align:center;min-width:220px;">
            <div style="font-weight:900;color:#d4b896;">{safe_rank}위 · {safe_name}</div>
            <div style="font-weight:900;color:#f59e0b;font-size:1.05rem;">{score:.1f}점</div>
            <div style="font-size:.82rem;color:#d4b896;">{safe_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    if caption:
        st.caption(caption)
    ...
    for col, (title, name, value, body) in zip(cols, cards):
        with col:
            ...
            <div class="mini-card">
                ...
                <div style="color:#f59e0b;font-weight:900;margin-top:.35rem;">{html.escape(str(value), quote=True)}</div>
                <div style="color:#d4b896;font-size:.86rem;margin-top:.35rem;line-height:1.45;">{html.escape(body, quote=True)}</div>
            </div>
            """, unsafe_allow_html=True)
...
def build_multi_situation_svg(participants: List[Dict[str, object]], power_map: Dict[str, object]) -> str:
    ...
    for row in pair_rows:
        ...
        parts.append(
            f'<text x="{mx}" y="{my+4}" text-anchor="middle" font-size="11" font-weight="900" fill="#d4b896">{label}</text>'
        )
    ...
    for p in participants:
        ...
        parts.append(f'<text x="{x}" y="{y-2}" text-anchor="middle" font-size="16" font-weight="900" fill="#fde68a">{html.escape(name)}</text>')
        parts.append(f'<text x="{x}" y="{y+17}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#d4b896">{html.escape(display_role)}</text>')
...
tbody td {
    ...
    border: 1px solid rgba(232,121,160,0.18) !important;
    line-height: 1.75 !important;
}
...
<script>
(function(){
    var css=`.svg-radar-card,.chem-radar-card{background:#22101c;border:1px solid rgba(90,40,64,.60);border-radius:20px;padding:.75rem;box-shadow:none;}
...
.element-bar-track,.chem-track{height:10px;background:#2d1a24;border-radius:999px;overflow:hidden;}
...
.chem-line{display:grid;grid-template-columns:56px 1fr 54px;gap:.42rem;align-items:center;margin:.22rem 0;font-size:.82rem;color:#b89a6b;}
.chem-fill{height:100%;border-radius:999px;}
.chem-fill.mine{background:#92400e;}
.chem-fill.friend{background:#6b8cff;}
.daewun-flow-track{display:flex;gap:.55rem;overflow-x:auto;padding:.35rem .1rem .55rem .1rem;}
.daewun-flow-card{min-width:104px;background:#22101c;border:2px solid #5a2840;border-radius:16px;padding:.75rem .65rem;text-align:center;box-shadow:none;}
...
.radar-legend{display:flex;gap:1rem;justify-content:center;align-items:center;margin-top:.1rem;font-size:.88rem;color:#d4b896;font-weight:850;}
...
.chem-overlap-row{display:grid;grid-template-columns:34px 1fr 1fr;gap:.45rem;align-items:center;background:#12080f;border:1px solid rgba(230,164,184,.28);border-radius:12px;padding:.45rem .6rem;color:#d4b896;font-size:.86rem;}
.chem-overlap-row b{color:#f59e0b;}
.chem-overlap-row em{font-style:normal;color:#b89a6b;text-align:right;}
.luck-intensity-card{background:#22101c;border:1px solid rgba(230,164,184,.32);border-radius:16px;padding:.75rem .85rem;margin:.35rem 0 .45rem 0;}
...
.mini-meter{height:12px;background:#2d1a24;border-radius:999px;overflow:hidden;margin-top:.35rem;}
...
.flow-strength-meter{height:9px;background:#1e0f18;border-radius:999px;overflow:hidden;}
.flow-strength-meter span{display:block;height:100%;background:#64748b;border-radius:999px;}

.small-muted{font-size:.82rem;color:#b89a6b;margin-top:.55rem;line-height:1.55;}
.pipeline-card{padding:1rem;margin:.75rem 0;border-radius:18px;background:#22101c;border:1px solid rgba(90,40,64,.50);box-shadow:none;}
.flow-pipeline{display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin:.7rem 0;}
.flow-node{display:inline-flex;flex-direction:column;align-items:center;justify-content:center;min-width:88px;padding:.55rem .7rem;border-radius:16px;background:#180c14;border:1px solid rgba(100,150,220,0.35);color:#94a3b8;}
...
.pipe-node{min-width:92px;background:#180c14;border-color:#dbeafe;box-shadow:none;}
...
.pipe-label{font-size:.82rem;font-weight:900;color:#b89a6b;white-space:nowrap;line-height:1;}
...
.shinsal-card{position:relative;overflow:hidden;border-radius:26px;padding:1.08rem 1.05rem;border:1px solid rgba(214,66,115,.17);background:linear-gradient(135deg,#22101c,#12080f);box-shadow:0 14px 30px rgba(80,40,60,.08);}
...
.shinsal-icon{display:flex;align-items:center;justify-content:center;width:54px;height:54px;border-radius:18px;background:#22101c;box-shadow:0 8px 18px rgba(80,40,60,.08);font-size:2rem;line-height:1;flex:0 0 54px;}
.shinsal-name{font-size:1.22rem;font-weight:950;color:#fcd7a0;letter-spacing:-.04em;line-height:1.1;}
.shinsal-type{font-size:.88rem;color:#d4b896;font-weight:900;margin-top:.18rem;}
.shinsal-badge{display:inline-block;margin:.1rem 0 .45rem 0;padding:.22rem .62rem;border-radius:999px;background:rgba(214,66,115,.10);color:#f59e0b;font-weight:950;font-size:.8rem;}
.shinsal-summary{font-size:.94rem;color:#fcd7a0;line-height:1.55;font-weight:780;margin:.35rem 0 .62rem;}
.shinsal-detail{background:rgba(30,16,24,.92);border:1px solid rgba(230,164,184,.22);border-radius:16px;padding:.68rem .75rem;margin:.45rem 0;position:relative;z-index:1;}
.shinsal-detail-title{font-size:.88rem;color:#f59e0b;font-weight:950;margin-bottom:.22rem;}
.shinsal-detail-body{font-size:.84rem;color:#d4b896;line-height:1.52;font-weight:650;}
.shinsal-pos{font-size:.88rem;color:#d4b896;margin-top:.5rem;font-weight:800;}
.shinsal-footnote{font-size:.88rem;color:#d4b896;line-height:1.45;margin-top:.6rem;}
...
.manse-wrap {
    background: linear-gradient(135deg,#22101c 0%,#160a12 48%,#180c14 100%);
...
.manse-luck-wrap {
    ...
    padding:.9rem .85rem;
    border-radius:22px;
    background:linear-gradient(135deg,#22101c 0%,#160a12 56%,#180c14 100%);
    border:1px solid rgba(230,164,184,.32);
...
.manse-luck-card.caution { background:linear-gradient(135deg,#22101c,#160a12); }
...
.manse-luck-el.el-화 { background:#22101c; }
.manse-luck-el.el-토 { background:#160a12; }
.manse-luck-el.el-금 { background:#180c14; }
.manse-luck-el.el-수 { background:#180c14; }
.manse-luck-score {
    color:#f59e0b !important;
    font-size:.82rem;
    font-weight:950;
...
.saju-landscape-shell {
    ...
    background: linear-gradient(135deg,#180c14 0%,#22101c 56%,#160a12 100%);
...
.saju-landscape-mini-card {
    padding:.68rem .72rem;
    border-radius:18px;
    background:linear-gradient(135deg,#22101c,#180c14);
...
.saju-landscape-role {
    ...
    padding:.68rem .74rem;
    border-radius:18px;
    background:linear-gradient(135deg,#180c14,#22101c);
    border:1px solid rgba(70,92,120,.12);
    color:#fde68a !important;
    font-size:.84rem;
    line-height:1.48;
...
.share-card-wrap {
    ...
    background:
        ...
        linear-gradient(135deg,#fefce8 0%,#fffdf8 56%,#f7fbff 100%);
...
.today-compass {
    ...
    background: linear-gradient(135deg,#0d1e14 0%,#180c14 54%,#22101c 100%);
...
.today-compass-prescription {
    ...
    border-radius:18px;
    padding:.78rem .86rem;
    background:linear-gradient(135deg,#22101c,#12080f);
    border:1px solid rgba(214,66,115,.16);
}
...
.today-quick-card {
    ...
    background:
        ...
        linear-gradient(135deg,#eefcff 0%,#fefce8 58%,#fffaf0 100%);
...
div[data-testid="stRadio"] div[role="radiogroup"] label[aria-checked="true"] {
    background:linear-gradient(135deg,#1e2d4a,#22101c) !important;
...
.stApp { overflow-x: hidden !important; background-color: #090f1e !important; }
...
div[data-baseweb="checkbox"] svg { color: #f59e0b !important; }
...
div[data-baseweb="popover"] { background-color: #1e2d4a !important; }
div[data-baseweb="menu"] { background-color: #1e2d4a !important; }
div[data-baseweb="option"] { background-color: #1e2d4a !important; color: #fde68a !important; }
div[data-baseweb="option"]:hover { background-color: #2d1a24 !important; }
...
div[data-testid="stRadio"] > div > label {
    background-color: #1e2d4a !important;
    border: 1px solid rgba(232,121,160,0.40) !important;
    color: #fde68a !important;
}
...
[data-testid="stTimeInput"] input { background-color: #1e2d4a !important; color: #fde68a !important; }
...
[data-testid="stRadio"] > label,
[data-testid="stCheckbox"] > label,
[data-testid="stSelectbox"] > label { color: #d4b896 !important; font-weight: 600 !important; }

/* 전역 카드/박스 다크 오버라이드 */
[data-testid="stMetric"],
[data-testid="metric-container"] { background-color: #1e2d4a !important; border-radius: 12px !important; }
[data-testid="stExpander"] > div { background-color: #180c14 !important; }
[data-testid="stForm"] { background-color: #22101c !important; border: 1px solid rgba(232,121,160,0.20) !important; }
div.stAlert > div { border-radius: 10px !important; }
[data-testid="stAlert"][data-baseweb="notification"] { background-color: #22101c !important; }
/* 탭 */
[data-testid="stTabs"] [role="tablist"] { background-color: #1e2d4a !important; border-radius: 10px !important; }
[data-testid="stTabs"] [role="tab"] { color: #b89a6b !important; }
[data-testid="stTabs"] [role="tab"][aria-selected="true"] { color: #f59e0b !important; background-color: #2d1a24 !important; }
/* 구분선 */
hr { border-color: rgba(232,121,160,0.20) !important; }
...
[data-testid="stRadio"] { color: #fde68a !important; }
/* expander */
[data-testid="stExpander"] { background-color: #180c14 !important; border: 1px solid rgba(232,121,160,0.20) !important; }
[data-testid="stExpander"] summary { color: #d4b896 !important; }
/* info / success / warning 박스 */
[data-testid="stAlert"] { background-color: #1e2d4a !important; color: #fde68a !important; }
...
[data-testid="stMarkdownContainer"] li { color: #fde68a !important; }
[data-testid="stCaptionContainer"],
[data-testid="stCaptionContainer"] p { color: #b89a6b !important; }
.stCaption, .stCaption p { color: #b89a6b !important; }
[data-testid="stText"] { color: #fde68a !important; }
/* 헤딩 */
h1, h2, h3, h4 { color: #fcd7a0 !important; }
[data-testid="stMarkdownContainer"] h1,
[data-testid="stMarkdownContainer"] h2,
[data-testid="stMarkdownContainer"] h3,
[data-testid="stMarkdownContainer"] h4 { color: #fcd7a0 !important; }
/* 히어로 타이틀 명시 */
.hero-title { color: #fcd7a0 !important; }
.hero-title span { -webkit-text-fill-color: unset !important; color: #fcd7a0 !important;
    background: linear-gradient(92deg, #ff80b0 0%, #f59e0b 50%, #c084fc 90%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
.hero-subtitle { color: #d4b896 !important; }
...
[data-testid="stAppViewContainer"] { background-color: #090f1e !important; }
[data-testid="stHeader"] { background-color: #090f1e !important; }
section.main { background-color: #090f1e !important; }
.block-container { background-color: #090f1e !important; }
[data-testid="stSidebar"] { background-color: #1e2d4a !important; }
body { background-color: #090f1e !important; color: #fde68a !important; }
...
/* 버튼형 내부 페이지 */
.page-button-guide{font-size:.92rem;color:#b89a6b;margin:.4rem 0 .55rem 0;}
...
/* 재방문 안내 카드 */
.revisit-guide-card{background:#160a12;border:1px solid rgba(232,121,160,0.25);border-radius:18px;padding:14px 16px;color:#fde68a;margin-bottom:10px;}
...
@media (max-width: 768px) {
    ...
    .stButton > button {
        ...
        background: linear-gradient(180deg,#22101c,#12080f) !important;
    }
...
.revisit-guide-compact {
    margin-top: .72rem !important;
    background: linear-gradient(135deg,#22101c,#12080f) !important;
...
def render_my_saju_browser_storage_widget(save_url: str = "", key: str = "my_saju_browser_store") -> None:
    ...
    component_html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; border:1px solid rgba(232,121,160,0.30); background:#22101c; border-radius:16px; padding:12px; color:#fde68a;">
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        ...
        <a id="loadLink" target="_parent" style="display:none; text-decoration:none; border:1px solid #f59e0b; background:#1e2d4a; color:#f59e0b; border-radius:999px; padding:8px 12px; font-weight:800;">저장된 내 사주 불러오기</a>
        <button id="clearBtn" style="display:none; border:1px solid rgba(232,121,160,0.30); background:#1e2d4a; color:#b89a6b; border-radius:999px; padding:8px 12px; font-weight:800; cursor:pointer;">저장 삭제</button>
      </div>
...
def render_admin_analytics_dashboard() -> None:
    ...
    st.markdown(
        "<div class='signal-card' style='border-color:rgba(16,185,129,.28);background:linear-gradient(135deg,#ecfdf5,#fefce8);'>"
...
.stApp {
    background:
        ...
        linear-gradient(135deg, #fffbf0 0%, #fff4ec 45%, #f7fbff 100%);
...
.mini-card-value { color:#f59e0b; font-size:1.35rem; font-weight:900; margin-top:0.2rem; }
.score-number {
...
:root {
    --palja-readable-text: #fde68a;
    --palja-readable-muted: #b89a6b;
...
.main-character-card {
    background: linear-gradient(135deg, #22101c 0%, #1a1020 100%);
...
.meal-stage-wrap {
    background: linear-gradient(135deg, #22101c 0%, #160a12 52%, #180c14 100%);
...
.safe-table { width:100%; border-collapse:separate; border-spacing:0; font-size:.88rem; background:#22101c; border:1px solid rgba(230,164,184,.35); border-radius:14px; overflow:hidden; }
.safe-table th { text-align:left; background:#fff3f7; color:#d4b896; padding:.62rem .7rem; border-bottom:1px solid rgba(230,164,184,.35); white-space:nowrap; }
...
.viz-pill { display:inline-flex; align-items:center; gap:.35rem; background:#22101c; border:1px solid rgba(230,164,184,.35); color:#6a4554; border-radius:999px; padding:.32rem .6rem; font-size:.82rem; font-weight:850; margin:.15rem .22rem .15rem 0; }
.signal-card { background:#22101c; border:1px solid rgba(230,164,184,.30); border-radius:18px; padding:.85rem .9rem; margin:.45rem 0; box-shadow:0 8px 18px rgba(255,125,165,.07); }
.signal-title { color:#d4b896; font-weight:950; margin-bottom:.25rem; }
.signal-body { color:#b89a6b; line-height:1.55; font-size:.92rem; }
...
.thermo-pin { position:absolute; top:50%; width:30px; height:30px; transform:translate(-50%,-50%); border-radius:999px; background:#22101c; border:3px solid #4b2d3a; box-shadow:0 4px 12px rgba(0,0,0,.16); }
...
/* h3·h4 헤더 */
.stMarkdown h3 { color: #fcd7a0 !important; font-size: 1.12rem !important; }
.stMarkdown h4 { color: #f59e0b !important; font-size: 1.04rem !important; }
/* expander 헤더 */
[data-testid="stExpander"] summary p {
    color: #fde68a !important;
    font-weight: 800 !important;
...
def auto_input_panel(prefix: str, title: str, default_date: date, default_time: time):
    ...
    if time_unknown:
        st.info("태어난 시간을 몰라? 그러면 년주·월주·일주 세 기둥으로 간이 분석해줄게. 시주가 없어서 완전하진 않지만, 큰 흐름은 볼 수 있어.")
...
def _triangle_axis_svg(score_map: Dict[str, Dict[str, float]]) -> str:
    ...
    poly = f"<polygon points='{' '.join(points)}' fill='rgba(214,66,115,.22)' stroke='#92400e' stroke-width='4.0'/>"
    dots = "".join([f"<circle cx='{p.split(',')[0]}' cy='{p.split(',')[1]}' r='6' fill='#92400e' stroke='#fff' stroke-width='1.8'/>" for p in points])
...
def render_strength_operation_spotlight(payload: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div class="main-character-card" style="border-color:rgba(16,185,129,.35);background:linear-gradient(135deg,#0d1e14,#22101c);margin:.35rem 0 1rem;">
...
def _radar_svg_single(values: List[float], labels: List[str], stroke: str = "#92400e", fill: str = "rgba(214,66,115,.20)") -> str:
    ...
    for val, label, a in zip(values, labels, angles):
        ...
        if lx < cx - 10:
            ...
            anchor = 'start'
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='800' fill='#b89a6b'>{html.escape(label)}</text>")
...
def _radar_svg_dual(vals_a: List[float], vals_b: List[float], labels: List[str], name_a: str, name_b: str, max_val: float = 60.0) -> str:
    ...
    for a, label in zip(angles, labels):
        ...
        if lx < cx - 10:
            ...
            anchor = 'start'
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='850' fill='#b89a6b'>{html.escape(label)}</text>")
    ...
    for pnt in pts_a:
        x, y = pnt.split(',')
        dots += f"<circle cx='{x}' cy='{y}' r='3.8' fill='#92400e'/>"
    for pnt in pts_b:
        x, y = pnt.split(',')
        dots += f"<circle cx='{x}' cy='{y}' r='3.8' fill='#3b82f6'/>"
    legend = (
        f"<div class='radar-legend'><span><i style='background:#92400e'></i>{html.escape(name_a)}</span>"
        f"<span><i style='background:#3b82f6'></i>{html.escape(name_b)}</span></div>"
    )
    svg = (
        "<svg viewBox='0 0 300 300' width='100%' height='310' role='img'>"
        + ''.join(grid + axes)
        + f"<polygon points='{' '.join(pts_a)}' fill='rgba(214,66,115,.24)' stroke='#92400e' stroke-width='3'/>"
...
def render_climate_thermometer(result: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div style="background:#22101c;border:1px solid rgba(230,164,184,.30);border-radius:18px;padding:.85rem .9rem;margin:.3rem 0 .45rem 0;">
        <div style="display:flex;justify-content:space-between;color:#d4b896;font-size:.82rem;font-weight:850;margin-bottom:.45rem;">
        ...
        <div style="margin-top:.58rem;color:#d4b896;font-weight:900;">{html.escape(label)} · 온도감 {temp:.0f}/100</div>
        <div style="margin-top:.24rem;color:#d4b896;font-size:.88rem;">화 {fire:.1f}% / 수 {water:.1f}% · {html.escape(note)}</div>
    </div>
    """, unsafe_allow_html=True)
...
def _render_flow_pipeline_card(chart: Chart, result: Dict[str, object], row: Dict[str, object]) -> None:
    ...
    for _role, _el in elements:
        ...
        if _srcs:
            ...
            _source_items.append(
                ...
                f"<span style='font-size:11px;font-weight:800;color:#fbbf24;'>{html.escape(_role)} ({html.escape(_el)})</span>"
    ...
    _source_block = (
        ...
        "<div style='font-size:11px;font-weight:800;color:#b89a6b;margin-bottom:5px;'>📍 원국 어디에서 이 흐름이 나오냐면</div>"
        + "".join(_source_items) +
        "</div>"
...
def render_today_compass_card(payload: Dict[str, object]) -> None:
    ...
    st.caption("오늘 일진을 내 원국이랑 가볍게 맞춰보는 거야. 점 보러 가는 게 아니라, 오늘 하루 어느 방향으로 쓸지 감 잡는 용도야.")
    st.markdown(f"""
...
def render_today_quick_entry(payload: Dict[str, object]) -> None:
    ...
    # 십성 → 오늘 성격 한 줄
    _tengod_story = {
        "비견": "오늘 비견 기운이야. 나랑 비슷한 처지, 비슷한 힘을 가진 사람이 옆에 있거든 — 경쟁이 될 수도 있고 든든한 동료가 될 수도 있어. 이 사람아, 오늘은 이기느냐 지느냐보다 같이 가느냐를 먼저 생각해봐. 혼자 다 하려고 힘 빼지 마 — 손 잡을 사람 하나 찾아봐. 그 쪽이 훨씬 빨리 간다고, 내가 보장해.",
        "겁재": "오늘은 겁재 기운이야, 이 녀석. 조심해. 경쟁 신호, 분쟁 신호가 슬쩍 올라와 있거든. 내 걸 지키려다 오히려 더 잃는 날이야 — 억지로 붙잡지 마. 돈 문제, 다툼 문제 생길 것 같으면 오늘만큼은 딱 한 발 빼. 양보가 지는 게 아니야 — 오늘은 그게 제일 현명한 거거든. 알겠어?",
        "식신": "오늘은 식신 기운이야 — 몸이 원하는 대로 하면 돼. 먹고 싶은 거 먹고, 하고 싶은 거 하고, 만들고 싶은 거 만들어봐. 이 기운은 긴장하면 죽어버리거든. 억지로 생산적일 필요 없어. 거봐, 오늘 쉬는 게 내일의 힘이야 — 느긋하게 보내는 날이 충전하는 날이야. 쉬는 것도 실력이야.",
        "상관": "아이고, 오늘 상관 기운이네. 말이 청산유수처럼 나오고, 아이디어가 팡팡 터지는 날이거든. 창의적인 일, 표현하는 일, 설득하는 일에 딱 맞아. 근데 있잖아 — 이 기운이 강하면 말실수도 같이 와. 하고 싶은 말 다 뱉기 전에 한 박자만 쉬어봐. 특히 윗사람 앞에서는 더 조심해, 알겠지? 머리는 활짝 열고, 입은 살짝 조여놔.",
        "편재": "오늘은 편재 기운이야. 돈이 움직이고 사람이 움직이는 날이거든. 기회가 반짝 눈앞에 나타날 수 있어 — 보이면 잡아봐. 근데 이 녀석, 이 기운은 너무 욕심 부리면 손가락 사이로 모래처럼 빠져나가거든. 쯧쯧. 적당히 잡고 적당히 놔주는 게 오늘의 기술이야. 흘러가는 건 보내줘.",
        "정재": "오늘은 정재 기운 — 꼼꼼하고 안정적인 날이야. 계획해둔 거 차근차근 실행하기에 딱 좋거든. 서두르거나 즉흥적으로 움직이면 오히려 어긋나 — 이 사람아, 오늘만큼은 서두르지 마. 정해진 대로, 원칙대로 가면 돼. 급하게 굴지 않아도 충분히 잘 된다고, 내가 장담해.",
        "편관": "오늘은 편관 기운이야. 긴장감이 공중에 떠 있는 날이거든. 결단을 요구받거나 압박을 받을 수 있어. 이럴 때 질질 끌면 더 힘들어지거든 — 야, 딱 끊어서 결정해. 두려움은 일단 옆에 내려놓고 지금 할 수 있는 것 먼저 해. 편관은 도망갈수록 커지고, 맞부딪히면 오히려 작아지는 기운이야. 도망가지 마.",
        "정관": "오늘은 정관 기운 — 반듯하게 가는 날이야. 바르게 행동하고, 약속 지키고, 책임 다하면 나중에 인정받아. 눈앞의 이익보다 의리랑 원칙이 더 중요한 날이거든. 이 사람아, 오늘 잘 쌓아둔 신뢰가 나중에 든든한 자산이 돼 — 지금 하는 행동 하나하나가 평판을 만든다는 거, 잊지 마.",
        "편인": "오늘은 편인 기운이야. 직관이 살아나고, 영감이 번뜩이는 날이거든. 논리보다 느낌이 더 잘 맞는 날이야. 혼자 공부하거나, 조용히 생각 정리하거나, 새로운 분야 탐색하기에 딱 좋아. 대신 있잖아 — 오늘은 시끄러운 자리에 있으면 기운이 산만해지거든. 나만의 시간 좀 확보해봐. 이거 중요해.",
        "정인": "오늘은 정인 기운 — 배우고 받아들이는 날이야. 누가 조언해주면 귀담아 들어봐, 이 사람아. 스승이 따로 없어도 오늘 만나는 사람, 읽는 글, 보는 것 하나하나가 다 가르침이 될 수 있거든. 내가 모른다는 걸 인정하는 게 오늘의 힘이야. 거봐, 받아들이는 자세가 열려 있을수록 오늘 더 많은 게 채워지거든.",
    }
    _tg_key = ""
    ...
    if "중첩" in interaction_overview:
        st.caption("이 사람아, 같은 지지쌍에서 합·형·파가 한꺼번에 잡힐 때는 복합 신호로 묶어 보여준단다.")
    st.caption("천간·지지 낱낱이, 원국 반응, 합충 상세는 아래 상세 보기에서 확인해봐.")
...
def render_saju_forecast_box(forecast: dict) -> None:
    ...
    lbg        = str(forecast.get("level_bg", "#180c14"))
    ...
    if signals:
        with st.expander("📡 주요 신호 자세히 보기", expanded=False):
            ...
            for sig in signals:
                ...
                st.markdown(
                    f"<div style='background:#180c14;border-left:3px solid {sc};"
...
def _draw_saju_landscape_pillow(draw, model: Dict[str, object], box: Tuple[int, int, int, int], font_small, font_mid, font_big) -> None:
    ...
    bx = x2 - 124
    for el in primary[:2]:
        color = LANDSCAPE_ELEMENT_META.get(el, {}).get("color", "#92400e")
...
def _shinsal_badges_html(chart: Chart, result: Dict[str, object], limit: int = 3) -> str:
    ...
    for item in items:
        ...
        pills.append(
            f"<span style='display:inline-flex;align-items:center;gap:.28rem;padding:.28rem .56rem;border-radius:999px;background:#22101c;border:1px solid rgba(230,164,184,.45);color:#b89a6b;font-size:.88rem;font-weight:900;box-shadow:0 4px 10px rgba(214,66,115,.06);'><span>{icon}</span><span>{name}</span></span>"
        )
...
def make_share_card_png_bytes(title: str, operation: str, keywords: str, overload: str, interpretation: str, one_liner: str, badges: List[str] | None = None, landscape_model: Dict[str, object] | None = None) -> bytes | None:
    ...
    for line in _wrap_for_image(draw, title, f_title, W-170)[:2]:
        draw.text((x, y), line, fill="#fde68a", font=f_title)
        y += 66
    ...
    url_y = foot_y1 + 82
    for ln in url_lines:
        draw.text((text_x, url_y), ln, fill="#b89a6b", font=f_tiny)
        url_y += 24

    draw.text((86, H-44), "※ 사주MRI 결과 카드는 오락·자기이해용 요약입니다.", fill="#d4b896", font=f_tiny)
...
def make_simple_first_share_png_bytes(payload: Dict[str, object], char: Dict[str, str]) -> bytes | None:
    ...
    y += 56
    # 이름 + 사주 원국
    draw.text((x, y), f"{_share_name} 님의 사주 원국", fill="#fcd7a0", font=f_small)
    y += 34
    ...
    for pi, (_pl, _pv) in enumerate(zip(_p_labels, _pillar_parts)):
        ...
        draw.text((cx_p - 16, y), _pl, fill="#fcd7a0", font=f_tiny)
        _pv_display = _pv if _pv else "-"
        draw.text((cx_p - 28, y + 24), _pv_display, fill="#fde68a", font=f_title)
    y += 90

    # 체질 한줄 설명
    for line in _wrap_for_image(draw, tone_plain, f_small, W-170)[:2]:
        draw.text((x, y), line, fill="#b89a6b", font=f_small)
        y += 30
    ...
    for label, info, desc in score_boxes:
        ...
        for ln in _wrap_for_image(draw, desc, f_tiny, sw-40)[:2]:
            draw.text((sx+20, y+136 if ln == _wrap_for_image(draw, desc, f_tiny, sw-40)[:1][0] else y+158), ln, fill="#b89a6b", font=f_tiny)
        sx += sw + 18
    y += 208
...
def render_daewun_train(payload: Dict[str, object]) -> None:
    ...
    if not daewuns:
        st.info(
            "이 사람아, 생년월일시 자동 산출로 들어와야 10년 단위 대운 흐름이 나와. "
            "직접 입력 방식으로 들어오면 생년월일이 없으니까 대운을 계산을 못 하거든 — 자동 산출로 다시 들어와봐."
        )
        return
    ...
    for item in visible:
        ...
        parts.append(
            ...
            f"<div style='font-size:12px;color:#b89a6b;line-height:1.5;'>{html.escape(story)}</div>"
            f"</div>"
        )
    parts.append("</div>")
...
def render_relationship_orientation_card(compatibility: Dict[str, object]) -> None:
    ...
    st.markdown(
        "<div class='signal-card' style='border-color:rgba(16,185,129,.35);background:linear-gradient(135deg,#0d1e14,#22101c);margin:.7rem 0 1rem;'>"
...
def _mini_axis_polygon(axis_values: Dict[str, float]) -> str:
    ...
    for a, lab in zip(angles, labels):
        ...
        if lx < cx - 5:
            ...
            anchor = "start"
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='9' font-weight='800' fill='#b89a6b'>{html.escape(lab)}</text>")
    return "<svg viewBox='0 0 160 160' width='160' height='160'>" + "".join(grid) + f"<polygon points='{' '.join(pts)}' fill='rgba(214,66,115,.22)' stroke='#92400e' stroke-width='2'/>" + "".join(label_nodes) + "</svg>"
...
def make_chemistry_share_card_png_bytes(
    ...
    def panel(x1, y1, x2, y2, label, body, fill="#fff9fc", body_font=None, max_lines=3):
        ...
        for ln in lines:
            draw.text((x1+22, yy), ln, fill="#fde68a", font=bf)
            yy += gap
...
def _chem_story_text(mine: dict, friend: dict, compatibility: dict, chem: dict) -> str:
    ...
    # 점수 이야기
    if score >= 78:
        score_story = f"아이고 세상에, {josa(my_name, "이/")}랑 {fr_name} 좀 봐봐! 궁합점수가 {score:.0f}점이야! 이런 케미 쉽게 보기 힘들거든 — 내가 사주 많이 봤는데 이 정도면 진짜야."
    elif score >= 65:
        score_story = f"그래, {josa(my_name, "이/")}랑 {josa(fr_name, "이/가")} 꽤 잘 맞는 사이야. {score:.0f}점이면 좋은 편이거든 — 이 정도 나오는 사이도 흔치 않아."
    elif score >= 50:
        score_story = f"{josa(my_name, "이/")}랑 {josa(fr_name, "이/가")} 보통 수준의 케미야. {score:.0f}점. 맞는 부분도 있고 조율도 좀 필요해 — 뭐, 그렇다고 나쁜 건 아니야."
    elif score >= 35:
        score_story = f"음, 이 사람아. {josa(my_name, "이/")}랑 {josa(fr_name, "이/가")} 좀 다른 결을 가진 사이야. {score:.0f}점이니까 맞춰가려는 노력이 필요해 — 안 하면 삐걱거리거든."
    else:
        score_story = f"쯧쯧, 솔직히 말할게. {josa(my_name, "이/")}랑 {josa(fr_name, "이/가")} 오행 구조가 꽤 달라. {score:.0f}점. 쉽지는 않겠지만 — 그래서 서로 배울 것도 더 많은 사이야. 어렵다고 포기하지 마."

    # 케미 유형 이야기
    type_stories = {
        "퍼즐형 케미": (f"거봐, 이 둘은 딱 퍼즐 조각처럼 맞는 사이야. {josa(my_name, "이/가")} 부족한 걸 {josa(fr_name, "이/가")} 갖고 있고, "
                       f"{josa(fr_name, "이/가")} 약한 부분을 {josa(my_name, "이/가")} 채워줘. 이런 관계가 오래가는 법이거든 — 내가 장담해."),
        "거울형 케미": (f"아이고, 이 두 사람 얼마나 닮았는지 봐봐. 생각하는 방식, 움직이는 리듬, 중요하게 여기는 것들이 "
                       f"비슷비슷해. 말 반만 해도 알아듣는 그런 사이야 — 이런 케미 흔하지 않아."),
        "스파크형 케미": (f"야야야, 이 사이에서는 불꽃이 좀 튀겠어. 서로 자극하고 흔드는 기운이 있거든. 나쁜 게 아니야 — "
                         f"이런 케미가 서로를 크게 만들어주는 법이야. 다만 감정 관리는 잘 해야 해, 알겠지?"),
        "리듬형 케미": (f"이 두 사람 호흡이 참 잘 맞아. 같은 박자로 움직이는 사이야. 같이 뭔가를 하면 "
                       f"— 일이든 생활이든 — 서로 발 맞추기가 편한 관계야. 이거 보통 복이 아니야."),
        "조율형 케미": (f"이 두 사람은 노력하면서 맞춰가는 사이야. 처음부터 척척 맞는 건 아닌데, "
                       f"조율하고 맞추다 보면 오히려 단단해지는 관계거든. 기대치를 잘 맞추는 게 핵심이야 — 이 사람아, 포기하지 마."),
    }
    type_story = type_stories.get(chem_title, "이 두 사람만의 고유한 케미가 있어 — 내 눈에는 다 보여.")

    # 축 강조
    axis_lines = []
    if complement >= 68:
        axis_lines.append("서로 부족한 걸 채워주는 힘이 특히 강해 — 함께 있으면 더 완전해지는 느낌이 드는 사이야. 이거 진짜야.")
    if alliance >= 68:
        axis_lines.append("닮은 구석이 많아서 처음부터 말이 잘 통해. 설명을 길게 안 해도 이해하는 그런 케미야 — 아이고, 부럽다.")
    if sync >= 65:
        axis_lines.append("생활 리듬이 잘 맞아. 같이 움직이면 발이 저절로 맞춰지는 사이야. 거봐, 이게 쉬운 게 아니거든.")
    if tension >= 65:
        axis_lines.append("긴장감이 좀 있는 사이야, 쯧쯧. 근데 그 자극을 어떻게 쓰느냐에 따라 이 관계가 확 달라질 수 있어 — 잘만 쓰면 약이야.")
...
def _render_chem_prescription(mine: dict, friend: dict, compatibility: dict) -> None:
    ...
    for adv in advices:
        st.markdown(
            f"<div style='background:#22101c;border-left:3px solid #92400e;"
            f"padding:9px 13px;border-radius:6px;margin-bottom:6px;"
            f"font-size:13px;color:#e8d5a0;line-height:1.7;'>{adv}</div>",
            unsafe_allow_html=True,
        )
...
def render_group_chemistry_diagnosis(ga: dict) -> None:
    ...
    st.markdown(
        f"<div style='background:#180c14;border:1px solid rgba(232,121,160,0.25);border-radius:8px;"
    ...
    if ga["over_elements"]:
        ...
        st.markdown(
            ...
            f"font-size:13px;color:#e8d5a0;line-height:1.7;'>"
            f"▲ 과다 — {over_str}<br>"
    ...
    tc, tbg = _THERMO_COLORS.get(_t["label"], ("#94a3b8", "#180c14"))
    st.markdown(
    ...
    st.markdown(
        f"<div style='background:#22101c;border:1px solid #fde68a;padding:9px 13px;"
    ...
    st.markdown(
        f"<div style='background:#180c14;border:1px solid rgba(232,121,160,0.25);border-radius:8px;"
        f"padding:9px 13px;margin-bottom:6px;font-size:13px;color:#e8d5a0;line-height:1.7;'>"
    ...
    for role in ga["roles"]:
        rc = role["role_color"]
        st.markdown(
            f"<div style='background:#22101c;border:0.5px solid #e0e0e8;border-radius:8px;"
    ...
    if ga["group_rx"]:
        for kind, hj, kw, rx_text in ga["group_rx"]:
            ...
            st.markdown(
                f"<div style='background:{'#0d1e14' if kind=='under' else '#160a12'};"
                f"border-left:4px solid {color};"
                f"padding:9px 13px;border-radius:0 6px 6px 0;"
                f"font-size:13px;color:#e8d5a0;line-height:1.7;margin-bottom:6px;'>"
    ...
    if _conclusion:
        st.markdown("#### 🧩 종합 결론")
        st.markdown(
            f"<div style='background:#180c14;border-left:4px solid #7c6cd0;"
...
def render_multi_chem_result(participants: list) -> None:
    ...
    n = len(participants)
    node_colors  = ["#3B5BDB", "#92400e", "#16a34a", "#d97706", "#7c6cd0"]
    ...
    for p in pairs:
        ...
        svg.append(
            ...
            f'rx="4" fill="#22101c" fill-opacity="0.96" stroke="{color}" stroke-width="1.2"/>'
        )
        svg.append(
    ...
    with c2:
        st.markdown(
            f"<div style='background:#160a12;border-left:3px solid #d97706;"
            f"padding:8px 12px;border-radius:6px;font-size:12px;margin-bottom:4px;'>"
    ...
    for p in sorted_pairs:
        ...
        st.markdown(
            f"<div style='background:#22101c;border:1px solid #fde68a;"
            ...
            f"<div style='font-size:13px;color:#e8d5a0;line-height:1.7;'>{gtext}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
def render_battle_result_board(mine: Dict[str, object], friend: Dict[str, object], battle_summary: Dict[str, object], compatibility: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div class="chem-hero-premium" style="text-align:center;padding:1.45rem 1.1rem;background:linear-gradient(135deg,#22101c,#12080f);border:1px solid rgba(255,145,180,.30);border-radius:26px;box-shadow:0 14px 30px rgba(255,125,165,.12);margin:.5rem 0 1rem 0;">
    ...
    if view == " 케미 한눈에":
        ...
        st.markdown(
            f"<div style='background:#22101c;border:1px solid #fde68a;padding:12px 16px;"
...
def render_selected_daily_luck_card(chart: Chart, result: Dict[str, object], target_date: date) -> None:
    ...
    st.markdown(
        f"<div style='background:#180c14;border-left:4px solid #7c3aed;padding:12px 16px;"
    ...
    # 십성 카드
    _tg_color = {
        "비견":"#a78bfa","겁재":"#fbbf24","식신":"#34d399","상관":"#fb923c",
        "편재":"#fbbf24","정재":"#fcd34d","편관":"#f87171","정관":"#60a5fa",
        "편인":"#818cf8","정인":"#6ee7b7",
    }.get(_tg_focus, "#b89a6b")
    ...
    _tg_html = (
        ...
        f"<div style='font-size:12px;font-weight:800;color:#b89a6b;margin-bottom:6px;'>🔮 오늘 천간·지지 십성 분석</div>"
        f"<div style='font-size:12px;color:#a0b8c8;line-height:1.7;margin-bottom:6px;'>"
        f"<b style='color:#d4b896;'>{html.escape(_stem_detail)}</b><br>"
        f"<b style='color:#d4b896;'>{html.escape(_branch_detail)}</b>"
        f"</div>"
    )
    if _tg_focus:
        _tg_html += (
    ...
    if _tg_caution:
        _tg_html += f"<div style='font-size:12px;color:#fbbf24;margin-top:3px;'>⚠️ {html.escape(_tg_caution)}</div>"
    if _tg_action:
    ...
    if _interactions:
        ...
        for _it in _interactions[:5]:
            _ik = _it.get("kind","")
            _ic = _KIND_COLOR.get(_ik,"#b89a6b")
            _iname = _it.get("name","")
            ...
            _inter_rows += (
                ...
                f"<span style='font-size:13px;font-weight:700;color:#fde68a;'>{html.escape(_iname)}</span>"
        ...
        st.markdown(
            ...
            "<div style='font-size:12px;font-weight:800;color:#b89a6b;margin-bottom:8px;'>"
    ...
    if _month_bg:
        st.markdown(
            f"<div style='background:#180c14;border-left:3px solid #3b82f6;"
...
def render_daily_luck_calendar(payload: Dict[str, object]) -> None:
    ...
    st.markdown("### 📅 일운 캘린더")
    st.caption("날짜별로 일운·월운, 십성, 그리고 원국과의 합·충·형·파·해 신호를 한눈에 볼 수 있어. 하루하루 흐름 보기에 딱이야.")
...
def render_daewun_text_overview(payload: dict) -> None:
    ...
    if current:
        ...
        if not daewuns:
            st.caption("거봐, 전체 대운 목록은 생년월일시 자동 산출 모드에서 나와. 직접 입력 방식이면 대운 계산이 안 되거든.")
    else:
        st.info(
            "이 사람아, 10년의 처방은 생년월일시 자동 산출로 들어왔을 때 볼 수 있어. "
            "원국을 직접 입력한 경우엔 생년월일이 없으니까 대운 계산이 안 되거든 — 자동 산출로 다시 들어와봐."
        )

    st.caption("아이고, 대운은 10년마다 확 바뀌는 큰 흐름이야. 그 안에서 네 삶의 계절이 달라진다고 보면 돼 — 무조건 좋고 나쁜 게 아니라 계절처럼 흘러가는 거야.")
...
def render_sewun_text_overview(payload: dict) -> None:
    ...
    if row_this:
        ...
        st.markdown(
            f"<div style='background:#22101c;border-left:4px solid #92400e;padding:14px 16px;"
            f"border-radius:8px;margin-bottom:12px;font-size:14px;color:#fde68a;line-height:1.8;'>"
            f"<div style='font-size:12px;color:#fcd7a0;margin-bottom:6px;'>"
        ...
        st.info(
            "이 사람아, 올해의 처방은 생년월일시 자동 산출로 들어왔을 때 볼 수 있어. "
            "원국을 직접 입력한 경우엔 세운 계산이 안 되거든 — 생년월일시로 다시 들어와봐."
        )
    ...
    if row_this and _sewun_chart and gz not in ("-", ""):
        ...
        with st.expander("🩺 올해 세운 깊이 보기 (합·충·용신·체질 분석)", expanded=False):
            # 1. 세운 글자와 용신/기신
            st.markdown(
                f"<div style='background:#22101c;border-left:4px solid #92400e;padding:12px 14px;"
                f"border-radius:6px;margin-bottom:10px;'>"
                f"<div style='font-size:12px;color:#fcd7a0;font-weight:700;margin-bottom:6px;'>올해 세운 글자 분석 · {gz}</div>"
                f"<div style='font-size:13px;color:#e8d5a0;line-height:1.9;'>"
            ...
            if _harmony_notes or _clash_notes:
                ...
                if _harmony_notes:
                    st.markdown(
                        ...
                        f"<div style='font-size:13px;color:#e8d5a0;line-height:1.8;'>"
                ...
                if _clash_notes:
                    st.markdown(
                        f"<div style='background:#160a12;border-left:3px solid #ea580c;"
                        f"padding:10px 12px;border-radius:6px;margin-bottom:8px;'>"
                ...
                st.caption("올해 세운 글자와 원국 사이에 뚜렷한 합·충 신호는 없어. 그게 꼭 나쁜 건 아니거든 — 조용하게 내 페이스대로 가는 해라는 뜻이야. 기본기 다지기 딱 좋아.")
...
def render_hanuneyo_text_explanation(payload, char, result):
    ...
    def _make_diagnosis(dom_el, wk_el, wk_pct_val, tmp, str_label, s_name, b_name):
        """진단명 — 한자·의학 용어 없이 쉬운 한글로."""
        _EL_KO = {"木":"🌿 나무 기운","火":"🔥 불 기운","土":"🪨 흙 기운","金":"⚔️ 쇠 기운","水":"💧 물 기운"}
        _TEMP  = {
            "과열": "🔴 뜨거운 체질", "열성": "🟠 따뜻한 체질",
            "온성": "🟢 균형 체질",   "냉성": "🔵 서늘한 체질", "한랭": "❄️ 차가운 체질",
        }
        _t_key = ("과열" if tmp>=73 else "열성" if tmp>=62 else "온성" if tmp>=53 else "냉성" if tmp>=42 else "한랭")
        _temp_disp = _TEMP[_t_key]
        _str_disp = {
            "극신강":"기운 매우 강함", "신강":"기운 강함",
            "중화":"기운 균형",       "신약":"기운 약함", "극신약":"기운 매우 약함",
        }.get(str_label, "기운 균형")
        _dom_ko = _EL_KO.get(dom_el, dom_el)
        _title = f"{_dom_ko}  ·  {_str_disp}"
        _wk_part = ""
        if wk_el and wk_el not in ("-", dom_el):
            _wk_ko = _EL_KO.get(wk_el, wk_el)
            _f = float(wk_pct_val)
            _wk_part = f"  ·  {_wk_ko} {'거의 없음' if _f<8 else '부족' if _f<16 else '살짝 아쉬움'}"
        _title += _wk_part
        _sub = f"{_temp_disp}  ·  {s_name}({day_stem}) · {b_name}({day_branch})"
        return f"{_sub}\n{_title}"
    ...
    def _make_grandma_intro() -> str:
        ...
        # 한글 소문자 키 (STEMS/BRANCHES 반환값과 동일)
        _EL_KO   = {"목":"나무(木)","화":"불(火)","토":"흙(土)","금":"쇠(金)","수":"물(水)"}
        _EL_ICON = {"목":"🌿","화":"🔥","토":"🪨","금":"⚔️","수":"💧"}
        ...
        _temp_story = {
            "과열성": "불기운이 확실히 강한 과열 체질이야. 열정은 넘치는데 자칫 소진이 빠를 수 있어. 차갑고 조용한 환경, 물 기운이 이 체질한테는 보약이야.",
            "열성":   "따뜻한 기운이 강한 체질이야. 어딜 가나 온기를 뿜어내거든. 너무 달아오르면 과열되기 쉬우니 가끔은 조용히 쉬는 시간을 챙겨봐.",
            "온성":   "사주 온도가 딱 균형권이야. 차갑지도 뜨겁지도 않은 — 가장 안정적인 체질 중 하나야. 이 균형을 유지하는 게 이 체질의 핵심이야.",
            "냉성":   "서늘한 냉성 체질이야. 차분하고 냉정하게 세상을 보는 눈이 있거든. 너무 차가워지면 고립되기 쉬우니 따뜻한 사람들 곁에 있는 게 중요해.",
            "한랭성": "한랭 체질이야. 깊이 생각하는 힘은 있지만 움직임이 무거워질 수 있어. 따뜻한 음식, 따뜻한 사람, 햇볕 — 이게 이 체질한테 제일 필요한 처방이야.",
        }

        # ② 주성분
        _dom_story = {
            "목": "사주 주성분은 🌿<b>나무(木) 기운</b>이야. 새싹이 올라오듯 앞으로 뻗어나가고 싶어 하는 — 새로운 걸 시작하고 도전하는 힘이 강한 사주야.",
            "화": "사주 주성분은 🔥<b>불(火) 기운</b>이야. 활활 타오르며 주변을 환하게 밝히는 — 드러내고 표현하고 이끌어가는 힘이 있는 사주야.",
            "토": "사주 주성분은 🪨<b>흙(土) 기운</b>이야. 땅이 만물을 품듯 묵직하게 자리 잡고 버티는 — 중심을 잡고 균형을 맞추는 힘이 강한 사주야.",
            "금": "사주 주성분은 ⚔️<b>쇠(金) 기운</b>이야. 쇳덩이처럼 단단하고 날카로운 — 기준을 딱 세우고 확실하게 끝내는 힘이 있는 사주야.",
            "수": "사주 주성분은 💧<b>물(水) 기운</b>이야. 물이 흘러가듯 부드럽게 스며드는 — 상황에 따라 유연하게 적응하고 깊이 생각하는 힘이 강한 사주야.",
        }

        # ③ 신강/신약
        _str_story = {
            "극신강": "타고난 힘이 극도로 강한 편이야. 혼자서도 거뜬히 밀고 나가는 힘이 있거든. 다만 이 강한 힘이 자칫 주변과 충돌을 만들 수 있어 — 힘 빼는 법, 유연하게 받아주는 법을 익히면 이 사주의 힘이 두 배가 돼.",
            "신강":   "타고난 힘이 강한 편이야. 스스로 방향을 잡고 밀어붙이는 뚝심이 있거든. 주도적으로 사는 게 이 사주한테 잘 맞아. 지나치게 고집 세게 굴지만 않으면 충분히 잘 해.",
            "중화":   "타고난 힘이 균형 잡혀 있어. 혼자 미는 힘과 도움받는 힘이 적당히 맞아. 극단적으로 강하거나 약하지 않으니 — 어떤 환경에 놔도 제 몫을 하는 사주야.",
            "신약":   "타고난 힘이 약한 편이야. 혼자 뚝심으로 밀기보다 좋은 환경, 좋은 사람이 곁에 있을 때 훨씬 잘 피어나거든. 이 사주는 환경을 잘 고르는 게 핵심이야.",
            "극신약": "타고난 힘이 많이 부족한 편이야. 혼자 돌파하기보다 도움을 잘 받고 잘 활용하는 게 이 사주의 운용 방식이거든. 도움받는 걸 잘 하는 사람이 오히려 더 멀리 가는 경우가 많아.",
        }

        # ④ 결핍
        _wk_story = {
            "목": "나무 기운이 부족하니 새로운 걸 시작하는 힘이 좀 달릴 수 있어. 작은 도전부터 하나씩 해봐.",
            "화": "불 기운이 적으니 표현하고 드러내는 게 쑥스러울 수 있어. 조금씩 소통하는 연습을 해봐.",
            "토": "흙 기운이 부족하니 중심을 잡기가 쉽지 않을 수 있어. 규칙적인 생활이 참 중요해.",
            "금": "쇠 기운이 적으니 딱 끊고 결정하는 게 좀 어려울 수 있어. 하나씩 끝내는 습관을 들여봐.",
            "수": "물 기운이 부족하니 쉬어가는 여유가 좀 필요해. 혼자 조용히 생각하는 시간을 가져봐.",
        }

        # ⑤ 용신 처방
        _rx_map = {
            "목": "봄 기운, 초록색 환경, 새로운 시작과 도전이 힘이 돼.",
            "화": "밝은 공간, 사람들 속에서 활동하는 것이 에너지를 살려줘.",
            "토": "규칙적인 루틴, 안정된 환경, 묵묵히 지키는 것이 핵심이야.",
            "금": "하나씩 끝내는 습관, 명확한 기준 세우기가 이 사주를 살려줘.",
            "수": "혼자 쉬는 시간, 조용한 사색, 깊이 생각하는 시간이 보약이야.",
        }
        ...
        # 2. 주성분 + 신강약
        _dom_sent = _dom_story.get(str(dominant_el), f"사주 주성분은 {_EL_ICON.get(str(dominant_el),'')}{_EL_KO.get(str(dominant_el), str(dominant_el))}야.")
        _str_sent = _str_story.get(strength_label, "")
        parts.append(f"{_dom_sent}<br>{_str_sent}")

        # 3. 결핍
        if weak_el and str(weak_el) not in ("-", str(dominant_el)):
            _wk_icon = _EL_ICON.get(str(weak_el), "")
            _wk_ko   = _EL_KO.get(str(weak_el), str(weak_el))
            _wk_pct_f = float(weak_pct or 0)
            _wk_degree = "거의 없다시피 해" if _wk_pct_f < 8 else "꽤 부족해" if _wk_pct_f < 16 else "살짝 아쉬운 편이야"
            _wk_sent  = _wk_story.get(str(weak_el), "")
            parts.append(f"그런데 있잖아, {_wk_icon}<b>{_wk_ko}</b>이 {_wk_degree}. {_wk_sent}")

        # 4. 용신/기신 + 처방
        if _fw_primary_list:
            _pri_str = "·".join(_EL_ICON.get(e,"") + _EL_KO.get(e, e) for e in _fw_primary_list)
            _brd_str = ("·".join(_EL_ICON.get(e,"") + _EL_KO.get(e, e) for e in _fw_burden_list)) if _fw_burden_list else ""
            _rx = _rx_map.get(str(_fw_primary_list[0]), "")
            _yong_sent = (
                f"이 사주에서 제일 필요한 기운은 <b>{_pri_str}</b>야. "
                f"이 기운이 들어올 때 흐름이 열리고 좋은 일이 생겨."
            )
            if _brd_str:
                _yong_sent += f" 반대로 <b>{_brd_str}</b> 기운은 좀 부담이 되는 방향이야 — 이 기운이 강해지는 시기엔 조심하는 게 좋아."
            if _rx:
        ...
        return "".join(
            f"<div style='margin-bottom:11px;line-height:1.95;font-size:14px;color:#fef3c7;'>{p}</div>"
            for p in parts
        )
    ...
    for _an, _icon_a in [("기초체력","💪"),("흐름과 연결","🔄"),("현실작동력","⚙️")]:
        ...
        _sc_a = {"강함":"#4ade80","보통":"#fbbf24","약함":"#f87171"}.get(_st_a,"#b89a6b")
    ...
    def _row(label, value, value_color="#fef3c7"):
        return (
            f"<tr>"
    ...
    # 진단서 HTML
    _cert_html = f"""
<div style='background:linear-gradient(160deg,#22101c 0%,#1a0d18 100%);
...
"""
    ...
    if _t_rx:
        st.markdown(
            f"<div style='background:#160a12;border-left:4px solid #f59e0b;"
            f"padding:12px 14px;border-radius:6px;margin-bottom:8px;'>"
    ...
    with st.expander("🔄 흐름과 연결", expanded=False):
        ...
        for _an, _pct, _st in _axis_results:
            _sc, _sbg = _label_colors.get(_st, ("#94a3b8", "#180c14"))
            _bars_html += (
    ...
    with st.expander("✨ 신살·특수신호", expanded=False):
        ...
        if _sh_hits:
            ...
            _SH_TONE = {
                ...
                "역마":     ("🟠", "중립 신호", "#160a12", "#ea580c"),
                "도화":     ("🩷", "중립 신호", "#1a0d18", "#f59e0b"),
                "화개":     ("🟣", "중립 신호", "#180c14", "#7c3aed"),
                "공망":     ("⚪", "주의 신호", "#180c14", "#94a3b8"),
            }
            # 신살별 의사 텍스트
            _SH_DOC_TEXT = {
            ...
            st.markdown(
                "<div style='background:#180c14;border:1px solid #3a2830;border-radius:8px;"
    ...
    with st.expander("⚖️ 신약신강판단", expanded=False):
        ...
        st.markdown(
            "<div style='background:#180c14;border:1px solid #3b82f6;border-radius:8px;"
        ...
        for _tn, _tm, _tv, _tlevel, _tlabel in _sorted_ten:
            ...
            _badge_color = {"strong": "#fbbf24", "mid": "#a0c4cc", "weak": "#7ecfa8"}.get(_tlevel, "#b89a6b")
            _ten_rows_html += (
                f"<div style='margin-bottom:10px;'>"
        ...
        st.markdown(
            f"<div style='background:#180c14;border-left:4px solid #60a5fa;padding:12px 14px;"
            f"border-radius:6px;margin-bottom:10px;'>"
        ...
        st.markdown(
            ...
            f"<div style='font-size:13px;color:#e8d5a0;line-height:1.95;'>{_YONG_CONCEPT}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
        if _logic_explain:
            st.markdown(
                ...
                f"<div style='font-size:13px;color:#e8d5a0;line-height:1.95;'>{_logic_explain}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
        if _fw_primary:
        ...
        if _fw_bindings:
            st.markdown(
                ...
                "<div style='font-size:13px;color:#e8d5a0;line-height:1.9;'>"
                + "".join(
        ...
        if _fw_clashes:
            st.markdown(
                "<div style='background:#160a12;border-left:4px solid #ea580c;padding:10px 14px;"
                "border-radius:6px;margin-bottom:8px;'>"
        ...
        for _fw in _fw_flows:
            ...
            if not _doc or _fn == "식신제살/상관패인":
                continue
            _fc, _fbg = _STRENGTH_COLOR.get(_fs, ("#6b7280", "#180c14"))
            # 원국 근거 구성
            _fsrc_items = []
            ...
            _fsrc_html = (
                ...
                "<div style='font-size:11px;color:#b89a6b;font-weight:700;margin-bottom:3px;'>📍 원국 어디서 이 흐름이 나오냐면</div>"
                + "<br>".join(_fsrc_items) +
                "</div>"
            ) if _fsrc_items else ""
            st.markdown(
                ...
                f"</div>"
                f"<div style='font-size:13px;color:#e8d5a0;line-height:1.8;'>{_doc}</div>"
                + _fsrc_html +
                f"</div>",
                unsafe_allow_html=True,
            )
...
def render_saju_yebo_page(payload: dict) -> None:
    """📡 사주예보 — 종합예보(원국+4운 가중치 분석) + 4운 카드 + 상세 서브메뉴."""
    from datetime import date as _date
    chart      = payload.get("chart")
    result     = payload.get("result", {}) or {}
    luck_flow  = payload.get("luck_flow") or {}
    ...
    # ── 대운/세운/월운/일운 간지 추출 ─────────────────────────
    _dw_gz = (luck_flow.get("current_daewun") or {}).get("ganzhi", "") or ""
    ...
    _strength_label = str(result.get("strength_label", "중화"))
    _dominant_el = ""
    snap = _single_front_snapshot(payload)
    _dominant_el = str(snap.get("dominant", ("-",0))[0] if isinstance(snap.get("dominant"), tuple) else snap.get("dominant", "-"))

    _EL_KO   = {"목":"나무(木)","화":"불(火)","토":"흙(土)","금":"쇠(金)","수":"물(水)"}
    _EL_ICON = {"목":"🌿","화":"🔥","토":"🪨","금":"⚔️","수":"💧"}
    _CLASH_KO = {"충":"충(衝)","형":"형(刑)","파":"파(破)","해":"해(害)","원진":"원진(怨嗔)"}
    _MERGE_KO = {"육합":"육합(六合)","삼합완성":"삼합(三合)","반합":"반합(半合)","방합":"방합(方合)"}

    # 4운 레이어 정의 (가중치: 대운40 세운30 월운20 일운10)
    _layers = [
        ("🌊 대운", _dw_gz, 0.40, "#93c5fd"),
        ("🌸 세운", _sw_gz, 0.30, "#c4b5fd"),
        ("🌼 월운", _ww_gz, 0.20, "#86efac"),
        ("☀️ 일운", _iw_gz, 0.10, "#fde68a"),
    ]

    # 각 레이어 분석
    _layer_analysis = []
    for _lname, _gz, _wt, _lcol in _layers:
        if not _gz or len(_gz) < 2:
            _layer_analysis.append({
                "name": _lname, "gz": _gz or "?", "weight": _wt, "color": _lcol,
                "el_stem": "-", "el_branch": "-",
                "yong_stem": None, "yong_branch": None,
                "interactions": [], "score": 0.0,
            })
            continue
        _stem_c   = _gz[0]
        _branch_c = _gz[1] if len(_gz) > 1 else ""
        _el_s = STEMS.get(_stem_c, {}).get("element", "-")
        _el_b = BRANCHES.get(_branch_c, {}).get("element", "-") if _branch_c else "-"
        def _yong_tag(el):
            if el in _primary_els: return "용신"
            if el in _burden_els:  return "기신"
            return "중립"
        _ys = _yong_tag(_el_s)
        _yb = _yong_tag(_el_b)
        # 이 레이어의 종합 용신 점수 (용신=+1, 기신=-1, 중립=0)
        _ys_score = (1 if _ys == "용신" else -1 if _ys == "기신" else 0)
        _yb_score = (1 if _yb == "용신" else -1 if _yb == "기신" else 0)
        _raw_score = (_ys_score * 0.45 + _yb_score * 0.55) * _wt  # 지지 비중 약간 높게
        # 지지 × 원국 합충 검출
        _inters = []
        if chart and _branch_c:
            try:
                _inters = detect_luck_branch_interactions(chart, _branch_c)
            except Exception:
                _inters = []
        # 합이면 보너스, 충·형이면 페널티
        for _it in _inters:
            _it_type = str(_it.get("type",""))
            if _it_type in ("삼합완성","육합","반합","방합"):
                _raw_score += 0.05 * _wt
        ...
        _layer_analysis.append({
            ...
            "el_stem": _el_s, "el_branch": _el_b,
            "yong_stem": _ys, "yong_branch": _yb,
            "interactions": _inters,
            "score": _raw_score,
        })

    # 종합 점수 합산
    _total_score = sum(la["score"] for la in _layer_analysis)

    # ─────────────────────────────────────────────────────────
    # 표시 섹션: 4운 카드 → 분석표 → 오행 변화 → 종합 텍스트
    # ─────────────────────────────────────────────────────────
    ...
    _best_layer  = max(_layer_analysis, key=lambda la: la.get("score", 0), default=None)
    _worst_layer = min(_layer_analysis, key=lambda la: la.get("score", 0), default=None)

    # ── 원국 기운 소개 ──────────────────────────────────────────
    _dom_ko   = _EL_KO.get(_dominant_el, _dominant_el)
    _dom_icon = _EL_ICON.get(_dominant_el, "")
    _str_ko = {
        "극신강": "기운이 넘쳐흐를 정도로 강한",
        "신강":   "기운이 든든하게 강한",
        "중화":   "기운이 고르게 균형 잡힌",
        "신약":   "기운이 좀 약해서 받쳐줘야 하는",
        "극신약": "기운이 많이 약한",
    }.get(_strength_label, "기운이 나름대로 흘러가는")
    ...
    # ── 용신/기신 ───────────────────────────────────────────────
    if _primary_els:
        _pri_str = "·".join(_EL_ICON.get(e, "") + _EL_KO.get(e, e) for e in _primary_els)
        _pri_part = (
            f"이 사주에서 제일 필요한 기운은 <b>{_pri_str}</b>야. "
            "이걸 용신이라고 하는데, 쉽게 말하면 이 기운이 들어올 때 사주가 살아나고 흐름이 열려. "
            "직업, 환경, 사람 — 뭐든 이 방향과 잘 맞으면 잘 풀리는 거야."
        )
        if _burden_els:
            _brd_str = "·".join(_EL_ICON.get(e, "") + _EL_KO.get(e, e) for e in _burden_els)
            _pri_part += (
                f"<br>반대로 <b>{_brd_str}</b> 기운은 이 사주한테 부담이야. "
                "이 기운이 강해지는 시기엔 뭔가 막히고 힘든 느낌이 드는 거야 — 그렇다고 손 놓는 게 아니고, "
                "그때일수록 기본에 충실하게 버텨야 해."
            )
        _yebo_parts.append(_pri_part)

    # ── 대운 ────────────────────────────────────────────────────
    _dw_la = next((la for la in _layer_analysis if "대운" in la["name"]), None)
    if _dw_la and _dw_la.get("stem"):
        _dw_es = _EL_ICON.get(_dw_la["el_stem"], "") + _EL_KO.get(_dw_la["el_stem"], _dw_la["el_stem"])
        _dw_eb = _EL_ICON.get(_dw_la["el_branch"], "") + _EL_KO.get(_dw_la["el_branch"], _dw_la["el_branch"])
        _dw_ys = _dw_la.get("yong_stem", "중립")
        _dw_yb = _dw_la.get("yong_branch", "중립")
        _dw_tone = (
            "아주 좋은 방향으로 흘러가고 있어" if _dw_la["score"] > 0.10
            else "조심스러운 방향이야" if _dw_la["score"] < -0.10
            else "어느 쪽으로도 크게 치우치지 않는 방향이야"
        )
        _inters_dw = _dw_la.get("interactions", [])
        _inter_dw_txt = ""
        if _inters_dw:
            _it0 = _inters_dw[0]
            _ko0 = _CLASH_KO.get(_it0.get("type", ""), None) or _MERGE_KO.get(_it0.get("type", ""), None) or _it0.get("type", "")
            _inter_dw_txt = f" 게다가 원국과 <b>{_ko0}</b> 신호까지 있어."
        _yebo_parts.append(
            f"지금 10년 큰 흐름인 <b>대운 {_dw_la['gz']}</b>을 봐. "
            f"천간은 <b>{_dw_es}({_dw_ys})</b>, 지지는 <b>{_dw_eb}({_dw_yb})</b>으로 들어오고 있어. "
            f"이 대운은 전체적으로 <b>{_dw_tone}</b>.{_inter_dw_txt} "
            f"10년 흐름의 큰 판이 여기서 결정되니까, 이 기운 방향이 얼마나 내 원국이랑 맞느냐가 제일 중요해."
        )

    # ── 세운 ────────────────────────────────────────────────────
    _sw_la = next((la for la in _layer_analysis if "세운" in la["name"]), None)
    if _sw_la and _sw_la.get("stem"):
        _sw_es = _EL_ICON.get(_sw_la["el_stem"], "") + _EL_KO.get(_sw_la["el_stem"], _sw_la["el_stem"])
        _sw_eb = _EL_ICON.get(_sw_la["el_branch"], "") + _EL_KO.get(_sw_la["el_branch"], _sw_la["el_branch"])
        _sw_ys = _sw_la.get("yong_stem", "중립")
        _sw_yb = _sw_la.get("yong_branch", "중립")
        _sw_tone = (
            "힘을 실어주는 해야" if _sw_la["score"] > 0.05
            else "조심해야 하는 해야" if _sw_la["score"] < -0.05
            else "어느 방향으로도 크게 치우치지 않는 해야"
        )
        _yebo_parts.append(
            f"올해 세운 <b>{_sw_la['gz']}</b>는 어때? "
            f"천간 <b>{_sw_es}({_sw_ys})</b>, 지지 <b>{_sw_eb}({_sw_yb})</b>으로 흘러가. "
            f"대운이라는 큰 판 위에 올해 파도가 얹히는 건데, 올해는 <b>{_sw_tone}</b>.<br>"
    ...
    if _mw_la and _mw_la.get("stem"):
        _mw_tone = (
            "힘이 붙는 달이야" if _mw_la["score"] > 0.02
    ...
    for _la in _layer_analysis:
        for _it in (_la.get("interactions") or []):
            _it_t = str(_it.get("type", ""))
            _it_n = str(_it.get("name", ""))
            _lname_s = _la["name"].replace("🌊 ", "").replace("🌸 ", "").replace("🌼 ", "").replace("☀️ ", "")
            _ko_c = _CLASH_KO.get(_it_t, None)
            _ko_m = _MERGE_KO.get(_it_t, None)
            if _ko_c:
                _all_clashes2.append(f"{_lname_s} <b>{_la.get('gz','')}</b> — {_ko_c}")
            elif _ko_m:
                _all_merges2.append(f"{_lname_s} <b>{_la.get('gz','')}</b> — {_ko_m}")

    if _all_merges2:
        _merge_txt = ", ".join(_all_merges2[:3])
        _yebo_parts.append(
            f"합(合) 신호가 보여 — {_merge_txt}. "
            "합이 성립되면 기운이 한 곳으로 모이면서 강해져. "
            "이 흐름이 살아 있는 동안은 협력·결합·계획 성사 쪽으로 에너지가 열려 있어."
        )
    if _all_clashes2:
        _clash_txt = ", ".join(_all_clashes2[:3])
        _yebo_parts.append(
            f"충·형(衝·刑) 신호도 있어 — {_clash_txt}. "
            "충은 기운이 서로 부딪히는 거야. 변화, 이동, 예상치 못한 일이 생길 수 있어. "
            "나쁜 게 아니라 흔들리는 신호니까 — 흔들릴 때 버티면 새 길이 열리는 거야."
        )
    if not _all_merges2 and not _all_clashes2:
        _yebo_parts.append(
            "원국과 운 사이에 뚜렷한 합충 신호는 지금 보이지 않아. "
            "큰 파동 없이 흘러가는 시기야 — 이럴 때가 오히려 차분하게 준비하기 좋아."
        )

    # ── 처방 + 종합 평가 ─────────────────────────────────────────
    if _best_layer and _best_layer.get("score", 0) > 0.05:
        _bl_n  = _best_layer["name"]
        _bl_gz = _best_layer.get("gz", "")
        _yebo_parts.append(
            f"{_PERIOD_DESC.get(_bl_n, _bl_n)} <b>{_bl_gz}</b> 기운이 용신 방향으로 들어오고 있어 — "
            f"지금이 그 기회야 — 잘 잡아봐."
        )
    if _worst_layer and _worst_layer.get("score", 0) < -0.05:
        _wl_n  = _worst_layer["name"]
        _wl_gz = _worst_layer.get("gz", "")
        _yebo_parts.append(
            f"{_PERIOD_DESC.get(_wl_n, _wl_n)} <b>{_wl_gz}</b> 기운이 원국에 부담을 주고 있어. "
            f"지금은 무리하지 않고 아끼는 게 좋아."
        )

    if _total_score >= 0.25:
        _final_yebo = (
            "종합하면, 지금은 <b>아주 좋은 흐름</b>이야. "
            "용신 기운이 여러 층에서 들어오고 있고, 원국이 지금 운을 잘 받아내고 있어. "
            "이럴 때 큰 결정, 새 시작, 도전 — 다 좋아. 준비된 거 있으면 지금 움직여봐."
        )
    elif _total_score >= 0.10:
        _final_yebo = (
            "종합하면, 지금은 <b>전체적으로 괜찮은 흐름</b>이야. "
            "완벽하진 않아도 운이 등 뒤에서 밀어주고 있어. "
            "무리하지 않으면서 꾸준히 가면 좋은 결과가 차곡차곡 쌓이는 시기야."
        )
    elif _total_score >= -0.10:
        _final_yebo = (
            "종합하면, 지금은 <b>균형점</b>이야. "
            "운에서 특별히 강한 도움도 없고, 특별한 방해도 없어. "
            "결국 내가 어떻게 움직이느냐가 결과를 만드는 시기거든 — 기본기를 단단히 하고 꾸준히 해봐."
        )
    elif _total_score >= -0.25:
        _final_yebo = (
            "종합하면, 지금은 <b>조심해야 할 시기</b>야. "
            "기신 방향 기운이 좀 강하게 들어오고 있어. "
            "무리하게 밀고 나가기보다 지키고 아끼는 전략이 맞아. 큰 결정은 잠깐 미루는 게 좋아."
        )
    else:
        _final_yebo = (
    ...
    for row in rows:
        ...
        <div class="ability-bar-row">
            <div class="ability-bar-head">
                <span class="ability-bar-title">{info['icon']} {info['primary']} <span style="font-size:.88rem;color:#d4b896;">({info['legacy']})</span></span>
                <span class="ability-bar-score">{score:.1f} / {max_score:.0f}</span>
            </div>
            <div style="font-size:.82rem;color:#d4b896;font-weight:760;margin-bottom:.32rem;">{info['short']} · {info['meaning']}</div>
...
def render_score_formula_diagram(result: Dict[str, object], prefix: str = "single") -> None:
    ...
    st.markdown(f"""
    <div style="background:#22101c;border:1px solid rgba(230,164,184,0.45);border-radius:18px;padding:1rem 1rem 1.1rem 1rem;margin:0.35rem 0 1rem 0;">
        ...
        <div style="display:flex;flex-wrap:wrap;align-items:center;gap:0.55rem;">
            ...
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">+</div>
            <div style="flex:1 1 150px;background:#22101c;border:1px solid #f2ced8;border-radius:14px;padding:0.8rem;text-align:center;"><div style="font-size:0.94rem;color:#d4b896;font-weight:950;">신살·공망</div><div style="font-size:1.18rem;color:#d95b84;font-weight:900;">{shinsal:+.1f}</div><div style="font-size:.88rem;color:#d4b896;">체감 보정</div></div>
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">=</div>
    ...
    def _pillar_col(label: str, p: Pillar | None, highlight: bool = False) -> str:
        if p is None:
            return (
                ...
                "<div class='manse-big stem' style='background:#180c14;color:#d4b896!important;'>?</div>"
                "<div class='manse-ko'>시주 미상</div>"
                "<div class='manse-role'>-</div>"
                "<div class='manse-big branch' style='background:#180c14;color:#d4b896!important;'>?</div>"
...
def _mini_bar_html(label: str, value: float, max_value: float = 100.0, caption: str = "") -> str:
    ...
    caption_html = f"<div style='font-size:.88rem;color:#d4b896;margin-top:.18rem;'>{html.escape(str(caption))}</div>" if caption else ""
    ...
    return (
        ...
        f"<div style='font-weight:900;color:#d4b896;'>{html.escape(str(label))}</div>"
        f"<div style='font-weight:900;color:#f59e0b;'>{value_label}</div>"
        f"</div>"
        f"<div style='height:10px;background:#180c14;border-radius:99px;overflow:hidden;border:1px solid rgba(230,164,184,.35);'>"
...
def _compatibility_polygon_svg(axis_data: List[Tuple[str, float, str]]) -> str:
    ...
    for idx, ((label, val, _caption), a) in enumerate(zip(axis_data, angles)):
        ...
        if idx == 0:
            ...
            ly += 5
        labels.append(
            f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='900' fill='#d4b896'>"
            f"{html.escape(label)}</text>"
        )

    poly = f"<polygon points='{' '.join(points)}' fill='rgba(214,66,115,.22)' stroke='#92400e' stroke-width='3'/>"
    dots = ""
    for pnt in points:
        x, y = pnt.split(",")
        dots += f"<circle cx='{x}' cy='{y}' r='4.5' fill='#92400e'/>"
...
def render_element_distribution_gauge(result: Dict[str, object]) -> None:
    ...
    st.markdown("<div style='background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:18px;padding:.9rem 1rem;'>" + "".join(html_rows) + "</div>", unsafe_allow_html=True)
...
def render_strength_scale(result: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div style="background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:18px;padding:1rem;margin:.35rem 0;">
...
def render_climate_gauge(result: Dict[str, object]) -> None:
    ...
    st.markdown("#### 🌡️ 조후 감각")
    climate_html = (
        "<div style='background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:18px;padding:.9rem 1rem;'>"
...
def render_useful_signal(useful: Dict[str, object], result: Dict[str, object] | None = None) -> None:
    ...
    st.markdown("<div style='display:flex;flex-wrap:wrap;gap:.65rem;'>" + "".join(
        f"<div style='flex:1 1 180px;background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:16px;padding:.82rem;'><div style='font-weight:950;color:#d4b896;'>{html.escape(k)}</div><div style='color:#b89a6b;font-weight:800;margin-top:.35rem;'>{html.escape(v)}</div></div>" for k, v in rows
...
def render_interaction_signal(result: Dict[str, object]) -> None:
    ...
    html_block = "<div style='background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:18px;padding:.9rem 1rem;'>"
...
def render_luck_timeline(luck_flow: Dict[str, object] | None) -> None:
    ...
    for r in rows:
        ...
        cards.append(f"<div style='flex:1 1 120px;background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:16px;padding:.82rem;'><div style='font-weight:950;color:#d4b896;'>{safe_year}</div><div style='font-weight:900;color:#92400e;margin:.25rem 0;'>{safe_tag}</div><div style='height:8px;background:#180c14;border-radius:99px;overflow:hidden;'><div style='height:100%;width:{pct:.1f}%;background:linear-gradient(90deg,#f8b3ca,#bda7ff);'></div></div><div style='font-size:.88rem;color:#d4b896;margin-top:.25rem;'>{safe_sewun} · {score:.1f}점</div></div>")
...
def render_compatibility_axis_gauge(compatibility: Dict[str, object]) -> None:
    ...
    html_block = "<div style='background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:18px;padding:.9rem 1rem;margin:.5rem 0;'>"
...
def render_patch_notes() -> None:
    ...
    for note in PATCH_NOTES:
        ...
        st.markdown(
            f"<div style='background:#22101c;border:1.5px solid rgba(214,66,115,.18);border-radius:16px;padding:1rem 1.2rem;margin:.65rem 0;box-shadow:0 4px 12px rgba(214,66,115,.07);'>"
            f"<div style='display:flex;align-items:center;gap:.35rem;margin-bottom:.55rem;'>"
            f"<span style='font-size:1.05rem;font-weight:950;color:#fde68a;'>{note['version']}</span>"
            f"{tag_html}"
        ...
        for icon_title, desc in note["items"]:
            st.markdown(
                f"<div style='margin:.35rem 0;padding:.4rem .65rem;background:#180c14;border-radius:10px;'>"
                f"<span style='font-weight:900;color:#f59e0b;font-size:.88rem;'>{icon_title}</span> "
                f"<span style='font-size:.85rem;color:#d4b896;'>{desc}</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
...
def render_calculation_steps() -> None:
    ...
    for i, step in enumerate(steps, start=1):
        html_block += f"<div style='flex:1 1 160px;background:#22101c;border:1px solid rgba(230,164,184,.28);border-radius:14px;padding:.72rem;'><div style='font-weight:950;color:#92400e;'>STEP {i}</div><div style='font-weight:850;color:#d4b896;'>{html.escape(step)}</div></div>"
    html_block += "</div>"
    st.markdown(html_block, unsafe_allow_html=True)
...
def render_detail_tabs(payload: Dict[str, object], prefix: str | None = None) -> None:
    ...
    if selected_detail == "핵심 요약":
        ...
        for row in info_rows:
            info_html += (
                f"<div style='background:#22101c;border:1px solid rgba(230,164,184,.32);border-radius:16px;padding:.82rem .9rem;'>"
                f"<div style='font-weight:950;color:#d4b896;font-size:.88rem;'>{html.escape(str(row['항목']))}</div>"
                f"<div style='font-weight:850;color:#fde68a;line-height:1.55;margin-top:.28rem;white-space:normal;overflow:visible;text-overflow:clip;'>{html.escape(str(row['요약']))}</div>"
                f"</div>"
            )
        info_html += "</div>"

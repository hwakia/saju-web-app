...
def _interaction_detail_html(interactions: list) -> str:
    ...
    # 타입별 배경색/아이콘/제목색
    TYPE_STYLE = {
        "삼합":  {"bg": "#f5eaed", "border": "#c9a0b0", "icon": "🔺", "color": "#1e1b4b"},
    ...
    TYPE_DESC = {
        "삼합": {
            "성립": (
                "지지 12글자를 네 가지 기운 덩어리로 묶는 게 삼합이야. "
                "① 申子辰은 수(水)국, ② 亥卯未는 목(木)국, ③ 寅午戌은 화(火)국, ④ 巳酉丑은 금(金)국인데, "
                "같은 국의 세 글자가 원국에 다 들어와 있으면 삼합이 이루어지는 거야. "
                "월지에 가운데 글자(子·卯·午·酉)가 있으면 그 힘이 훨씬 강해지거든."
            ),
            "효과": (
                "그 오행 기운이 한데 뭉쳐서 팍 세져. "
                "그게 용신이면 능력이 살아나고 흐름도 좋아지는데, "
                "기신이면 고집이 세지거나 한쪽으로 너무 쏠리는 게 문제가 될 수 있어. "
                "합 중에서 제일 강한 결속이라, 사주에 이게 있으면 그 기운이 원국 전체에 깔리는 느낌이야."
            ),
        },
        "방합": {
            "성립": (
                "같은 계절·방위의 지지 세 글자가 한 원국에 다 모이면 방합이야. "
                "① 寅卯辰은 봄·동방·목, ② 巳午未는 여름·남방·화, "
                "③ 申酉戌은 가을·서방·금, ④ 亥子丑은 겨울·북방·수거든. "
                "계절 에너지가 한 방향으로 쏟아지는 구조야."
            ),
            "효과": (
                "삼합이랑 비슷하게 그 오행이 강해지는데, 방합은 특히 방향성이 강해. "
                "한번 마음먹으면 그쪽으로 확 밀어붙이는 힘이 있거든. "
                "월지가 여기 포함되면 계절 기운까지 더해져서 체감이 훨씬 강하게 나와."
            ),
        },
        "반합": {
            "성립": (
                "삼합 세 글자 중에서 가운데 핵심 글자(子·卯·午·酉)가 포함된 두 글자만 있으면 반합이야. "
                "삼합의 절반짜리라고 보면 되거든."
            ),
            "효과": (
                "삼합 방향으로 기운이 기울긴 하는데, 완전히 합쳐지지는 않아. "
                "그 오행 쪽으로 성향이 기울어지는 느낌? "
                "나머지 한 글자가 나중에 세운이나 대운에서 들어오면 그때 삼합이 완성되기도 해."
            ),
        },
        "육합": {
            "성립": (
                "子丑, 寅亥, 卯戌, 辰酉, 巳申, 午未 — 이렇게 짝꿍처럼 묶인 여섯 쌍이 있어. "
                "이 두 글자가 원국에 함께 들어와 있으면 육합이 성립하는 거야."
            ),
            "효과": (
                "두 지지가 합쳐지면서 새 오행 색깔을 띠게 돼. "
                "子丑이 만나면 토(土), 寅亥는 목(木), 卯戌은 화(火), 辰酉는 금(金), 巳申은 수(水), 午未는 토(土)야. "
                "삼합이나 방합보다는 힘이 약하고, 두 글자가 서로 묶이다 보니 각자 혼자일 때보다 독립 작용이 좀 줄어드는 편이야."
            ),
        },
        "충": {
            "성립": (
                "지지에서 서로 정반대 자리에 있는 여섯 쌍이 충이야. "
                "子午, 丑未, 寅申, 卯酉, 辰戌, 巳亥 — 이 두 글자가 원국에 같이 있으면 충이 성립해. "
                "월지나 일지가 끼어 있으면 그 충격이 더 강하게 와."
            ),
            "효과": (
                "두 기운이 서로 충돌하면서 한쪽이나 양쪽 다 흔들리거든. "
                "환경이 갑자기 바뀐다거나 이직·이별·이동처럼 변동이 생기는 신호로 볼 수 있어. "
                "꼭 나쁜 것만은 아니야 — 기신 글자가 충으로 흔들리면 오히려 막혀 있던 게 풀리기도 하니까. "
                "어떤 글자가 충을 받느냐가 핵심이야."
            ),
        },
        "형": {
            "성립": (
                "형(刑)은 합도 충도 아닌 독특한 긴장 구조야. "
                "子卯는 무례지형, 寅巳申은 무은지형 삼형, 丑戌未는 지세지형 삼형인데, "
                "원국에 이 글자들이 조합으로 있으면 형이 이루어지는 거야. "
                "삼형은 세 글자 중 두 글자만 있어도 부분적으로 작용하거든."
            ),
            "효과": (
                "과도한 행동이나 압박, 긴장이 생기는 신호야. "
                "寅巳申 삼형은 힘 있는 글자끼리 서로 치고박는 형태라 몸이나 행동에서 과잉이 나오기 쉽고, "
                "丑戌未 삼형은 고집과 자기중심성이 강해지는 쪽으로, "
                "子卯 형은 선 넘기나 경계 무시 같은 형태로 나타날 수 있어. "
                "형 자체가 나쁜 게 아니라, 그 자리에 어떤 십성이 있느냐랑 같이 봐야 해."
            ),
        },
        "파": {
            "성립": (
                "子酉, 丑辰, 寅亥, 卯午, 巳申, 未戌 — 이 여섯 쌍이 파야. "
                "삼합이 완성되려는 걸 방해하거나 엇박자로 기운이 새는 구조거든."
            ),
            "효과": (
                "잘 되다가 막판에 빈틈이 생기거나 계획이 어그러지는 신호야. "
                "관계에서도 협력이 잘 되나 싶다가 어느 순간 어긋나는 패턴이 나오기 쉬워. "
                "충이나 형보다는 강도가 약해서 보조 신호로만 참고하면 돼."
            ),
        },
        "해": {
            "성립": (
                "子未, 丑午, 寅巳, 卯辰, 申亥, 酉戌 — 이 여섯 쌍이 해야. "
                "육합으로 뭉치려는 관계를 옆에서 방해하는 새는 자리라고 보면 돼."
            ),
            "효과": (
                "충처럼 확 충돌하는 게 아니라 서서히 기운이 새고 관계가 조금씩 손상되는 느낌이야. "
                "시기나 질투, 은근한 방해처럼 겉으로 잘 안 보이는 마찰로 나타날 수 있어. "
                "재성이나 관성 자리에 해가 걸려 있으면 그 부분을 좀 더 신경 써서 봐야 해."
            ),
        },
    }
    ...
    for it in interactions:
        ...
        card = (
            ...
            f"<div style='font-size:13px;font-weight:900;color:{style['color']};opacity:.7;letter-spacing:.5px;margin-bottom:3px;'>어떻게 이루어지냐면</div>"
            f"<div style='font-size:13px;color:#9898b8;line-height:1.7;'>{detail['성립']}</div>"
            f"</div>"
            # 효과
            f"<div>"
            f"<div style='font-size:13px;font-weight:900;color:{style['color']};opacity:.7;letter-spacing:.5px;margin-bottom:3px;'>어떤 작용을 하냐면</div>"
            f"<div style='font-size:13px;color:#9898b8;line-height:1.7;'>{detail['효과']}</div>"
            f"</div>"
            f"</div>"
        )
...
def make_markdown_report(chart: Chart, result: Dict[str, object]) -> str:
    ...
    lines.append(f"- 원국 총점: **{result.get('score_display', result['total'])} / 100**")
    lines.append(f"- 기본 총점: **{bd['기본 총점']} / 100**")
...
def meal_wealth_score(participant: Dict[str, object], month_gz: str, day_gz: str) -> Dict[str, object]:
    ...
    if total >= 75:
        score += 2.0
        notes.append("원국 총점 상위권 보조 가점")
    elif total < 60:
        score -= 2.0
        notes.append("원국 총점 하위권 보조 감점")
...
def render_role_winner_hero(meal_result: Dict[str, object]) -> None:
    ...
    st.markdown(textwrap.dedent(f"""
    <div style="background:linear-gradient(135deg,#16161e 0%,#1a1408 100%);border:1px solid rgba(140,140,210,.45);border-radius:24px;padding:1.05rem 1.15rem;margin:.75rem 0 1rem 0;text-align:center;box-shadow:0 14px 34px rgba(99,102,241,.10);">
        <div style="display:inline-block;background:#16161e;border:1px solid rgba(99,102,241,.22);border-radius:999px;padding:.22rem .72rem;color:#a5b4fc;font-weight:950;font-size:.88rem;">{emoji} {safe_title}</div>
        <div style="margin-top:.55rem;color:#9898b8;font-weight:850;">오늘의 당첨자</div>
        <div style="font-size:2.2rem;line-height:1.12;font-weight:950;color:#818cf8;margin-top:.2rem;">{emoji} {safe_name}</div>
        <div style="color:#9898b8;font-weight:900;margin-top:.45rem;">{safe_score_short} {safe_score}점 · {safe_confidence}</div>
        <div style="color:#9898b8;font-weight:760;line-height:1.55;margin-top:.35rem;">{safe_summary}</div>
    </div>
    """).strip(), unsafe_allow_html=True)
...
def compatibility_analysis(my_payload: Dict[str, object], friend_payload: Dict[str, object], relationship: str = "친구") -> Dict[str, object]:
    ...
    rows.append({"항목": "원국 총점 차이", "내 값": my_result["total"], "상대 값": fr_result["total"], "판정": f"{total_gap:.1f}점 차이 — 케미 점수에는 직접 반영하지 않음"})
...
def render_rank_pyramid(title: str, rows: List[Dict[str, object]], caption: str = "") -> None:
    ...
    for idx, row in enumerate(rows[:6], start=1):
        ...
        st.markdown(f"""
        <div style="width:{width}%;margin:.28rem auto;padding:.55rem .75rem;border:1px solid rgba(140,140,210,.38);border-radius:16px;background:#16161e;box-shadow:0 8px 18px rgba(99,102,241,.08);text-align:center;min-width:220px;">
            <div style="font-weight:900;color:#9898b8;">{safe_rank}위 · {safe_name}</div>
            <div style="font-weight:900;color:#818cf8;font-size:1.05rem;">{score:.1f}점</div>
            <div style="font-size:.82rem;color:#9898b8;">{safe_sub}</div>
        </div>
        """, unsafe_allow_html=True)
    if caption:
        st.caption(caption)
    ...
    for col, (title, name, value, body) in zip(cols, cards):
        with col:
            ...
            <div class="mini-card">
                ...
                <div style="color:#818cf8;font-weight:900;margin-top:.35rem;">{html.escape(str(value), quote=True)}</div>
                <div style="color:#9898b8;font-size:.86rem;margin-top:.35rem;line-height:1.45;">{html.escape(body, quote=True)}</div>
            </div>
            """, unsafe_allow_html=True)
...
def build_multi_situation_svg(participants: List[Dict[str, object]], power_map: Dict[str, object]) -> str:
    ...
    parts.append(f'<text x="{cx}" y="{cy-10}" text-anchor="middle" font-size="22" font-weight="900" fill="#9898b8">다자간 국면</text>')
    parts.append(f'<text x="{cx}" y="{cy+18}" text-anchor="middle" font-size="13" font-weight="700" fill="#9898b8">오행처럼 서로 주고받는 흐름</text>')
    parts.append(f'<text x="{cx}" y="{cy+38}" text-anchor="middle" font-size="13" font-weight="700" fill="#9898b8">동맹 · 협력 · 조율 · 긴장</text>')
    ...
    for row in pair_rows:
        ...
        parts.append(
            f'<text x="{mx}" y="{my+4}" text-anchor="middle" font-size="11" font-weight="900" fill="#9898b8">{label}</text>'
        )
    ...
    for p in participants:
        ...
        parts.append(f'<text x="{x}" y="{y-2}" text-anchor="middle" font-size="16" font-weight="900" fill="#d0d0e8">{html.escape(name)}</text>')
        parts.append(f'<text x="{x}" y="{y+17}" text-anchor="middle" font-size="11.5" font-weight="800" fill="#9898b8">{html.escape(display_role)}</text>')
...
def score_breakdown(result: Dict[str, object]) -> Dict[str, float]:
    ...
    return {"기본 총점": base, "카테고리 합계": category_sum, "전체 인상 보정": adjustment, "신살·공망": shinsal_adj, "운용성 보정": operability_adj, "최종 점수": final}
...
def render_score_breakdown_card(result: Dict[str, object]) -> None:
    ...
    <div class="mini-card">
        ...
        <div style="color:rgba(50,50,80,0.74); margin-top:0.55rem; line-height:1.55;">
...
def render_ability_structure_map() -> None:
    ...
    <div class="ability-map">
        ...
        <div class="ability-card">
            <div class="ability-card-title">흐름과 연결</div>
            <div class="ability-card-body">
...
def audit_summary_rows() -> List[Dict[str, str]]:
    return [
        ...
        {"항목": "총점 계산 구조", "판정": "대체로 양호", "보완": "기본 총점 + 전체 인상 보정 = 최종 점수 명시"},
...
thead th {
    ...
    border: 1px solid rgba(120,120,220,0.30) !important;
}
tbody td {
    color: #f0e0ea !important;
    font-size: 0.9rem !important;
    ...
    border: 1px solid rgba(120,120,220,0.18) !important;
    line-height: 1.75 !important;
}
tbody tr:nth-child(even) td {
    background: rgba(42,21,32,0.6) !important;
}
...
.daewun-flow-card {
    border: 2px solid rgba(90,40,64,0.80) !important;
...
@media (max-width: 768px) {
    ...
    [data-testid="stCaptionContainer"] p { font-size: 0.82rem !important; }
...
<script>
(function(){
    var css=`.svg-radar-card,.chem-radar-card{background:#16161e;border:1px solid rgba(90,40,64,.60);border-radius:20px;padding:.75rem;box-shadow:none;}
.element-bar-row{display:grid;grid-template-columns:34px 1fr 54px;align-items:center;gap:.45rem;margin:.34rem 0;color:#9898b8;font-size:.88rem;}
.element-bar-track,.chem-track{height:10px;background:#2e1422;border-radius:999px;overflow:hidden;}
.element-bar-fill{height:100%;background:#3730a3;border-radius:999px;}
.chem-element-row{display:grid;grid-template-columns:34px 1fr;gap:.5rem;align-items:center;margin:.55rem 0;}
.chem-element-label{font-weight:950;color:#c4c4e8;text-align:center;}
.chem-line{display:grid;grid-template-columns:56px 1fr 54px;gap:.42rem;align-items:center;margin:.22rem 0;font-size:.82rem;color:#c090a8;}
.chem-fill{height:100%;border-radius:999px;}
.chem-fill.mine{background:#3730a3;}
.chem-fill.friend{background:#6b8cff;}
.daewun-flow-track{display:flex;gap:.55rem;overflow-x:auto;padding:.35rem .1rem .55rem .1rem;}
.daewun-flow-card{min-width:104px;background:#16161e;border:2px solid #5a2840;border-radius:16px;padding:.75rem .65rem;text-align:center;box-shadow:none;}
.daewun-flow-card.current{background:#1e1e2c;border-color:#818cf8;}
.daewun-flow-badge{font-size:.86rem;color:#b888a0;font-weight:900;}
.daewun-flow-age{font-size:.82rem;color:#c090a8;}
.daewun-flow-gz{font-size:1.28rem;font-weight:950;color:#c4c4e8;}
.daewun-flow-score{font-size:.85rem;color:#818cf8;font-weight:900;}
.daewun-flow-icon{font-size:1.15rem;height:1.3rem;}
.radar-legend{display:flex;gap:1rem;justify-content:center;align-items:center;margin-top:.1rem;font-size:.88rem;color:#9898b8;font-weight:850;}
...
.chem-overlap-row{display:grid;grid-template-columns:34px 1fr 1fr;gap:.45rem;align-items:center;background:#1a1a26;border:1px solid rgba(140,140,210,.28);border-radius:12px;padding:.45rem .6rem;color:#9898b8;font-size:.86rem;}
.chem-overlap-row b{color:#818cf8;}
.chem-overlap-row em{font-style:normal;color:#c090a8;text-align:right;}
.luck-intensity-card{background:#16161e;border:1px solid rgba(140,140,210,.32);border-radius:16px;padding:.75rem .85rem;margin:.35rem 0 .45rem 0;}
.luck-intensity-label{color:#c090a8;font-size:.84rem;font-weight:850;}
.luck-intensity-value{font-size:1.8rem;line-height:1.15;color:#818cf8;font-weight:950;}
...
.flow-strength-label{display:flex;justify-content:space-between;gap:.5rem;font-size:.88rem;color:#c090a8;font-weight:900;margin-bottom:.2rem;}
.flow-strength-meter{height:9px;background:#222030;border-radius:999px;overflow:hidden;}
.flow-strength-meter span{display:block;height:100%;background:#64748b;border-radius:999px;}

.small-muted{font-size:.82rem;color:#c090a8;margin-top:.55rem;line-height:1.55;}
.pipeline-card{padding:1rem;margin:.75rem 0;border-radius:18px;background:#16161e;border:1px solid rgba(90,40,64,.50);box-shadow:none;}
.flow-pipeline{display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin:.7rem 0;}
.flow-node{display:inline-flex;flex-direction:column;align-items:center;justify-content:center;min-width:88px;padding:.55rem .7rem;border-radius:16px;background:#0d1828;border:1px solid rgba(100,150,220,0.35);color:#94a3b8;}
.flow-node b{font-size:.9rem;}
.flow-node em{font-style:normal;font-weight:900;color:#60a5fa;}
.pipe-pipeline{gap:.55rem;align-items:center;flex-wrap:wrap;}
.pipe-node{min-width:92px;background:#0d1828;border-color:#dbeafe;box-shadow:none;}
...
.shinsal-card{position:relative;overflow:hidden;border-radius:26px;padding:1.08rem 1.05rem;border:1px solid rgba(99,102,241,.17);background:linear-gradient(135deg,#16161e,#1a1a26);box-shadow:0 14px 30px rgba(40,40,80,.08);}
.shinsal-card::after{content:"";position:absolute;right:-34px;top:-34px;width:116px;height:116px;border-radius:999px;background:rgba(99,102,241,.08);}
.shinsal-head{display:flex;align-items:center;gap:.75rem;margin-bottom:.55rem;position:relative;z-index:1;}
.shinsal-icon{display:flex;align-items:center;justify-content:center;width:54px;height:54px;border-radius:18px;background:#16161e;box-shadow:0 8px 18px rgba(40,40,80,.08);font-size:2rem;line-height:1;flex:0 0 54px;}
.shinsal-name{font-size:1.22rem;font-weight:950;color:#c4c4e8;letter-spacing:-.04em;line-height:1.1;}
.shinsal-type{font-size:.88rem;color:#9898b8;font-weight:900;margin-top:.18rem;}
.shinsal-badge{display:inline-block;margin:.1rem 0 .45rem 0;padding:.22rem .62rem;border-radius:999px;background:rgba(99,102,241,.10);color:#818cf8;font-weight:950;font-size:.8rem;}
.shinsal-summary{font-size:.94rem;color:#c4c4e8;line-height:1.55;font-weight:780;margin:.35rem 0 .62rem;}
.shinsal-detail{background:rgba(30,16,24,.92);border:1px solid rgba(140,140,210,.22);border-radius:16px;padding:.68rem .75rem;margin:.45rem 0;position:relative;z-index:1;}
.shinsal-detail-title{font-size:.88rem;color:#818cf8;font-weight:950;margin-bottom:.22rem;}
.shinsal-detail-body{font-size:.84rem;color:#9898b8;line-height:1.52;font-weight:650;}
.shinsal-pos{font-size:.88rem;color:#9898b8;margin-top:.5rem;font-weight:800;}
.shinsal-footnote{font-size:.88rem;color:#9898b8;line-height:1.45;margin-top:.6rem;}
...
@media (max-width: 768px) {
    ...
    .stButton > button[kind="primary"] {
        background: linear-gradient(90deg, #b91c5c, #3730a3) !important;
...
label[data-baseweb="checkbox"] * {
    color: #d0d0e8 !important;
    -webkit-text-fill-color: #d0d0e8 !important;
    opacity: 1 !important;
...
.manse-wrap {
    background: linear-gradient(135deg,#16161e 0%,#1a1408 48%,#0d1828 100%);
    border: 1px solid rgba(140,140,210,.36);
    border-radius: 24px;
    padding: 1rem .9rem;
    box-shadow: 0 14px 30px rgba(99,102,241,.08);
...
.manse-col {
    background:#16161e;
    border:1px solid rgba(140,140,210,.32);
    border-radius:18px;
    overflow:hidden;
    box-shadow:0 8px 18px rgba(40,40,80,.05);
}
.manse-col.manse-day {
    border:2px solid rgba(99,102,241,.50);
...
.manse-head {
    ...
    font-weight:950;
    font-size:.88rem;
    border-bottom:1px solid rgba(140,140,210,.32);
}
.manse-role {
    text-align:center;
    color:#c090a8 !important;
...
.manse-big {
    ...
    line-height:1;
    font-weight:950;
    border:2px solid rgba(40,40,70,.18);
    color:#9898b8 !important;
    box-shadow:inset 0 0 0 1px rgba(255,255,255,.65), 0 6px 14px rgba(40,40,80,.08);
}
.manse-big.stem { margin-top:.1rem; }
.manse-big.branch { margin-bottom:.25rem; }
.manse-el-목 { background:linear-gradient(135deg,#0d1a10,#1a2a18); }
.manse-el-화 { background:linear-gradient(135deg,#1a0808,#2a1010); }
.manse-el-토 { background:linear-gradient(135deg,#1a1408,#2a2010); }
.manse-el-금 { background:linear-gradient(135deg,#1a2030,#1e2438); }
.manse-el-수 { background:linear-gradient(135deg,#08101a,#101828); }
.manse-ko {
    text-align:center;
    color:#9898b8 !important;
    font-size:.84rem;
...
.manse-hidden {
    border-top:1px dashed rgba(140,140,210,.45);
...
.manse-summary-card {
    background:#16161e;
    border:1px solid rgba(140,140,210,.3);
    border-radius:16px;
    padding:.65rem .7rem;
}
.manse-summary-title {
...
.manse-luck-wrap {
    ...
    padding:.9rem .85rem;
    border-radius:22px;
    background:linear-gradient(135deg,#16161e 0%,#1a1408 56%,#0d1828 100%);
    border:1px solid rgba(140,140,210,.32);
    box-shadow:0 12px 26px rgba(40,40,80,.055);
}
.manse-luck-head {
    display:flex;
    justify-content:space-between;
...
.manse-luck-chip {
    ...
    border-radius:999px;
    background:#16161e;
    border:1px solid rgba(140,140,210,.34);
    color:#818cf8 !important;
    font-size:.84rem;
    font-weight:950;
    white-space:nowrap;
...
.manse-luck-card {
    ...
    overflow:hidden;
    box-shadow:0 8px 18px rgba(40,40,80,.045);
}
.manse-luck-card.current {
    border:2px solid rgba(99,102,241,.52);
    box-shadow:0 11px 24px rgba(99,102,241,.115);
}
.manse-luck-card.good { background:linear-gradient(135deg,#0d1e14,#112018); }
.manse-luck-card.caution { background:linear-gradient(135deg,#16161e,#1a1408); }
...
.manse-luck-el {
    ...
    padding:.15rem .36rem;
    border-radius:999px;
    border:1px solid rgba(40,40,70,.12);
    color:#d0d0e8 !important;
    font-size:.82rem;
    font-weight:880;
}
.manse-luck-el.el-목 { background:#0d1e14; }
.manse-luck-el.el-화 { background:#16161e; }
.manse-luck-el.el-토 { background:#1a1408; }
.manse-luck-el.el-금 { background:#0d1828; }
.manse-luck-el.el-수 { background:#0d1828; }
.manse-luck-score {
    color:#818cf8 !important;
    font-size:.82rem;
    font-weight:950;
...
.manse-luck-mini {
    ...
    right:.54rem;
    bottom:.48rem;
    color:rgba(99,102,241,.18) !important;
    font-size:2.1rem;
    font-weight:950;
    line-height:1;
}
...
@media (max-width: 768px) {
    ...
    .manse-head { font-size:.88rem; padding:.36rem .12rem; }
    .manse-role { font-size:.82rem; min-height:1.35rem; padding:.25rem .08rem .1rem .08rem; }
    .manse-big {
        width:42px;
        height:42px;
        border-radius:10px;
...
.saju-landscape-shell {
    ...
    background: linear-gradient(135deg,#0d1828 0%,#16161e 56%,#1a1408 100%);
...
.saju-landscape-panel {
    background:#16161e;
    border:1px solid rgba(90,40,64,.50);
    border-radius:24px;
    padding:.86rem .9rem;
}
...
.saju-landscape-mini-card {
    padding:.68rem .72rem;
    border-radius:18px;
    background:linear-gradient(135deg,#16161e,#0d1828);
...
.saju-landscape-one {
    ...
    padding:.62rem .72rem;
    border-radius:18px;
    background:linear-gradient(135deg,#16161e,#1a1a26);
    color:#d0d0e8 !important;
    font-size:.9rem;
    line-height:1.45;
    font-weight:900;
...
.saju-landscape-role {
    ...
    padding:.68rem .74rem;
    border-radius:18px;
    background:linear-gradient(135deg,#0d1828,#16161e);
    border:1px solid rgba(70,92,120,.12);
    color:#d0d0e8 !important;
    font-size:.84rem;
    line-height:1.48;
...
.share-card-wrap {
    ...
    box-shadow: 0 16px 34px rgba(99,102,241,.10);
}
.share-card-kicker {
    display:flex;
    justify-content:space-between;
...
.share-card-box {
    background:#16161e;
    border:1px solid rgba(90,40,64,.60);
    border-radius:18px;
    padding:.72rem .78rem;
}
.share-card-label {
...
.element-tone-card {
    ...
    text-align:center;
    border:1px solid rgba(140,140,210,.20);
...
.today-compass {
    ...
    background: linear-gradient(135deg,#0d1e14 0%,#14141e 54%,#16161e 100%);
...
.today-interaction-kind.caution { background: #3730a3; }
...
.operation-meter {
    ...
    box-shadow: inset 0 0 0 1px rgba(255,255,255,.62), 0 7px 18px rgba(40,40,80,.08);
}
.operation-marker {
    position:absolute;
    top:-21px;
...
.operation-marker-badge {
    ...
    font-weight:950;
    white-space:nowrap;
    box-shadow:0 7px 16px rgba(40,40,70,.18);
}
...
div[data-testid="stRadio"] div[role="radiogroup"] label {
    ...
    border:1.5px solid rgba(99,102,241,.24) !important;
    border-radius:999px !important;
    padding:.58rem .95rem !important;
    min-height:48px !important;
    box-shadow:0 8px 18px rgba(99,102,241,.045);
}
div[data-testid="stRadio"] div[role="radiogroup"] label:has(input:checked),
div[data-testid="stRadio"] div[role="radiogroup"] label[aria-checked="true"] {
    background:linear-gradient(135deg,#1e1e2c,#16161e) !important;
...
@media (max-width:768px){
    ...
    .operation-marker-badge{font-size:.80rem;}
}
...
.stApp { overflow-x: hidden !important; background-color: #0f0f14 !important; }
...
div[data-baseweb="textarea"] {
    background-color: #1e1e2c !important;
    border-color: rgba(120,120,220,0.30) !important;
}
...
[data-baseweb="radio-group"] label {
    background-color: #1e1e2c !important;
    border: 1.5px solid rgba(120,120,220,0.40) !important;
    border-radius: 999px !important;
...
div[data-testid="stCheckbox"] label {
    background-color: #1e1e2c !important;
    border: 1.5px solid rgba(120,120,220,0.35) !important;
...
div[data-baseweb="checkbox"] svg { color: #818cf8 !important; }
...
div[data-baseweb="popover"] { background-color: #1e1e2c !important; }
div[data-baseweb="menu"] { background-color: #1e1e2c !important; }
div[data-baseweb="option"] { background-color: #1e1e2c !important; color: #d0d0e8 !important; }
div[data-baseweb="option"]:hover { background-color: #3a1e2c !important; }

/* 라디오 baseweb 강제 다크 */
[data-baseweb="radio"] + div,
label[data-baseweb="radio"],
[role="radio"] { background: transparent !important; }
div[data-testid="stRadio"] label > div:first-child {
    background-color: transparent !important;
    border-color: #818cf8 !important;
}
/* Streamlit 2024 라디오 pill 컨테이너 */
div[data-testid="stRadio"] > div > label {
    background-color: #1e1e2c !important;
    border: 1px solid rgba(120,120,220,0.40) !important;
    color: #d0d0e8 !important;
}
...
[data-testid="stTextInput"] > div > div:focus-within {
    border-color: #818cf8 !important;
    box-shadow: 0 0 0 2px rgba(120,120,220,0.20) !important;
}
...
[data-testid="stTimeInput"] input { background-color: #1e1e2c !important; color: #d0d0e8 !important; }
...
[data-testid="stRadio"] > label,
[data-testid="stCheckbox"] > label,
[data-testid="stSelectbox"] > label { color: #9898b8 !important; font-weight: 600 !important; }

/* 전역 카드/박스 다크 오버라이드 */
[data-testid="stMetric"],
[data-testid="metric-container"] { background-color: #1e1e2c !important; border-radius: 12px !important; }
[data-testid="stExpander"] > div { background-color: #221018 !important; }
[data-testid="stForm"] { background-color: #16161e !important; border: 1px solid rgba(120,120,220,0.20) !important; }
div.stAlert > div { border-radius: 10px !important; }
[data-testid="stAlert"][data-baseweb="notification"] { background-color: #16161e !important; }
/* 탭 */
[data-testid="stTabs"] [role="tablist"] { background-color: #1e1e2c !important; border-radius: 10px !important; }
[data-testid="stTabs"] [role="tab"] { color: #c090a8 !important; }
[data-testid="stTabs"] [role="tab"][aria-selected="true"] { color: #818cf8 !important; background-color: #3a1e2c !important; }
/* 구분선 */
hr { border-color: rgba(120,120,220,0.20) !important; }
...
[data-testid="stRadio"] { color: #d0d0e8 !important; }
/* expander */
[data-testid="stExpander"] { background-color: #221018 !important; border: 1px solid rgba(120,120,220,0.20) !important; }
[data-testid="stExpander"] summary { color: #9898b8 !important; }
/* info / success / warning 박스 */
[data-testid="stAlert"] { background-color: #1e1e2c !important; color: #d0d0e8 !important; }
...
[data-testid="stText"] { color: #d0d0e8 !important; }
/* 헤딩 */
h1, h2, h3, h4 { color: #c4c4e8 !important; }
[data-testid="stMarkdownContainer"] h1,
[data-testid="stMarkdownContainer"] h2,
[data-testid="stMarkdownContainer"] h3,
[data-testid="stMarkdownContainer"] h4 { color: #c4c4e8 !important; }
/* 히어로 타이틀 명시 */
.hero-title { color: #c4c4e8 !important; }
.hero-title span { -webkit-text-fill-color: unset !important; color: #c4c4e8 !important;
    background: linear-gradient(92deg, #818cf8 0%, #818cf8 50%, #c084fc 90%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
.hero-subtitle { color: #9898b8 !important; }
...
[data-testid="stAppViewContainer"] { background-color: #0f0f14 !important; }
[data-testid="stHeader"] { background-color: #0f0f14 !important; }
section.main { background-color: #0f0f14 !important; }
.block-container { background-color: #0f0f14 !important; }
[data-testid="stSidebar"] { background-color: #1e1e2c !important; }
body { background-color: #0f0f14 !important; color: #d0d0e8 !important; }
...
/* 버튼형 내부 페이지 */
.page-button-guide{font-size:.92rem;color:#c090a8;margin:.4rem 0 .55rem 0;}
...
/* 재방문 안내 카드 */
.revisit-guide-card{background:#1a1408;border:1px solid rgba(120,120,220,0.25);border-radius:18px;padding:14px 16px;color:#d0d0e8;margin-bottom:10px;}
.revisit-guide-title{font-weight:800;color:#f890b8;margin-bottom:8px;font-size:1rem;}
...
@media (max-width: 768px) {
    ...
    .manse-year-card {
        ...
        box-shadow: 0 8px 18px rgba(40,40,80,.06) !important;
    }
    ...
    .today-quick-pill { align-self:flex-start !important; font-size:.86rem !important; padding:.24rem .52rem !important; }
    ...
    .element-bar-row { grid-template-columns: 28px 1fr 42px !important; gap: .32rem !important; font-size:.88rem !important; }
...
@media (max-width: 768px) {
    ...
    .hero-wrap {
        ...
        box-shadow: 0 10px 24px rgba(40,40,80,.07) !important;
    }
    ...
    .revisit-guide-card {
        ...
        box-shadow: 0 8px 18px rgba(40,40,80,.055) !important;
    }
    ...
    .stLinkButton > a {
        ...
        font-weight: 900 !important;
        letter-spacing: -.01em !important;
        box-shadow: 0 8px 16px rgba(99,102,241,.10) !important;
    }

    .stButton > button {
        border: 1px solid rgba(99,102,241,.18) !important;
        background: linear-gradient(180deg,#16161e,#1a1a26) !important;
    }

    .stButton > button[kind="primary"] {
        background: linear-gradient(180deg,#3730a3,#bc275c) !important;
        border-color: rgba(188,39,92,.92) !important;
        box-shadow: 0 10px 18px rgba(99,102,241,.18) !important;
    ...
    [data-testid="stExpander"] {
        ...
        border: 1px solid rgba(140,140,210,.22) !important;
        background: #16161e !important;
    }
...
@media (max-width: 768px) {
    ...
    .chem-hero-premium {
        ...
        box-shadow: 0 10px 24px rgba(40,40,80,.075) !important;
    }
...
.revisit-guide-compact {
    margin-top: .72rem !important;
    background: linear-gradient(135deg,#16161e,#1a1a26) !important;
    border: 1px solid rgba(140,140,210,.34) !important;
}
.revisit-mini-steps {
    display: flex;
    gap: .34rem;
...
.single-axis-triangle-wrap {
    ...
    border: 1px solid rgba(140,140,210,.30);
    border-radius: 20px;
...
.axis-bar-track {
    ...
    border-radius:999px;
    background:#16161e;
    border:1px solid rgba(140,140,210,.22);
    overflow:hidden;
}
.axis-bar-fill {
    height:100%;
...
.single-axis-triangle-wrap {
    ...
    border: 1px solid rgba(140,140,210,.28);
    border-radius: 18px;
...
def render_my_saju_browser_storage_widget(save_url: str = "", key: str = "my_saju_browser_store") -> None:
    ...
    component_html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; border:1px solid rgba(120,120,220,0.30); background:#16161e; border-radius:16px; padding:12px; color:#d0d0e8;">
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        <button id="saveBtn" style="display:none; border:1px solid #3730a3; background:#3730a3; color:white; border-radius:999px; padding:8px 12px; font-weight:800; cursor:pointer;">이 기기에 저장</button>
        <a id="loadLink" target="_parent" style="display:none; text-decoration:none; border:1px solid #818cf8; background:#1e1e2c; color:#818cf8; border-radius:999px; padding:8px 12px; font-weight:800;">저장된 내 사주 불러오기</a>
        <button id="clearBtn" style="display:none; border:1px solid rgba(120,120,220,0.30); background:#1e1e2c; color:#c090a8; border-radius:999px; padding:8px 12px; font-weight:800; cursor:pointer;">저장 삭제</button>
      </div>
      <div id="msg" style="margin-top:8px; font-size:13px; color:#9898b8;"></div>
    </div>
    <script>
      (function() {{
...
def render_admin_analytics_dashboard() -> None:
    ...
    st.markdown(
        "<div class='signal-card' style='border-color:rgba(16,185,129,.28);background:linear-gradient(135deg,#0d1e14,#16161e);'>"
...
.hero-wrap {
    ...
    border-radius: 28px;
    background:
        radial-gradient(circle at 15% 15%, rgba(120,120,220,0.12), transparent 40%),
        linear-gradient(135deg, rgba(42,21,32,0.98), rgba(30,13,20,1.0));
    border: 1px solid rgba(120,120,220,0.28);
    box-shadow: 0 16px 38px rgba(120,120,220,0.10);
    margin-bottom: 1rem;
    text-align: center;
}
.hero-title {
...
.hero-title span {
    background: linear-gradient(92deg, #6366f1 0%, #818cf8 48%, #a78bfa 90%);
...
.glass-panel, .result-arena, .detail-wrap {
    ...
    border: 1px solid rgba(100,100,220,0.24);
...
label, .stRadio label, .stCheckbox label {
    color: rgba(50,50,80,0.92) !important;
    font-weight: 760 !important;
}
[data-testid="stCaptionContainer"] { color: rgba(40,40,70,0.82) !important; }
...
.stButton > button[kind="primary"] {
    background: linear-gradient(90deg, #b91c5c, #3730a3);
...
.mini-card-title { color: rgba(50,50,80,0.68); font-size:0.9rem; font-weight:800; }
...
.score-label { color: rgba(40,40,70,0.82); font-weight: 750; margin-top: -0.45rem; }
.stAlert { border-radius: 16px; }
button[data-baseweb="tab"] { color: rgba(50,50,80,0.84); font-weight: 800; }
hr { border-color: rgba(100,100,220,0.22); }
...
:root {
    --palja-readable-text: #d0d0e8;
    --palja-readable-muted: #c090a8;
    --palja-readable-title: #818cf8;
}
...
.step-panel {
    ...
    border-radius: 26px;
    background: linear-gradient(135deg, #16161e, #1a1a26);
    border: 1px solid rgba(100,100,220,0.26);
...
.compact-kpi {
    ...
    border-radius: 20px;
    background: #16161e;
    border: 1px solid rgba(100,100,220,0.20);
    min-height: 112px;
}
...
.ability-card {
    background: #16161e;
    border: 1px solid rgba(100,100,220,0.22);
    border-radius: 20px;
    padding: 1rem 1rem;
    min-height: 150px;
}
...
.support-menu-wrap {
    margin-top: 1.15rem;
    padding-top: 0.75rem;
    border-top: 1px dashed rgba(100,100,220,0.35);
}
.support-menu-title {
    color: #c090a8 !important;
...
div[role="radiogroup"] label {
    background: #16161e !important;
    border: 1px solid rgba(100,100,220,0.22) !important;
    border-radius: 999px !important;
...
div[role="radiogroup"] label {
    background: #16161e !important;
    border: 1px solid rgba(100,100,220,0.28) !important;
    border-radius: 999px !important;
...
div[data-testid="stCheckbox"] label {
    background: #16161e !important;
    border: 1px solid rgba(100,100,220,0.24) !important;
    border-radius: 16px !important;
...
.main-character-card {
    background: linear-gradient(135deg, #16161e 0%, #1a1020 100%);
...
.ability-bar-track {
    ...
    background: #2e1422;
    overflow: hidden;
    box-shadow: inset 0 1px 2px rgba(50,50,80,0.08);
}
.ability-bar-fill {
    height: 100%;
    border-radius: 999px;
    background: linear-gradient(90deg, #818cf8, #3730a3);
}
.battle-board {
    background: linear-gradient(135deg, #fff7fb 0%, #fffdf7 100%);
    border: 1px solid rgba(100,100,220,0.32);
    border-radius: 24px;
    padding: 1.15rem;
    margin: 0.9rem 0 1rem 0;
}
...
.multi-situation-wrap {
    background: linear-gradient(135deg, #16161e 0%, #1a1020 100%);
    border: 1px solid rgba(100,100,220,0.28);
    border-radius: 22px;
...
.legend-pill {
    ...
    font-weight: 900;
    background: #16161e;
    border: 1px solid rgba(140,140,210,0.36);
    color: #9898b8;
}
...
.multi-role-card {
    background: #16161e;
    border: 1px solid rgba(140,140,210,0.34);
    border-radius: 18px;
...
.meal-stage-wrap {
    background: linear-gradient(135deg, #16161e 0%, #1a1408 52%, #221018 100%);
    border: 1px solid rgba(140,140,210,0.42);
    border-radius: 26px;
    padding: 1.15rem 1.15rem;
    margin: 0.9rem 0 1rem 0;
    color: #d0d0e8;
...
.meal-ready-card {
    background: #16161e;
    border: 1px solid rgba(140,140,210,0.38);
    border-radius: 18px;
...
.pick-sealed-card {
    background: linear-gradient(135deg,#16161e 0%,#1a1a26 100%);
...
.safe-table { width:100%; border-collapse:separate; border-spacing:0; font-size:.88rem; background:#16161e; border:1px solid rgba(140,140,210,.35); border-radius:14px; overflow:hidden; }
.safe-table th { text-align:left; background:#16161e; color:#9898b8; padding:.62rem .7rem; border-bottom:1px solid rgba(140,140,210,.35); white-space:nowrap; }
.safe-table td { padding:.58rem .7rem; border-bottom:1px solid rgba(140,140,210,.22); color:#5f4550; vertical-align:top; }
.safe-table tr:last-child td { border-bottom:0; }
.viz-pill { display:inline-flex; align-items:center; gap:.35rem; background:#16161e; border:1px solid rgba(140,140,210,.35); color:#6a4554; border-radius:999px; padding:.32rem .6rem; font-size:.82rem; font-weight:850; margin:.15rem .22rem .15rem 0; }
.signal-card { background:#16161e; border:1px solid rgba(140,140,210,.30); border-radius:18px; padding:.85rem .9rem; margin:.45rem 0; box-shadow:0 8px 18px rgba(100,100,220,.07); }
.signal-title { color:#9898b8; font-weight:950; margin-bottom:.25rem; }
.signal-body { color:#c090a8; line-height:1.55; font-size:.92rem; }
.thermo-track { height:22px; background:#1e1e26; border-radius:999px; overflow:hidden; border:1px solid rgba(110,82,94,.18); position:relative; }
.thermo-fill { height:100%; border-radius:999px; }
.thermo-pin { position:absolute; top:50%; width:30px; height:30px; transform:translate(-50%,-50%); border-radius:999px; background:#16161e; border:3px solid #4b2d3a; box-shadow:0 4px 12px rgba(0,0,0,.16); }
...
/* h3·h4 헤더 */
.stMarkdown h3 { color: #c4c4e8 !important; font-size: 1.12rem !important; }
.stMarkdown h4 { color: #818cf8 !important; font-size: 1.04rem !important; }
/* expander 헤더 */
[data-testid="stExpander"] summary p {
    color: #d0d0e8 !important;
    font-weight: 800 !important;
...
th, td {
    color: #d0d0e8 !important;
    border: 1px solid rgba(120,120,220,0.20) !important;
...
def _climate_character_axis(chart: Chart, result: Dict[str, object]) -> Dict[str, str]:
    ...
    if balance_gap <= -12:
        ...
        if caution_notes:
            note += (
                " 그런데 여기서 한 가지 더 알아야 해 — 巳(사)·酉(유)·丑(축) 이 세 글자가 원국 지지에 다 있으면"
                " 사유축 금국(金局)이라는 구조가 성립해. 금국이 뭐냐면, 이 세 글자가 서로 뭉쳐서"
                " 금(金) 기운을 한꺼번에 키우는 합국 구조야. 근데 여기서 중요한 게 있어 —"
                " 巳(사)는 원래 불(火)의 기운을 품고 있는 글자거든. 여름 초입의 뜨겁고 건조한 기운이야."
                " 그런데 금국이 성립하면 巳가 품고 있던 불의 온기가 금 쪽으로 끌려가서 약해지는 거야."
                " 결국 따뜻함을 줬어야 할 불 기운이 줄어드니까, 전체 사주 온도가 생각보다 더 서늘하게 나오는 거지."
            )
        return {
            "adj": "차분하게 구조화하는",
            "tone": note,
...
def _single_front_snapshot(payload: Dict[str, object]) -> Dict[str, object]:
    ...
    display_total = 0.0
    display_max = 0.0
    # v5.150: category_totals_synced_to_total 사용 → 3대 능력치 합계 = 총점 일치
    _cat_rows = (
...
def _triangle_axis_svg(score_map: Dict[str, Dict[str, float]]) -> str:
    ...
    for idx, ratio in enumerate((0.22, 0.44, 0.66, 0.88, 1.0), start=1):
        ...
        grid.append(f"<polygon points='{' '.join(pts)}' fill='none' stroke='rgba(99,102,241,{opacity})' stroke-width='{width}'/>")
    ...
    poly_shadow = f"<polygon points='{' '.join(points)}' fill='rgba(99,102,241,.10)' stroke='none' transform='translate(0,2)'/>"
    poly = f"<polygon points='{' '.join(points)}' fill='rgba(99,102,241,.22)' stroke='#3730a3' stroke-width='4.0'/>"
    dots = "".join([f"<circle cx='{p.split(',')[0]}' cy='{p.split(',')[1]}' r='6' fill='#3730a3' stroke='#fff' stroke-width='1.8'/>" for p in points])
    center_glow = f"<circle cx='{cx}' cy='{cy}' r='16' fill='rgba(99,102,241,.07)'/>"

    return (
        "<div style='background:rgba(255,255,255,.96);border:1px solid rgba(140,140,210,.30);border-radius:18px;"
...
def render_strength_operation_spotlight(payload: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div class="main-character-card" style="border-color:rgba(16,185,129,.35);background:linear-gradient(135deg,#0d1e14,#16161e);margin:.35rem 0 1rem;">
...
def _radar_svg_single(values: List[float], labels: List[str], stroke: str = "#3730a3", fill: str = "rgba(99,102,241,.20)") -> str:
    ...
    for val, label, a in zip(values, labels, angles):
        ...
        if lx < cx - 10:
            ...
            anchor = 'start'
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='800' fill='#c090a8'>{html.escape(label)}</text>")
...
def _radar_svg_dual(vals_a: List[float], vals_b: List[float], labels: List[str], name_a: str, name_b: str, max_val: float = 60.0) -> str:
    ...
    for a, label in zip(angles, labels):
        ...
        if lx < cx - 10:
            ...
            anchor = 'start'
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='850' fill='#c090a8'>{html.escape(label)}</text>")
    ...
    for pnt in pts_a:
        x, y = pnt.split(',')
        dots += f"<circle cx='{x}' cy='{y}' r='3.8' fill='#3730a3'/>"
    for pnt in pts_b:
        x, y = pnt.split(',')
        dots += f"<circle cx='{x}' cy='{y}' r='3.8' fill='#3b82f6'/>"
    legend = (
        f"<div class='radar-legend'><span><i style='background:#3730a3'></i>{html.escape(name_a)}</span>"
        f"<span><i style='background:#3b82f6'></i>{html.escape(name_b)}</span></div>"
    )
    svg = (
        "<svg viewBox='0 0 300 300' width='100%' height='310' role='img'>"
        + ''.join(grid + axes)
        + f"<polygon points='{' '.join(pts_a)}' fill='rgba(99,102,241,.24)' stroke='#3730a3' stroke-width='3'/>"
...
def render_climate_thermometer(result: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div style="background:#16161e;border:1px solid rgba(140,140,210,.30);border-radius:18px;padding:.85rem .9rem;margin:.3rem 0 .45rem 0;">
        <div style="display:flex;justify-content:space-between;color:#9898b8;font-size:.82rem;font-weight:850;margin-bottom:.45rem;">
        ...
        <div style="margin-top:.58rem;color:#9898b8;font-weight:900;">{html.escape(label)} · 온도감 {temp:.0f}/100</div>
        <div style="margin-top:.24rem;color:#9898b8;font-size:.88rem;">화 {fire:.1f}% / 수 {water:.1f}% · {html.escape(note)}</div>
    </div>
    """, unsafe_allow_html=True)
...
def _render_flow_pipeline_card(chart: Chart, result: Dict[str, object], row: Dict[str, object]) -> None:
    ...
    _source_block = (
        "<div style='background:#14141c;border-left:3px solid rgba(244,114,182,.35);"
...
def _element_tone_cards_html(pct: Dict[str, float]) -> str:
    cards = []
    for el in ELEMENTS:
        meta = ELEMENT_VISUALS.get(el, {"icon": "•", "bg": "#ffffff", "fg": "#2e2e42"})
        cards.append(
...
def render_today_quick_entry(payload: Dict[str, object]) -> None:
    ...
    # ── 진단 풀이 텍스트 ─────────────────────────────────
    _day_gz   = str(compass.get("day_gz", "-"))
    _day_title = str(compass.get("day_luck_title", _day_gz))
    _inter    = str(compass.get("interaction_overview") or "")
    _ten_god  = str(compass.get("day_tengod_summary") or "")
    _presc    = str(compass.get("ten_god_action") or compass.get("prescription") or "")

    # 십성 → 오늘 성격 한 줄
    _tengod_story = {
        "비견": "오늘 진단은 비견 기운이야. 나랑 비슷한 처지, 비슷한 힘을 가진 사람이 내 옆에 있거든 — 경쟁이 될 수도 있고 든든한 동료가 될 수도 있어. 오늘은 이기느냐 지느냐보다 같이 가느냐를 먼저 생각해봐. 혼자 다 하려고 힘 빼지 말고, 손 잡을 사람 하나 찾아봐. 그 쪽이 훨씬 빨리 가.",
        "겁재": "오늘은 겁재 기운이라 조심해야 해. 경쟁 신호, 분쟁 신호가 슬쩍 올라와 있거든. 내 걸 지키려다 오히려 더 잃는 날이야 — 억지로 붙잡지 마. 돈 문제, 다툼 문제가 생길 것 같으면 오늘만큼은 한 발 빼는 게 상책이야. 양보가 쪽팔린 게 아니라, 오늘은 그게 제일 현명한 거거든.",
        "식신": "오늘은 식신 기운 — 몸이 원하는 대로 하면 돼. 먹고 싶은 거 먹고, 하고 싶은 거 하고, 만들고 싶은 거 만들어봐. 이 기운은 긴장하면 죽어버리거든. 억지로 생산적일 필요 없어. 오늘 쉬는 게 내일의 힘이야. 느긋하게 보내는 날이 충전하는 날이야.",
        "상관": "오늘은 상관 기운이야. 말이 청산유수처럼 나오고, 아이디어가 팡팡 터지는 날이거든. 창의적인 일, 표현하는 일, 설득하는 일에 딱 맞아. 근데 이 기운이 강하면 말실수도 같이 와. 하고 싶은 말 다 뱉기 전에 한 박자만 쉬어봐 — 특히 윗사람 앞에서는 더 조심해. 머리는 활짝 열고, 입은 살짝 조여놓는 날이야.",
        "편재": "오늘은 편재 기운이야. 돈이 움직이고 사람이 움직이는 날이거든. 기회가 반짝하고 눈앞에 나타날 수 있어 — 보이면 잡아봐. 근데 이 기운은 너무 욕심 부리면 손가락 사이로 모래처럼 빠져나가거든. 적당히 잡고 적당히 놔주는 게 오늘의 기술이야. 흘러가는 건 보내줘.",
        "정재": "오늘은 정재 기운 — 꼼꼼하고 안정적인 날이야. 계획해둔 걸 차근차근 실행하기에 딱 좋아. 서두르거나 즉흥적으로 움직이면 오히려 어긋나거든. 오늘은 정해진 대로, 원칙대로 가면 돼. 급하게 굴지 않아도 충분히 잘 된다는 걸 믿어봐.",
        "편관": "오늘은 편관 기운이야. 긴장감이 공중에 떠 있는 날이거든. 결단을 요구받거나 압박을 받을 수 있어. 이럴 때 질질 끌면 더 힘들어지거든 — 딱 끊어서 결정해. 두려움은 일단 옆에 내려놓고, 지금 할 수 있는 것 먼저 해. 편관은 도망갈수록 커지고, 맞부딪히면 오히려 작아지는 기운이야.",
        "정관": "오늘은 정관 기운 — 반듯하게 가는 날이야. 바르게 행동하고, 약속을 지키고, 책임을 다하면 나중에 인정받아. 눈앞의 이익보다 의리와 원칙이 더 중요한 날이거든. 오늘 잘 쌓아둔 신뢰가 나중에 든든한 자산이 돼. 지금 하는 행동 하나하나가 평판을 만들어.",
        "편인": "오늘은 편인 기운이야. 직관이 살아나고, 영감이 번뜩이는 날이거든. 논리보다 느낌이 더 잘 맞는 날이야. 혼자 공부하거나, 조용히 생각을 정리하거나, 새로운 분야를 탐색하기에 딱 좋아. 대신 오늘은 너무 많은 사람 만나거나 시끄러운 자리에 있으면 기운이 산만해지거든. 나만의 시간을 좀 확보해봐.",
        "정인": "오늘은 정인 기운 — 배우고 받아들이는 날이야. 누군가 조언을 해주면 귀담아 들어봐. 스승이 따로 없어도 오늘 만나는 사람, 읽는 글, 보는 것 하나하나가 다 가르침이 될 수 있거든. 내가 모른다는 걸 인정하는 게 오늘의 힘이야. 받아들이는 자세가 열려 있을수록 오늘 더 많은 게 채워져.",
    }
    _tg_key = ""
    for _tg in _tengod_story:
        if _tg in _ten_god:
            _tg_key = _tg
            break
    ...
    _signal_line = ""
    if "형" in _inter:
        _signal_line = " 오늘은 형살 신호가 잡혀. 억누르거나 억눌리는 기운이 껴 있는 날이거든. 속에서 뭔가 꽉 막히는 느낌이 들 수 있어. 감정을 안으로만 삭이지 말고, 가까운 사람한테라도 살짝 털어내봐. 말 못 할 상황이면 몸을 움직이는 것도 방법이야 — 산책, 운동, 청소 뭐든."
    elif "충" in _inter:
        _signal_line = " 오늘은 충 신호가 있어. 내 원국이랑 오늘 기운이 서로 부딪히는 날이거든. 막히거나 어긋나는 일이 생겨도 놀라지 마. 억지로 밀어붙이면 더 크게 부딪혀 — 오늘만큼은 유연하게 돌아가는 길을 찾아봐. 고집 부릴수록 손해야."
    elif "합" in _inter:
        _signal_line = " 오늘은 합 신호야. 내 원국이랑 오늘 기운이 잘 엮이는 날이거든. 사람이 먼저 다가오거나, 생각지 못한 연결이 생길 수 있어. 억지로 만들려 하지 않아도 자연스럽게 맞아 들어가는 흐름이니까 — 열린 마음으로 받아들여봐."

    # 처방 한 줄
    _rx_line = f" {_presc}" if _presc and _presc not in ("-", "없음") else ""

    # 오늘 흐름 지수 — 충·형·겁재(주의) / 합(좋음) / 기타(보통)
    _is_warn = "충" in _inter or "형" in _inter or "겁재" in _ten_god
    _is_good = "합" in _inter and not _is_warn
    if _is_warn:
        _flow_grade, _flow_color = "⚠️ 주의", "#dc2626"
    elif _is_good:
        _flow_grade, _flow_color = "✅ 좋음", "#16a34a"
    else:
    ...
    st.markdown(
        f"<div style='background:#0d1e14;border-left:4px solid #4ade80;padding:12px 16px;"
        f"border-radius:8px;margin-bottom:12px;font-size:14px;color:#14532d;line-height:1.8;'>"
        f"<div style='display:flex;justify-content:space-between;align-items:center;"
        f"margin-bottom:5px;'>"
        f"<span style='font-size:12px;color:#a0b4bc;'>오늘 처방전 · {_day_gz}</span>"
        f"<span style='font-size:12px;font-weight:700;color:{_flow_color};'>"
        f"오늘 흐름 {_flow_grade}</span>"
        f"</div>"
        f"{_story_text}</div>",
        unsafe_allow_html=True,
    )
...
def _svg_pill(x: int, y: int, label: str, fill: str = "#ffffff", stroke: str = "#e2e8f0") -> str:
    ...
    return (
        ...
        f"<text x='{x + 13}' y='{y + 19}' font-size='12' font-weight='900' fill='#94a3b8'>{safe}</text></g>"
    )
...
def saju_landscape_svg(model: Dict[str, object], compact: bool = False) -> str:
    ...
    if dtype == "winter_silence":
        fx = int(W*0.33)
        fy = field_top - 18
        figures = f"<circle cx='{fx}' cy='{fy-34}' r='9' fill='#94a3b8'/><rect x='{fx-10}' y='{fy-26}' width='20' height='42' rx='8' fill='#465669'/><path d='M{fx-20} {fy-6} L{fx-38} {fy+20} M{fx+20} {fy-6} L{fx+4} {fy+22}' stroke='#58697b' stroke-width='4' stroke-linecap='round'/>"
...
def _draw_saju_landscape_pillow(draw, model: Dict[str, object], box: Tuple[int, int, int, int], font_small, font_mid, font_big) -> None:
    ...
    if dtype == "winter_silence":
        ...
        draw.ellipse((fx-8, fy-32, fx+8, fy-16), fill="#94a3b8")
    ...
    bx = x2 - 124
    for el in primary[:2]:
        color = LANDSCAPE_ELEMENT_META.get(el, {}).get("color", "#3730a3")
...
def _shinsal_badges_html(chart: Chart, result: Dict[str, object], limit: int = 3) -> str:
    ...
    for item in items:
        ...
        pills.append(
            f"<span style='display:inline-flex;align-items:center;gap:.28rem;padding:.28rem .56rem;border-radius:999px;background:#16161e;border:1px solid rgba(140,140,210,.45);color:#c090a8;font-size:.88rem;font-weight:900;box-shadow:0 4px 10px rgba(99,102,241,.06);'><span>{icon}</span><span>{name}</span></span>"
        )
...
def make_share_card_png_bytes(title: str, operation: str, keywords: str, overload: str, interpretation: str, one_liner: str, badges: List[str] | None = None, landscape_model: Dict[str, object] | None = None) -> bytes | None:
    ...
    draw.rounded_rectangle((28, 28, W-28, H-28), radius=46, fill="#fffdf8", outline="#9898b8", width=3)
    ...
    x = 86
    y = 84
    draw.text((x, y), "사주MRI", fill="#3730a3", font=f_logo)
    draw.text((W-310, y+4), "시각화 사주 분석", fill="#d0d0e8", font=f_label)

    y += 66
    for line in _wrap_for_image(draw, title, f_title, W-170)[:2]:
        draw.text((x, y), line, fill="#d0d0e8", font=f_title)
        y += 66

    def box(x1, y1, x2, y2, label, body):
        draw.rounded_rectangle((x1, y1, x2, y2), radius=24, fill="#fff8fb", outline="#f1cbd7", width=2)
        draw.text((x1+24, y1+20), label, fill="#3730a3", font=f_label)
        yy = y1 + 62
        for ln in _wrap_for_image(draw, body, f_body, x2-x1-48)[:2]:
            draw.text((x1+24, yy), ln, fill="#d0d0e8", font=f_body)
            yy += 39
    ...
    draw.text((112, y+20), "핵심 한 줄", fill="#3730a3", font=f_label)
    line_y = y + 58
    for ln in _wrap_for_image(draw, one_liner, f_body, 850)[:2]:
        draw.text((112, line_y), ln, fill="#d0d0e8", font=f_body)
        line_y += 38
    ...
    if badge_items:
        ...
        for badge in reversed(badge_items):
            ...
            draw.text((x1 + 16, by + 7), str(badge), fill="#c090a8", font=f_small)
            bx = x1 - 10
    ...
    draw.text((text_x, foot_y1 + 16), "다시 들어오기", fill="#3730a3", font=f_label)
    draw.text((text_x, foot_y1 + 50), "카톡 링크가 지나가면 바로가기 만들기 또는 주소 저장으로 다시 들어오세요.", fill="#d0d0e8", font=f_small)
    url_lines = _wrap_for_image(draw, APP_PUBLIC_URL, f_tiny, 690)[:2]
    url_y = foot_y1 + 82
    for ln in url_lines:
        draw.text((text_x, url_y), ln, fill="#c090a8", font=f_tiny)
        url_y += 24

    draw.text((86, H-44), "※ 사주MRI 결과 카드는 오락·자기이해용 요약입니다.", fill="#9898b8", font=f_tiny)
...
def make_simple_first_share_png_bytes(payload: Dict[str, object], char: Dict[str, str]) -> bytes | None:
    ...
    draw.text((x, y), "사주MRI", fill="#3730a3", font=f_logo)
    draw.text((W-300, y+4), "사주 진단서 요약", fill="#d0d0e8", font=f_label)

    y += 56
    # 이름 + 사주 원국
    draw.text((x, y), f"{_share_name} 님의 사주 원국", fill="#c4c4e8", font=f_small)
    y += 34
    ...
    for pi, (_pl, _pv) in enumerate(zip(_p_labels, _pillar_parts)):
        ...
        draw.text((cx_p - 16, y), _pl, fill="#c4c4e8", font=f_tiny)
        _pv_display = _pv if _pv else "-"
        draw.text((cx_p - 28, y + 24), _pv_display, fill="#d0d0e8", font=f_title)
    y += 90

    # 체질 한줄 설명
    for line in _wrap_for_image(draw, tone_plain, f_small, W-170)[:2]:
        draw.text((x, y), line, fill="#c090a8", font=f_small)
        y += 30
    ...
    if badge_items:
        ...
        for item in badge_items:
            ...
            draw.text((bx + 14, by + 8), label, fill="#c090a8", font=f_small)
            bx += tw + 10
        y = by + row_h + 18
    ...
    def box(x1, y1, x2, y2, label, body_lines, fill="#fff8fb"):
        ...
        draw.text((x1+22, y1+18), label, fill="#3730a3", font=f_label)
        yy = y1 + 58
        for ln in body_lines:
            draw.text((x1+22, yy), ln, fill="#d0d0e8", font=f_small)
            yy += 30
    ...
    draw.text((86, y), "오행 비율", fill="#3730a3", font=f_label)
    pill_x = 86
    pill_y = y + 38
    for el in ELEMENTS:
    ...
    draw.text((86, y), "기운 3대 축", fill="#3730a3", font=f_label)
    y += 38
    score_boxes = [
    ...
    for label, info, desc in score_boxes:
        ...
        draw.text((sx+20, y+16), label, fill="#d0d0e8", font=f_label)
        score = float(info.get("score", 0) or 0)
        max_score = float(info.get("max", 0) or 0)
        pct_score = float(info.get("pct", 0) or 0)
        draw.text((sx+20, y+58), f"{score:.1f}점", fill="#3730a3", font=f_body)
        draw.rounded_rectangle((sx+20, y+106, sx+sw-20, y+120), radius=8, fill="#f5e8ef")
        draw.rounded_rectangle((sx+20, y+106, sx+20 + int((sw-40) * pct_score / 100.0), y+120), radius=8, fill="#3730a3")
        for ln in _wrap_for_image(draw, desc, f_tiny, sw-40)[:2]:
            draw.text((sx+20, y+136 if ln == _wrap_for_image(draw, desc, f_tiny, sw-40)[:1][0] else y+158), ln, fill="#c090a8", font=f_tiny)
        sx += sw + 18
    y += 208
    ...
    for ln in _wrap_for_image(draw, one_liner, f_small, 850)[:2]:
        draw.text((112, line_y), ln, fill="#d0d0e8", font=f_small)
        line_y += 30
...
def render_daewun_train(payload: Dict[str, object]) -> None:
    ...
    if current:
        for i, item in enumerate(daewuns):
            if (item.get("start_age") == current.get("start_age")
                    and item.get("ganzhi") == current.get("ganzhi")):
                current_idx = i
                break

    # 현재 ±3 (최대 7개)
    start_i = max(0, current_idx - 3)
    end_i   = min(len(daewuns), current_idx + 4)
    visible  = daewuns[start_i:end_i]

    base_total = float(result.get("total", 0) or 0)
    useful     = result.get("useful", {}) or {}

    def _dw_story(score, mod, mood, gz, is_current):
        """카드 안 한 줄 풀이."""
        if is_current:
            prefix = "지금 이 흐름이야."
        else:
            prefix = "이 시절엔"
        if score >= 75:
            feeling = "바람이 등을 밀어주는 때야."
        elif score >= 60:
            feeling = "순한 기운이 흐르는 편이야."
        elif score >= 45:
            feeling = "무난하지만 욕심은 줄이는 게 좋아."
        else:
            feeling = "거스르는 바람이야. 버티는 게 현명해."
        mod_hint = ""
        if mod >= 2:    mod_hint = " 원국보다 올라가 있어."
        elif mod <= -2: mod_hint = " 원국보다 눌리는 편이야."
        return f"{prefix} {feeling}{mod_hint}"

    # 대운 카드 그리드 HTML
    parts = ["<div style='display:flex;flex-wrap:wrap;gap:10px;margin-bottom:16px;'>"]
    for item in visible:
        gz       = str(item.get("ganzhi", "-"))
        age      = str(item.get("start_age", "-"))
        is_cur   = bool(current
                        and item.get("start_age") == current.get("start_age")
                        and item.get("ganzhi")    == current.get("ganzhi"))
        try:
            detail  = _luck_station_plain_detail(gz, chart, useful, base_total, result)
            score   = int(detail.get("score", 50) or 50)
            mod     = float(detail.get("mod", 0.0) or 0.0)
            mood    = str(detail.get("mood", "보통 흐름"))
            note    = _clean_display_text(str(detail.get("note", "")))
            has_clash   = bool(detail.get("clashes"))
            has_binding = bool(detail.get("bindings"))
        except Exception:
            score, mod, mood, note, has_clash, has_binding = 50, 0.0, "보통", "", False, False

        story = _dw_story(score, mod, mood, gz, is_cur)
        ...
        signal_icon = ""
        if has_clash:   signal_icon = " ⚡"
        elif has_binding: signal_icon = " 🔗"

        parts.append(
            f"<div style='flex:1;min-width:120px;max-width:180px;background:{bg};"
            f"border:{border};border-radius:10px;padding:12px 10px;'>"
            f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;'>"
            f"<span style='background:{badge_bg};color:#fff;border-radius:4px;padding:1px 7px;"
            f"font-size:13px;font-weight:700;'>{badge_text}</span>"
            f"<span style='font-size:13px;color:#a0b4bc;'>{html.escape(age)}세~</span></div>"
            f"<div style='font-size:20px;font-weight:800;color:#93c5fd;text-align:center;"
            f"letter-spacing:2px;margin-bottom:4px;'>{html.escape(gz)}{signal_icon}</div>"
            f"<div style='font-size:13px;color:#b0c4cc;margin-bottom:4px;text-align:center;'>"
    ...
    if current:
        try:
            gz_cur = str(current.get("ganzhi", "-"))
            detail_cur = _luck_station_plain_detail(gz_cur, chart, useful, base_total, result)
            if detail_cur.get("bindings"):
                st.caption("🔗 현재 대운 — 원국과 합으로 묶이는 글자가 있어. 결합·변화의 신호야.")
                _markdown_signal_cards(detail_cur["bindings"])
            if detail_cur.get("clashes"):
                st.caption("⚡ 현재 대운 — 원국과 충으로 움직이는 글자가 있어. 변동·이동 신호야.")
                _markdown_signal_cards(detail_cur["clashes"])
        except Exception:
            pass
...
def render_relationship_orientation_card(compatibility: Dict[str, object]) -> None:
    ...
    st.markdown(
        "<div class='signal-card' style='border-color:rgba(16,185,129,.35);background:linear-gradient(135deg,#0d1e14,#16161e);margin:.7rem 0 1rem;'>"
...
def _mini_axis_polygon(axis_values: Dict[str, float]) -> str:
    ...
    for ratio in [0.5, 1.0]:
        pts = [f"{cx + _math.cos(a) * max_r * ratio:.1f},{cy + _math.sin(a) * max_r * ratio:.1f}" for a in angles]
        grid.append(f"<polygon points='{' '.join(pts)}' fill='none' stroke='rgba(99,102,241,.18)' stroke-width='1'/>")
    pts = []
    ...
    for a, lab in zip(angles, labels):
        ...
        if lx < cx - 5:
            ...
            anchor = "start"
        label_nodes.append(f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='9' font-weight='800' fill='#c090a8'>{html.escape(lab)}</text>")
    return "<svg viewBox='0 0 160 160' width='160' height='160'>" + "".join(grid) + f"<polygon points='{' '.join(pts)}' fill='rgba(99,102,241,.22)' stroke='#3730a3' stroke-width='2'/>" + "".join(label_nodes) + "</svg>"
...
def make_chemistry_share_card_png_bytes(
    ...
    draw.text((x, y), logo_text, fill="#3730a3", font=f_logo)
    pair_w, _ = measure(pair_text, f_label)
    draw.text((W - 92 - pair_w, y + 2), pair_text, fill="#d0d0e8", font=f_label)
    ...
    draw.text((122, y+18), "한눈 궁합점수", fill="#3730a3", font=f_score_label)
    draw.text((122, y+48), f"{int(score)}/100", fill="#3730a3", font=f_score)
    y += 146
    ...
    def panel(x1, y1, x2, y2, label, body, fill="#fff9fc", body_font=None, max_lines=3):
        ...
        for ln in lines:
            draw.text((x1+22, yy), ln, fill="#d0d0e8", font=bf)
            yy += gap
    ...
    draw.text((118, y+18), "핵심 한 줄", fill="#3730a3", font=f_label)
    yy = y + 54
...
def _chem_story_text(mine: dict, friend: dict, compatibility: dict, chem: dict) -> str:
    """두 사람의 케미를 따뜻한 말투로 풀어준다."""
    my_name  = str(mine.get("name", "참가자1"))
    fr_name  = str(friend.get("name", "참가자2"))
    score    = float(compatibility.get("score", 0) or 0)
    axes     = compatibility.get("axes") or {}
    complement = float(axes.get("상호 보완성", 0) or 0)
    alliance   = float(axes.get("동맹성", 0) or 0)
    sync       = float(axes.get("흐름 동조성", 0) or 0)
    tension    = float(axes.get("긴장도", 0) or 0)
    chem_title = chem.get("title", "조율형 케미")
    ...
    type_stories = {
        ...
        "스파크형 케미": (f"이 사이에서는 불꽃이 좀 튀겠어. 서로 자극하고 흔드는 기운이 있거든. 나쁜 게 아니야 — "
                         f"이런 케미가 서로를 크게 만들어주는 법이야. 다만 감정 관리는 잘 해야 해."),
        "리듬형 케미": (f"이 두 사람 호흡이 참 잘 맞아. 같은 박자로 움직이는 사이야. 같이 뭔가를 하면 "
                       f"— 일이든 생활이든 — 서로 발 맞추기가 편한 관계야."),
        "조율형 케미": (f"이 두 사람은 노력하면서 맞춰가는 사이야. 처음부터 척척 맞는 건 아닌데, "
                       f"조율하고 맞추다 보면 오히려 단단해지는 관계거든. 기대치를 잘 맞추는 게 핵심이야."),
    }
    type_story = type_stories.get(chem_title, "이 두 사람만의 고유한 케미가 있어.")

    # 축 강조
    axis_lines = []
    if complement >= 68:
        axis_lines.append("서로 부족한 걸 채워주는 힘이 특히 강해 — 함께 있으면 더 완전해지는 느낌이 드는 사이야.")
    if alliance >= 68:
        axis_lines.append("닮은 구석이 많아서 처음부터 말이 잘 통해. 설명을 길게 안 해도 이해하는 그런 케미야.")
    if sync >= 65:
        axis_lines.append("생활 리듬이 잘 맞아. 같이 움직이면 발이 저절로 맞춰지는 사이야.")
    if tension >= 65:
        axis_lines.append("긴장감이 좀 있는 사이야. 그 자극을 어떻게 쓰느냐에 따라 이 관계가 확 달라질 수 있어.")

    parts = [score_story, type_story] + axis_lines
    return " ".join(parts)


def _render_chem_prescription(mine: dict, friend: dict, compatibility: dict) -> None:
    """케미 처방전 — 두 사람이 잘 지내는 방법을 풀어준다."""
    axes     = compatibility.get("axes") or {}
    complement = float(axes.get("상호 보완성", 0) or 0)
    alliance   = float(axes.get("동맹성", 0) or 0)
    tension    = float(axes.get("긴장도", 0) or 0)
    ...
    advices = []
    if complement >= 65:
        advices.append("💊 서로 다른 강점이 잘 맞으니, 상대가 잘하는 것을 존중하고 역할을 나눠봐. 같이 하면 혼자보다 훨씬 커지는 사이야.")
    if alliance >= 65:
        advices.append("🪞 닮은 점이 많아서 편하지만, 같은 약점도 공유해. 서로에게 새로운 시각을 일부러 가져다줘봐.")
    if tension >= 60:
        if clash >= 2:
            advices.append("⚡ 충이 강하게 만나는 사이야. 감정이 올라올 때 바로 반응하지 말고, 한 박자 쉬어가는 습관이 아주 중요해.")
        else:
            advices.append("⚡ 자극이 있는 사이야. 그 긴장을 싸움으로 쓰지 말고, 서로를 성장시키는 에너지로 써봐.")
    if harmony >= 2:
        advices.append("🤝 합이 잘 맞는 기둥이 있어. 공통 목표나 루틴을 만들면 이 관계가 훨씬 안정돼.")
    if not advices:
        advices.append("🎛️ 서로 맞춰가는 사이야. 기대치를 솔직하게 이야기하는 것이 이 관계를 오래 가게 하는 비결이야.")

    st.markdown("#### 📋 케미 처방")
    for adv in advices:
        st.markdown(
            f"<div style='background:#16161e;border-left:3px solid #3730a3;"
            f"padding:9px 13px;border-radius:6px;margin-bottom:6px;"
            f"font-size:13px;color:#ddd0d8;line-height:1.7;'>{adv}</div>",
            unsafe_allow_html=True,
        )



def group_chemistry_analysis(participants: list) -> dict:
    """
    모임 케미 전용 그룹 오행 균형 분석.

    단순 쌍별 케미의 합산이 아니라, 모임 전체를 하나의 명식 집합으로 보고
    오행 과다·부족 / 체온 편향 / 강약 분포를 진단한다.

    과다 기준: 그룹 평균 27% 초과 (균형점 20% 대비 +7%)
    부족 기준: 그룹 평균 13% 미만 (균형점 20% 대비 -7%)
    """
    ELEM_KR  = ["목", "화", "토", "금", "수"]
    ...
    ELEM_RX_GROUP = {
        ...
        "토": "규칙적 모임 일정 만들기, 함께 식사, 공동 루틴 정하기",
        "금": "공동 목표·규칙 정하기, 정리·결산 시간, 명확한 역할 분담",
        "수": "조용한 대화 시간, 여행·명상·독서 모임, 물가 근처에서 만나기",
    }
    OVER_RX = {
        "목": "새것을 자꾸 시작하거나 일을 벌리는 경향에 주의. 마무리를 함께 챙겨.",
        "화": "흥분·과열·경쟁이 쉽게 달아오를 수 있어. 서늘하고 조용한 시간도 필요해.",
        "토": "각자의 영역을 지나치게 고집하거나 움직이기 싫어질 수 있어. 변화를 일부러 만들어봐.",
        "금": "비판이나 기준이 너무 강해질 수 있어. 유연하게 보는 연습을 같이 해.",
        "수": "생각이 너무 많아지거나 결정을 못 할 수 있어. 가볍게 행동으로 옮기는 연습을 해봐.",
    }

    n = len(participants)

    # ── 1. 개인별 오행 비율·체온·강약 수집 ─────────────────────
    member_pcts      = []
    member_temps     = []
    member_strengths = []
    ...
    over_elements  = [el for el in ELEM_KR if group_avg[el] >= 27.0]
    under_elements = [el for el in ELEM_KR if group_avg[el] <= 13.0]

    # 균형 점수: 각 원소의 20% 대비 편차 합산 (100점 만점)
    balance_score = max(0, round(100 - sum(abs(group_avg[el] - 20.0) for el in ELEM_KR) * 2))

    # ── 3. 그룹 체온 분석 ───────────────────────────────────────
    avg_temp      = sum(member_temps) / n
    temp_variance = (sum((t - avg_temp) ** 2 for t in member_temps) / n) ** 0.5

    if avg_temp >= 65:
        temp_label = "과열 집단"
        temp_note  = ("모임 전체가 뜨거운 기운으로 가득해. 흥이 넘치지만 과열되거나 충돌이 생길 수 있어. "
                      "서늘하고 조용한 환경이 도움이 돼.")
        temp_rx    = "서늘한 공간·물·조용한 대화 시간 확보"
    elif avg_temp >= 55:
        temp_label = "열성 집단"
        temp_note  = ("따뜻하고 활발한 기운의 모임이야. 에너지가 넘치는데, 가끔 쉬어가는 시간도 중요해.")
        temp_rx    = "중간 휴식·물 마시기·조용한 대화 시간"
    elif avg_temp >= 45:
        temp_label = "균형 집단"
        temp_note  = ("온도 균형이 잘 맞는 모임이야. 뜨겁지도 차갑지도 않아서 오래 함께하기 좋아.")
        temp_rx    = "현재 패턴 유지"
    elif avg_temp >= 35:
        temp_label = "냉성 집단"
        temp_note  = ("차분하고 이성적인 모임이야. 깊은 대화는 잘 되는데, 함께 열기를 올리는 활동이 도움이 돼.")
        temp_rx    = "활동적인 야외 활동·따뜻한 음식·불 주변 모임"
    else:
        temp_label = "한랭 집단"
        temp_note  = ("기운이 많이 차가운 모임이야. 서로 마음을 열고 따뜻하게 나누는 시간이 꼭 필요해.")
        temp_rx    = "따뜻한 음료·불 주변 모임·몸 쓰는 활동"

    # 편차가 크면 온도 충돌 주의
    if temp_variance >= 15:
        temp_note += " 개인 간 온도 차이가 커서 너무 뜨겁거나 차가운 자리에선 불편한 사람이 생길 수 있어."

    # ── 4. 강약 분포 ────────────────────────────────────────────
    strength_count = {"극신강": 0, "신강": 0, "중화": 0, "신약": 0, "극신약": 0}
    for sl in member_strengths:
        if sl in strength_count:
            strength_count[sl] += 1
    strong_n = strength_count["신강"] + strength_count["극신강"]
    weak_n   = strength_count["신약"] + strength_count["극신약"]

    if strong_n > weak_n + 1:
        strength_balance = "추진력 중심"
        strength_note    = "힘 강한 사람들이 많아. 리더십 경쟁이 생길 수 있으니 역할 분담을 명확히 해."
    elif weak_n > strong_n + 1:
        strength_balance = "협력형"
        strength_note    = "유연하고 협력적인 사람들이 많아. 결단이 필요할 때 방향을 잡아줄 사람이 필요해."
    else:
        strength_balance = "균형형"
        strength_note    = "강한 사람과 유연한 사람이 잘 섞여 있어. 역할이 자연스럽게 나뉘는 좋은 구성이야."

    # ── 5. 개인 기여 역할 분류 ──────────────────────────────────
    roles = []
    for i, p in enumerate(participants):
        pct    = member_pcts[i]
        dom_el = max(ELEM_KR, key=lambda e: pct.get(e, 0))
        dom_pct = pct.get(dom_el, 0)
        str_lbl = member_strengths[i]

        # 내 주 원소가 그룹 결핍 원소를 채워주는가?
        if dom_el in under_elements:
            role_type  = "균형자"
            role_note  = (f"그룹에서 부족한 {ELEM_HJ[dom_el]}({ELEM_KW[dom_el]}) 기운을 채워주는 핵심 역할. "
                          f"이 사람이 이 모임에 균형을 가져와.")
            role_color = "#16a34a"
        elif dom_el in over_elements:
            role_type  = "증폭자"
            role_note  = (f"이미 넘치는 {ELEM_HJ[dom_el]}({ELEM_KW[dom_el]}) 기운을 더 높여. "
                          f"이 방면은 조금 자제하고 다른 역할을 찾으면 모임이 더 조화로워.")
            role_color = "#dc2626"
        else:
            if str_lbl in ("중화",):
                role_type  = "조율자"
                role_note  = "강약이 균형 잡혀 있어 모임의 중심을 자연스럽게 잡아줘."
                role_color = "#3B5BDB"
            elif str_lbl in ("극신강", "신강"):
                role_type  = "드라이버"
                role_note  = "추진력 있게 모임을 끌어가는 역할. 방향을 잡는 데 도움이 돼."
                role_color = "#d97706"
            else:
                role_type  = "완충자"
                role_note  = "유연하게 다른 사람들의 기운을 흡수해 충돌을 부드럽게 완화해."
                role_color = "#7c6cd0"

        roles.append({
            "name":       p.get("name", f"참가자{i+1}"),
            "dom_el":     dom_el,
            "dom_hj":     ELEM_HJ[dom_el],
    ...
    # ── 6. 그룹 처방 ────────────────────────────────────────────
    group_rx = []
    for el in under_elements:
        group_rx.append(("under", ELEM_HJ[el], ELEM_KW[el], ELEM_RX_GROUP[el]))
...
def render_group_chemistry_diagnosis(ga: dict) -> None:
    """그룹 오행 균형 진단 결과를 렌더링한다."""
    ELEM_KR     = ["목", "화", "토", "금", "수"]
    ELEM_HJ     = ga["ELEM_HJ"]
    ELEM_KW     = ga["ELEM_KW"]
    ELEM_COLORS = ga["ELEM_COLORS"]

    st.markdown("### 🔬 모임 오행 균형 진단")
    st.caption("두 사람의 궁합과는 다르게, 모임 전체의 오행 비율을 합산해서 과다하거나 부족한 기운이 무엇인지 봐.")

    # ── 균형 점수 배지 ─────────────────────────────────────────
    bs = ga["balance_score"]
    bs_color = "#16a34a" if bs >= 70 else ("#d97706" if bs >= 50 else "#dc2626")
    bs_label = "균형 우수" if bs >= 70 else ("보통 균형" if bs >= 50 else "불균형 주의")
    st.markdown(
        f"<div style='background:#0d1828;border:1px solid rgba(120,120,220,0.25);border-radius:8px;"
        f"padding:10px 14px;margin-bottom:12px;'>"
        f"<span style='background:#3B5BDB;color:#fff;border-radius:4px;padding:2px 8px;"
        f"font-size:12px;font-weight:700;margin-right:8px;'>오행 균형 점수</span>"
        f"<span style='font-size:16px;font-weight:700;color:{bs_color};'>{bs}점 · {bs_label}</span>"
        f"</div>",
        unsafe_allow_html=True,
    )

    # ── 오행 분포 바 ───────────────────────────────────────────
    bars_html = "<div style='display:flex;gap:6px;margin-bottom:10px;'>"
    for el in ELEM_KR:
        pct    = ga["group_avg"].get(el, 0)
    ...
    # ── 과다·부족 설명 ─────────────────────────────────────────
    if ga["over_elements"]:
        over_str = ", ".join(f"{ELEM_HJ[e]}({ELEM_KW[e]})" for e in ga["over_elements"])
        st.markdown(
            f"<div style='background:#200d0d;border-left:4px solid #f87171;"
            f"padding:9px 13px;border-radius:0 6px 6px 0;margin-bottom:6px;"
            f"font-size:13px;color:#ddd0d8;line-height:1.7;'>"
            f"▲ 과다 — {over_str}<br>"
            f"이 기운은 이 모임에 이미 넘쳐. 이 방향의 활동이나 대화를 더 쌓으면 "
            f"오히려 갈등·과부하가 생길 수 있어.</div>",
            unsafe_allow_html=True,
        )
    if ga["under_elements"]:
        under_str = ", ".join(f"{ELEM_HJ[e]}({ELEM_KW[e]})" for e in ga["under_elements"])
        st.markdown(
            f"<div style='background:#0d1e14;border-left:4px solid #4ade80;"
            f"padding:9px 13px;border-radius:0 6px 6px 0;margin-bottom:6px;"
            f"font-size:13px;color:#ddd0d8;line-height:1.7;'>"
            f"▼ 부족 — {under_str}<br>"
            f"이 기운이 모임에서 약해. 의식적으로 이 성질의 활동을 만들면 "
            f"모임이 훨씬 더 조화롭고 풍요로워져.</div>",
            unsafe_allow_html=True,
        )
    if not ga["over_elements"] and not ga["under_elements"]:
        st.markdown(
            f"<div style='background:#0d1e14;border-left:4px solid #4ade80;"
            f"padding:9px 13px;border-radius:0 6px 6px 0;margin-bottom:6px;"
            f"font-size:13px;color:#b0c4cc;'>"
            f"✅ 오행이 비교적 고루 분포된 균형 잡힌 모임이야. "
    ...
    if _j:
        ...
        st.markdown(
            ...
            f"<div style='font-size:13px;color:#ddd0d8;line-height:1.7;'>{_j['note']}</div>"
    ...
    tc, tbg = _THERMO_COLORS.get(_t["label"], ("#94a3b8", "#16161e"))
    st.markdown(
    ...
    st.markdown(
        f"<div style='background:#16161e;border:1px solid #f8d0e0;padding:9px 13px;"
        f"border-radius:6px;font-size:13px;color:#ddd0d8;line-height:1.7;margin-bottom:4px;'>"
    ...
    st.markdown("#### 💪 강약 균형")
    _s = ga["strength"]
    dist = _s["distribution"]
    dist_str = " / ".join(f"{k} {v}명" for k, v in dist.items() if v > 0)
    st.markdown(
        f"<div style='background:#0d1828;border:1px solid rgba(120,120,220,0.25);border-radius:8px;"
        f"padding:9px 13px;margin-bottom:6px;font-size:13px;color:#ddd0d8;line-height:1.7;'>"
        f"<span style='font-weight:700;color:#3B5BDB;'>{_s['balance']}</span>"
        f" ({dist_str}) — {_s['note']}</div>",
        unsafe_allow_html=True,
    )
    ...
    # ── 개인 기여 역할 ─────────────────────────────────────────
    st.markdown("#### 🎭 각 사람의 모임 역할")
    for role in ga["roles"]:
        rc = role["role_color"]
        st.markdown(
            f"<div style='background:#16161e;border:0.5px solid #e0e0e8;border-radius:8px;"
            f"padding:8px 12px;margin-bottom:6px;display:flex;align-items:flex-start;gap:12px;'>"
            f"<div style='min-width:56px;text-align:center;background:{rc}18;"
            f"border:1px solid {rc}55;border-radius:6px;padding:5px 4px;flex-shrink:0;'>"
            f"<div style='font-size:12px;font-weight:700;color:{rc};line-height:1.3;'>{role['role_type']}</div>"
            f"</div>"
            f"<div>"
            f"<div style='font-size:13px;font-weight:600;color:#b0c4cc;'>"
            f"{role['name']} — {role['dom_hj']}({ELEM_KW.get(role['dom_el'],'')}) "
            f"{role['dom_pct']}% · {role['strength']}</div>"
            f"<div style='font-size:12px;color:#a0b4bc;margin-top:2px;'>{role['role_note']}</div>"
            f"</div></div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown("#### 📋 모임 처방")
    if ga["group_rx"]:
        for kind, hj, kw, rx_text in ga["group_rx"]:
            color = "#16a34a" if kind == "under" else "#d97706"
            ...
            st.markdown(
                ...
                f"<span style='font-weight:700;color:{color};'>{icon} {label}</span><br>{rx_text}</div>",
                unsafe_allow_html=True,
            )
    else:
        st.markdown(
            f"<div style='background:#0d1e14;border-left:4px solid #4ade80;"
            f"padding:9px 13px;border-radius:0 6px 6px 0;font-size:13px;color:#b0c4cc;'>"
    ...
    if _conclusion:
        st.markdown("#### 🧩 종합 결론")
        st.markdown(
            f"<div style='background:#14141e;border-left:4px solid #7c6cd0;"
...
def render_multi_chem_result(participants: list) -> None:
    """3~5명 모임 케미 — 폴리곤 다이어그램 + 관계 풀이."""
    import math as _math

    n = len(participants)
    node_colors  = ["#3B5BDB", "#3730a3", "#16a34a", "#d97706", "#7c6cd0"]

    # ── 쌍별 호환성 계산 ────────────────────────────────────────
    pairs = []
    for i in range(n):
        for j in range(i + 1, n):
            comp = compatibility_analysis(participants[i], participants[j], "친구")
            chem = chemistry_type_from_compatibility(comp)
            pairs.append({
                "i": i, "j": j,
                "a": participants[i]["name"], "b": participants[j]["name"],
                "score": float(comp.get("score", 0) or 0),
                "type": chem["title"],
                "stamp": chem.get("stamp", ""),
                "summary": str(comp.get("summary", "") or ""),
                "compat": comp, "chem": chem,
            })
    ...
    positions = []
    for k in range(n):
        ang = -_math.pi / 2 + 2 * _math.pi * k / n
        positions.append((cx + r_node * _math.cos(ang), cy + r_node * _math.sin(ang)))

    def _score_color(s: float) -> str:
        if s >= 73: return "#16a34a"
        if s >= 52: return "#d97706"
        return "#dc2626"

    svg = [
        f'<div style="background:rgba(20,8,16,0.96);border:1px solid rgba(140,140,210,.30);'
    ...
    # 엣지 (관계선)
    for p in pairs:
        x1, y1 = positions[p["i"]]; x2, y2 = positions[p["j"]]
        color = _score_color(p["score"])
        mx, my = (x1 + x2) / 2, (y1 + y2) / 2
        # 화살표 (양방향)
        svg.append(
            f'<defs><marker id="arr_{p["i"]}_{p["j"]}" markerWidth="6" markerHeight="4" '
            f'refX="5" refY="2" orient="auto">'
            f'<polygon points="0 0,6 2,0 4" fill="{color}" fill-opacity="0.7"/></marker></defs>'
        )
        svg.append(
            f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" '
            f'stroke="{color}" stroke-width="1.8" stroke-opacity="0.55" '
            f'marker-end="url(#arr_{p["i"]}_{p["j"]})"/>'
        )
        # 점수 배지
        svg.append(
            f'<rect x="{mx - 16:.1f}" y="{my - 9:.1f}" width="32" height="18" '
            f'rx="4" fill="#16161e" fill-opacity="0.96" stroke="{color}" stroke-width="1.2"/>'
        )
        svg.append(
    ...
    with c1:
        st.markdown(
            f"<div style='background:#0d1e14;border-left:3px solid #16a34a;"
            f"padding:8px 12px;border-radius:6px;font-size:12px;margin-bottom:4px;'>"
            f"<div style='font-weight:700;color:#4ade80;'>🔥 베스트 케미</div>"
            f"<div style='font-weight:600;color:#b0c4cc;'>{best['a']} × {best['b']}</div>"
            f"<div style='color:#b0c4cc;'>{best['score']:.0f}점 · {best['type']}{best['stamp']}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    with c2:
        st.markdown(
            f"<div style='background:#1a1408;border-left:3px solid #d97706;"
            f"padding:8px 12px;border-radius:6px;font-size:12px;margin-bottom:4px;'>"
            f"<div style='font-weight:700;color:#d97706;'>🎛️ 조율 포인트</div>"
            f"<div style='font-weight:600;color:#b0c4cc;'>{worst['a']} × {worst['b']}</div>"
            f"<div style='color:#b0c4cc;'>{worst['score']:.0f}점 · {worst['type']}{worst['stamp']}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    # ── 쌍별 케미 풀이 ──────────────────────────────────────────
    st.markdown("#### 💬 두 사람 이야기")
    for p in sorted_pairs:
        mine_p   = next(x for x in participants if x["name"] == p["a"])
        friend_p = next(x for x in participants if x["name"] == p["b"])
        gtext = _chem_story_text(mine_p, friend_p, p["compat"], p["chem"])
        st.markdown(
            f"<div style='background:#16161e;border:1px solid #f8d0e0;"
            f"border-radius:8px;padding:10px 14px;margin-bottom:8px;'>"
            f"<div style='font-size:12px;font-weight:700;color:#818cf8;margin-bottom:4px;'>"
            f"{p['stamp']} {p['a']} × {p['b']} — {p['score']:.0f}점 · {p['type']}</div>"
            f"<div style='font-size:13px;color:#ddd0d8;line-height:1.7;'>{gtext}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
...
def render_battle_result_board(mine: Dict[str, object], friend: Dict[str, object], battle_summary: Dict[str, object], compatibility: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div class="chem-hero-premium" style="text-align:center;padding:1.45rem 1.1rem;background:linear-gradient(135deg,#16161e,#1a1a26);border:1px solid rgba(100,100,220,.30);border-radius:26px;box-shadow:0 14px 30px rgba(100,100,220,.12);margin:.5rem 0 1rem 0;">
        <div style="font-size:2.4rem;line-height:1;">{chem['stamp']}</div>
        <div style="font-size:clamp(1.8rem,4vw,2.55rem);font-weight:950;color:#818cf8;letter-spacing:-.05em;">{safe_chem_title}</div>
        <div style="margin-top:.35rem;color:#d0d0e8;font-weight:950;font-size:1.15rem;">궁합점수 {safe_chem_score}/100</div>
        <div style="margin-top:.55rem;color:#9898b8;font-weight:760;">{safe_chem_desc}</div>
        <div style="margin-top:.45rem;color:#9898b8;font-size:.92rem;">{safe_summary}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if view == " 케미 한눈에":
        ...
        st.markdown(
            f"<div style='background:#16161e;border:1px solid #f8d0e0;padding:12px 16px;"
            f"border-radius:8px;font-size:14px;color:#ddd0d8;line-height:1.8;margin-bottom:8px;'>"
            f"{_gtext}</div>",
            unsafe_allow_html=True,
        )
        _chem_sum = str(compatibility.get("summary", "") or "")
        if _chem_sum:
            st.caption(f"💬 {_chem_sum}")
        st.divider()
        ...
        expert_view = st.radio(
            "케미 상세 항목",
            ["관계 분위기", "상세 케미", "합·충·형·원진", "대운·세운", "계산 참고 지표"],
            index=0,
            horizontal=True,
            key="chem_expert_detail_view_radio_v590",
        )

        if expert_view == "관계 분위기":
            render_relationship_orientation_card(compatibility)

        elif expert_view == "상세 케미":
...
def render_analysis_logic_panel(mode: str, key: str, show_toggle: bool = True) -> None:
    ...
    if mode == "single":
        title = "단일 분석 로직"
        rows = [
            ("총점", "대표 점수는 기초체력(기운의 크기) + 흐름과 연결 + 현실작동력(기운의 발현)에 신살·공망 체감 보정을 더한 값입니다. 전체 인상 보정은 해설용 참고 신호로만 봅니다."),
        ...
        title = "고급 다자간 분석 로직"
        rows = [
            ("순위", "참가자별 총점와 등급을 기준으로 1차 순위를 계산합니다."),
        ...
        if mode == "meal":
            rows = [
                ("밥값 회피력", "기본 50점에서 시작해 원국 재성 구조, 보완·부담 방향, 대운, 월운, 일운, 총점 보조값을 더하거나 뺍니다."),
    ...
    st.markdown(f"##### 🧪 {title}")
    st.caption("점수가 어떻게 계산됐는지 궁금할 때 확인해봐.")
...
def render_selected_daily_luck_card(chart: Chart, result: Dict[str, object], target_date: date) -> None:
    ...
    day_title    = str(compass.get("day_luck_title", day_gz))

    # ── 진단 풀이 ───────────────────────────────────────
    _tengod_picks = {
        "비견": "이날은 내 편이 필요하거나 동등하게 부딪히는 기운이야. 혼자보단 함께 움직이면 좋아.",
        "겁재": "이날은 경쟁이나 분쟁 기운이 좀 있어. 내 것을 지키되 욕심은 내려놔봐.",
        "식신": "이날은 먹고 즐기고 표현하는 날이야. 뭔가 만들거나 느긋하게 쉬기에 딱 좋아.",
        "상관": "이날은 말이 많아지고 표현이 강해지는 날이야. 아이디어는 좋지만 말실수 조심해.",
        "편재": "이날은 돈이나 사람이 빠르게 움직여. 기회는 잡되 너무 집착하지는 마.",
        "정재": "이날은 꼼꼼하고 안정적인 기운이야. 계획대로 하나씩 하면 딱 좋아.",
        "편관": "이날은 긴장감이 있어. 결단이 필요한 순간이 올 수 있으니 너무 움츠러들지 마.",
        "정관": "이날은 규칙과 책임의 기운이야. 바르게 행동하면 나중에 좋은 결과가 따라와.",
        "편인": "이날은 직관이 살아나는 날이야. 혼자 깊이 생각하거나 공부하기 좋아.",
        "정인": "이날은 배움과 안정의 기운이야. 도움받거나 지식을 흡수하기 좋은 날이거든.",
    }
    _tg_story = "이날 기운은 조용히 흐르는 편이야."
    for _tg, _story in _tengod_picks.items():
        if _tg in ten_god_line:
            _tg_story = _story
            break

    _sig_story = ""
    if "충" in interaction_overview:
        _sig_story = " 원국과 부딪히는 신호가 있어. 무리하게 밀기보다 유연하게 가봐."
    elif "합" in interaction_overview:
        _sig_story = " 원국과 연결되는 신호가 있어. 사람과 일이 자연스럽게 이어지는 날이야."

    _rx_story = ""
    if prescription and prescription not in ("-", "없음"):
        _rx_story = f" {prescription}"

    _story_full = f"{_tg_story}{_sig_story}{_rx_story}"

    st.markdown(
        f"<div style='background:#14141e;border-left:4px solid #7c3aed;padding:12px 16px;"
    ...
    _tg_html = (
        ...
        f"<b style='color:#9898b8;'>{html.escape(_stem_detail)}</b><br>"
        f"<b style='color:#9898b8;'>{html.escape(_branch_detail)}</b>"
        f"</div>"
    )
    if _tg_focus:
        _tg_html += (
    ...
    if _interactions:
        ...
        for _it in _interactions[:5]:
            ...
            _inter_rows += (
                ...
                f"<span style='font-size:13px;font-weight:700;color:#d0d0e8;'>{html.escape(_iname)}</span>"
...
def render_daily_luck_calendar(payload: Dict[str, object]) -> None:
    ...
    st.caption("날짜를 선택하면 그날의 기운과 흐름을 볼 수 있어. 어디까지나 참고로만 봐줘.")



def render_daewun_text_overview(payload: dict) -> None:
    """🌊 10년의 처방 — 요약 풀이 + 대운 카드 그리드."""
    luck_flow   = payload.get("luck_flow") or {}
    result      = payload.get("result", {}) or {}
    chart       = payload.get("chart")
    current     = luck_flow.get("current_daewun") if isinstance(luck_flow, dict) else None
    ...
    if current:
        gz        = str(current.get("ganzhi", "-"))
        start_age = current.get("start_age", "-")
        useful    = result.get("useful", {}) or {}
        base_total = float(result.get("total", 0) or 0)
        try:
            detail    = _luck_station_plain_detail(gz, chart, useful, base_total, result)
            score     = detail.get("score", 50)
            mood      = str(detail.get("mood", ""))
            note      = _clean_display_text(str(detail.get("note", "")))
            mod       = float(detail.get("mod", 0.0) or 0.0)
        except Exception:
            score, mood, note, mod = 50, "보통 흐름", "", 0.0

        # 점수 → 풀이 생성
        _score_grade = (
            "최상" if score >= 75 else
            "양호" if score >= 60 else
            "보통" if score >= 45 else "주의"
        )
        if score >= 75:
            score_story = (
                f"10년 흐름 강도 {score}점 — {_score_grade} 구간이야. "
                f"{gz} 대운 ({start_age}세부터 10년)은 원국이랑 아주 잘 맞는 시기거든. "
                f"밀어붙이면 생각보다 잘 풀려. 이 10년 안에 뭔가 하나 제대로 해보고 싶었던 거 있으면 지금이 그 타이밍이야. "
                f"기회가 왔을 때 머뭇거리지 마 — 이런 흐름은 자주 오지 않아."
            )
        elif score >= 60:
            score_story = (
                f"10년 흐름 강도 {score}점 — {_score_grade} 구간이야. "
                f"{gz} 대운 ({start_age}세부터 10년)은 무난하고 순탄한 편이거든. "
                f"극적인 대박보다는 꾸준히 쌓아가는 흐름이야. 뿌린 만큼 거두는 시기라서, "
                f"지금 성실하게 해두면 나중에 든든하게 돌아와. 조급해하지 말고 원칙대로 가봐."
            )
        elif score >= 45:
            score_story = (
                f"10년 흐름 강도 {score}점 — {_score_grade} 구간이야. "
                f"{gz} 대운 ({start_age}세부터 10년)은 좋은 해도 있고 버거운 해도 있어. "
                f"한꺼번에 너무 많이 벌리면 탈이 나는 흐름이거든 — 욕심을 조금 줄이고 내 페이스대로 가는 게 중요해. "
                f"작게 쌓고, 작게 잃고, 꾸준히 버티는 게 이 대운의 전략이야."
            )
        else:
            score_story = (
                f"10년 흐름 강도 {score}점 — {_score_grade} 구간이야. "
                f"{gz} 대운 ({start_age}세부터 10년)은 거스르는 기운이 좀 있어. "
                f"억지로 밀어붙이면 더 힘들어지는 시기거든. 새 걸 크게 시작하기보다 지금 가진 걸 잘 지키고 다지는 게 훨씬 현명해. "
                f"이 시기를 버티며 잘 준비해두면, 다음 대운 때 꽃이 피어."
            )
        mood_line = f"이 10년은 「{mood}」 기운이 전반에 깔려 있어. " if mood else ""
        note_line  = note if note else ""

        if mod >= 3:
            mod_story = "원국보다 기운이 많이 올라와 있는 시기야. 적극적으로 움직이면 생각보다 잘 돼 — 자신감 갖고 나가봐."
        elif mod >= 1:
            mod_story = "원국보다 살짝 좋은 기운이 더 실려 있어. 작은 기회들을 놓치지 말고 챙겨봐."
        elif mod <= -3:
            mod_story = "원국보다 기운이 꽤 눌리는 시기야. 무리하게 달리지 말고, 숨 고르는 시간으로 써봐. 쉬는 것도 전략이거든."
        elif mod <= -1:
            mod_story = "원국보다 살짝 눌리는 기운이 있어. 조급해하지 말고 차분하게 — 지금 할 수 있는 것부터 하나씩."
        else:
            mod_story = "원국과 비슷한 온도로 흘러가는 시기야. 큰 변동 없이 내 리듬대로 가면 돼."
        ...
        if not daewuns:
            st.caption("전체 대운 목록은 생년월일시 자동 산출 모드에서 표시됩니다.")
    else:
        st.info(
            "10년의 처방은 생년월일시 자동 산출 모드일 때 표시됩니다. "
            "직접 원국을 입력한 경우 대운 정보가 없습니다."
        )

    st.caption("대운은 10년마다 바뀌는 큰 흐름이야. 그 안에서 삶의 계절이 달라진다고 보면 돼.")
    ...
    if daewuns:
        ...
        with st.expander("📊 전체 대운 목록 보기"):
            render_manse_daewun_cards(payload)


def render_sewun_text_overview(payload: dict) -> None:
    """🌸 올해의 처방 — 세운 풀이 + 상세 연도표."""
    import datetime as _dt
    luck_flow  = payload.get("luck_flow") or {}
    sewun_rows = (luck_flow or {}).get("sewun_rows") or []
    ...
    if row_this:
        ...
        score = str(row_this.get("점수", "-"))
        delta = str(row_this.get("변동", ""))
        note  = _clean_display_text(str(row_this.get("비고", "")))
        ...
        _sewun_grade = (
            "최상" if score_num >= 75 else
            "양호" if score_num >= 60 else
            "보통" if score_num >= 45 else "주의"
        )
        if score_num >= 75:
            score_story = (
                f"올해 흐름 참고점 {score_num:.0f}점 — {_sewun_grade} 구간이야. "
                f"{this_year}년 {gz} 세운은 원국이랑 아주 잘 맞는 해거든. "
                f"해보고 싶었던 것, 미뤄뒀던 것 — 올해 안에 한번 제대로 시도해봐. "
                f"기운이 받쳐주는 해에 움직여야 일이 훨씬 수월해."
            )
        elif score_num >= 60:
            score_story = (
                f"올해 흐름 참고점 {score_num:.0f}점 — {_sewun_grade} 구간이야. "
                f"{this_year}년 {gz} 세운은 안정적이고 순탄한 편이야. "
                f"드라마틱한 변화보다 준비해온 걸 차분히 실행하기 좋은 흐름이거든. "
                f"꾸준히 하면 쌓이는 해야 — 조급해하지 말고 한 걸음씩 나가봐."
            )
        elif score_num >= 45:
            score_story = (
                f"올해 흐름 참고점 {score_num:.0f}점 — {_sewun_grade} 구간이야. "
                f"{this_year}년 {gz} 세운은 좋은 달도 있고, 좀 버거운 달도 섞여 있어. "
                f"한꺼번에 크게 벌이려 하면 에너지가 분산되거든. "
                f"올해는 선택과 집중 — 중요한 것 하나에만 힘을 쏟는 게 훨씬 나아."
            )
        else:
            score_story = (
                f"올해 흐름 참고점 {score_num:.0f}점 — {_sewun_grade} 구간이야. "
                f"{this_year}년 {gz} 세운은 거스르는 기운이 꽤 있는 해야. "
                f"새로운 걸 크게 벌이기보다 지금 있는 걸 잘 지키는 게 훨씬 현명해. "
                f"힘든 해가 맞지만 — 잘 버티고 나면 다음 해가 한결 편해지거든."
            )
        ...
        if delta:
            try:
                ...
                if d >= 3:
                    delta_story = "10년 큰 흐름보다도 올해 기운이 한층 더 올라와 있어. 대운 흐름보다 올해가 더 좋은 셈이야 — 적극적으로 움직여봐."
                elif d <= -3:
                    delta_story = "10년 큰 흐름보다 올해는 기운이 좀 눌리는 편이야. 대운이 나쁜 게 아니라 올해만 잠깐 숨 고르는 거라고 봐."
            except Exception:
                pass

        note_line = note if note else ""
        full_text = " ".join(filter(None, [score_story, delta_story, note_line]))
        next_hint = ""
        if row_next:
            next_gz    = str(row_next.get("세운", "-"))
            next_score = str(row_next.get("점수", "-"))
            next_hint  = (
                f"<div style='border-top:1px solid #e2e8f0;margin-top:10px;padding-top:8px;"
                f"font-size:12px;color:#a0b4bc;'>"
                f"내년 {int(this_year)+1}년 {next_gz} 세운은 참고점 {next_score}점으로 미리 봐두면 좋아.</div>"
            )

        st.markdown(
            f"<div style='background:#16161e;border-left:4px solid #3730a3;padding:14px 16px;"
            f"border-radius:8px;margin-bottom:12px;font-size:14px;color:#d0d0e8;line-height:1.8;'>"
            f"<div style='font-size:12px;color:#c4c4e8;margin-bottom:6px;'>"
            f"올해 처방전 · {gz} · {this_year}년</div>"
            f"{full_text}{next_hint}</div>",
            unsafe_allow_html=True,
        )
    else:
        st.info(
            "올해의 처방은 생년월일시 자동 산출 모드일 때 표시됩니다. "
    ...
    if row_this and _sewun_chart and gz not in ("-", ""):
        ...
        with st.expander("🩺 올해 세운 깊이 보기 (합·충·용신·체질 분석)", expanded=False):
            # 1. 세운 글자와 용신/기신
            st.markdown(
                f"<div style='background:#16161e;border-left:4px solid #3730a3;padding:12px 14px;"
                f"border-radius:6px;margin-bottom:10px;'>"
                f"<div style='font-size:12px;color:#c4c4e8;font-weight:700;margin-bottom:6px;'>올해 세운 글자 분석 · {gz}</div>"
                f"<div style='font-size:13px;color:#ddd0d8;line-height:1.9;'>"
            ...
            if _harmony_notes or _clash_notes:
                ...
                if _harmony_notes:
                    st.markdown(
                        ...
                        f"<div style='font-size:12px;font-weight:700;color:#4ade80;margin-bottom:4px;'>🔗 합 — 묶이는 기운</div>"
                        f"<div style='font-size:13px;color:#ddd0d8;line-height:1.8;'>"
                        + "<br>".join(f"· {n}" for n in _harmony_notes) +
                        f"<br><span style='font-size:12px;color:#a0b4bc;'>→ 합이 생기면 해당 기운이 변화하거나 묶여서 다른 흐름을 만들어. 좋은 방향이면 기회가, 안 좋은 방향이면 옴짝달싹이 어려울 수 있어.</span>"
                        f"</div></div>",
                        unsafe_allow_html=True,
                    )
                if _clash_notes:
                    st.markdown(
                        f"<div style='background:#1a1408;border-left:3px solid #ea580c;"
                        f"padding:10px 12px;border-radius:6px;margin-bottom:8px;'>"
                        f"<div style='font-size:12px;font-weight:700;color:#fb923c;margin-bottom:4px;'>⚡ 충 — 부딪히는 기운</div>"
                        f"<div style='font-size:13px;color:#ddd0d8;line-height:1.8;'>"
                        + "<br>".join(f"· {n}" for n in _clash_notes) +
                        f"<br><span style='font-size:12px;color:#a0b4bc;'>→ 충은 변화와 자극이야. 불편하게 느껴지지만, 기신 기운을 흔드는 충은 오히려 풀리는 계기가 되기도 해. 무조건 나쁜 신호는 아니야.</span>"
                        f"</div></div>",
                        unsafe_allow_html=True,
                    )
            else:
...
def render_hanuneyo_text_explanation(payload, char, result):
    """사주 진단서 — 일주(일간+일지) 이름 포함 의학 진단서 컨셉."""
    chart   = payload["chart"]
    snap    = _single_front_snapshot(payload)
    useful  = result.get("useful", {}) or {}
    op      = energy_operation_profile(result)
    ...
    # ── 일간·일지 추출 ───────────────────────────────────────
    day_stem   = str(chart.day.stem   if chart else "-")
    day_branch = str(chart.day.branch if chart else "-")

    # ── 천간 이름 (형용사 + 자연물) ─────────────────────────
    STEM_NAMES = {
        "甲": ("곧은 거목",         "소나무처럼 곧게 뻗어 나가는 강한 추진력"),
        "乙": ("유연한 덩굴",        "어떤 환경에도 뿌리 내리는 섬세한 적응력"),
        "丙": ("화끈한 태양",        "환하게 빛나며 주변을 이끄는 강렬한 존재감"),
        "丁": ("따뜻한 촛불",        "고요하지만 꺼지지 않는 집중력과 온기"),
        "戊": ("든든한 산맥",        "흔들리지 않고 묵묵히 버티는 넓은 포용력"),
        "己": ("알뜰한 텃밭",        "꼼꼼하게 가꾸고 실용적으로 결실 맺는 힘"),
        "庚": ("강직한 도끼",        "단호하게 결정하고 강하게 밀어붙이는 카리스마"),
        "辛": ("예민한 보검",        "예리하게 꿰뚫는 감각과 높은 기준"),
        "壬": ("넓은 바다",          "깊고 넉넉하게 담아내는 지혜와 포용"),
        "癸": ("감성적인 이슬비",    "섬세하고 깊은 감수성으로 스며드는 직관"),
    }

    # ── 지지 이름 (오행성·계절성 중심) ──────────────────────
    BRANCH_NAMES = {
        "子": ("맑은 한겨울 샘",     "동지 · 순수한 수기(水氣)의 정점. 차갑고 맑게 고인 물 에너지"),
        "丑": ("얼어붙은 대지",      "겨울 끝 · 차가운 냉토(冷土). 단단하게 굳은 대지 에너지"),
        "寅": ("솟구치는 새벽 나무", "이른 봄 · 강한 양목(陽木). 막 솟아오르는 새벽 기운"),
        "卯": ("무성한 봄 숲",       "한창 봄 · 부드러운 음목(陰木). 가득 피어난 봄의 기운"),
        "辰": ("안개 낀 봄 대지",    "봄 끝 · 축축한 습토(濕土). 물기 머금은 봄 흙 에너지"),
        "巳": ("달아오른 초여름",    "초여름 · 건조한 양화(陽火). 뜨겁게 달아오르는 기운"),
        "午": ("타오르는 한낮",      "하지 · 순수한 화기(火氣)의 정점. 가장 뜨거운 불 에너지"),
        "未": ("메마른 여름 땅",     "여름 끝 · 건조한 조토(燥土). 뜨겁고 건조한 대지 에너지"),
        "申": ("서늘한 가을 바위",   "초가을 · 단단한 양금(陽金). 서늘하게 식어가는 쇠 기운"),
        "酉": ("차가운 가을 보석",   "한가을 · 정제된 음금(陰金). 서리처럼 차갑고 날카로운 기운"),
        "戌": ("황혼의 메마른 땅",   "늦가을 · 건조한 조토(燥土). 황혼처럼 깊어지는 대지 에너지"),
        "亥": ("깊어지는 겨울 물",   "초겨울 · 고요한 음수(陰水). 어둡고 깊어지는 물 에너지"),
    }

    stem_name,   stem_desc   = STEM_NAMES.get(day_stem,   (day_stem,   ""))
    branch_name, branch_desc = BRANCH_NAMES.get(day_branch, (day_branch, ""))

    # ── 진단명 생성 ──────────────────────────────────────────
    def _make_diagnosis(dom_el, wk_el, wk_pct_val, tmp, str_label, s_name, b_name):
        temp_part = (
            "과열성(過熱性)" if tmp >= 73 else
            "열성(熱性)"    if tmp >= 62 else
            "온성(溫性)"    if tmp >= 53 else
            "냉성(冷性)"    if tmp >= 42 else
            "한랭성(寒冷性)"
        )
        el_hanja = {
            "木": "목기(木氣)", "火": "화기(火氣)", "土": "토기(土氣)",
            "金": "금기(金氣)", "水": "수기(水氣)",
        }
        str_part = {
            "극신강": "과항(過亢)", "신강": "항진(亢進)", "중화": "균형(均衡)",
            "신약": "부족(不足)", "극신약": "허약(虛弱)",
        }.get(str_label, "균형(均衡)")
        suffix = (
            "증후군(症候群)" if str_label in ("극신강", "극신약") else
            "기질형(氣質型)" if str_label in ("신강", "신약") else
            "체질형(體質型)"
        )
        wk_part = ""
        if wk_el and wk_el not in ("-", dom_el):
            wk_name = el_hanja.get(wk_el, wk_el)
            if float(wk_pct_val) < 8:   wk_part = f" · {wk_name} 고갈(枯渴)"
            elif float(wk_pct_val) < 16: wk_part = f" · {wk_name} 결핍(缺乏)"
            elif float(wk_pct_val) < 24: wk_part = f" · {wk_name} 저하(低下)"
        return (
            f"{temp_part} · {s_name}({day_stem}) · {b_name}({day_branch})\n"
            f"{el_hanja.get(dom_el, dom_el)} {str_part}{wk_part} {suffix}"
        )

    diagnosis_name = _make_diagnosis(
        dominant_el, weak_el, float(weak_pct),
        temp, strength_label, stem_name, branch_name
    )

    # ── 진단명 쉬운 설명 ─────────────────────────────────────
    def _make_plain(dom_el, wk_el, wk_pct_val, tmp, str_label):
        """진단명을 일상 언어로 풀어준다."""
        temp_story = {
            "과열성": "검진 결과, 불기운이 확실히 강한 과열 체질이야. 몸도 기운도 쉽게 달아오르거든. 열정은 넘치는데 자칫 소진이 빠를 수 있어. 식히는 방향 — 물 기운이나 차가운 환경, 느긋하게 쉬는 시간이 이 체질한테는 보약이야.",
            "열성":   "따뜻한 기운이 강한 체질이야. 어딜 가나 온기를 뿜어내거든. 주변 사람들이 네 옆에 있으면 따뜻하다고 느끼는 경우가 많아. 다만 너무 달아오르면 과열되기 쉬우니, 가끔은 혼자 조용히 쉬는 시간을 챙겨봐.",
            "온성":   "사주 온도가 딱 균형권이야. 차갑지도 뜨겁지도 않은 — 가장 안정적인 체질 중 하나야. 어떤 환경에서도 적응을 잘 하는 편이거든. 이 균형을 유지하는 게 이 체질의 핵심이야.",
            "냉성":   "서늘한 냉성 체질이야. 차분하고 냉정하게 세상을 보는 눈이 있거든. 감정에 휩쓸리지 않고 판단하는 힘이 강한 편이야. 다만 너무 차가워지면 고립되기 쉬우니 — 따뜻한 사람들 곁에 있는 게 이 체질의 처방이야.",
            "한랭성": "검진 결과, 한랭 체질이야. 겨울 새벽처럼 차가운 기운이 강하거든. 깊이 생각하는 힘은 있지만 움직임이 무거워질 수 있어. 따뜻한 음식, 따뜻한 사람, 햇볕 — 이게 이 체질한테 가장 필요한 처방이야. 온기를 적극적으로 채워봐.",
        }
        el_story = {
            "木": ("나무 기운", "새싹이 올라오듯 뻗어나가고 싶어 하는"),
            "火": ("불 기운",   "활활 타오르며 주변을 환하게 밝히는"),
            "土": ("흙 기운",   "대지처럼 묵직하게 자리 잡고 버티는"),
            "金": ("쇠 기운",   "쇳덩이처럼 단단하고 날카롭게 결정하는"),
            "水": ("물 기운",   "물처럼 부드럽게 흘러가며 깊이 생각하는"),
        }
        str_story = {
            "극신강": "타고난 힘이 극도로 강한 편이야. 혼자서도 거뜬히 밀고 나가는 힘이 있거든. 다만 이 강한 힘이 자칫 주변과 충돌을 만들 수 있어 — 힘 빼는 법, 유연하게 받아주는 법을 익히면 이 사주의 힘이 두 배가 돼.",
            "신강":   "타고난 힘이 강한 편이야. 스스로 방향을 잡고 밀어붙이는 뚝심이 있거든. 혼자 개척하는 힘이 있어서 주도적으로 사는 게 이 사주한테 잘 맞아. 지나치게 고집 세게 굴지만 않으면 충분히 잘 해.",
            "중화":   "타고난 힘이 균형 잡혀 있어. 혼자 미는 힘과 도움받는 힘이 적당히 맞아. 어느 상황에서도 잘 적응하는 편이거든. 극단적으로 강하거나 약하지 않으니 — 어떤 환경에 놔도 제 몫을 하는 사주야.",
            "신약":   "타고난 힘이 약한 편이야. 혼자 뚝심으로 밀기보다 좋은 환경, 좋은 사람이 곁에 있을 때 훨씬 잘 피어나거든. 이 사주는 환경을 잘 고르는 게 핵심이야. 힘이 없는 게 아니라 — 발현되는 방식이 다른 거야.",
            "극신약": "타고난 힘이 많이 부족한 편이야. 혼자 돌파하기보다 도움을 잘 받고 잘 활용하는 게 이 사주의 운용 방식이거든. 부끄럽게 생각할 거 없어 — 도움받는 걸 잘 하는 사람이 오히려 더 멀리 가는 경우가 많아.",
        }
        wk_story = {
            "木": "나무 기운이 부족하니, 새로운 걸 시작하는 힘이 좀 달릴 수 있어. 작은 도전부터 하나씩 해봐.",
            "火": "불 기운이 적으니, 표현하고 드러내는 게 쑥스러울 수 있어. 조금씩 소통하는 연습을 해봐.",
            "土": "흙 기운이 부족하니, 중심을 잡기가 쉽지 않을 수 있어. 규칙적인 생활이 참 중요해.",
            "金": "쇠 기운이 적으니, 딱 끊고 결정하는 게 좀 어려울 수 있어. 하나씩 끝내는 습관을 들여봐.",
            "水": "물 기운이 부족하니, 쉬어가는 여유가 좀 필요해. 혼자 조용히 생각하는 시간을 가져봐.",
        }
        t_key = (
            "과열성" if tmp >= 73 else "열성" if tmp >= 62 else
            "온성"   if tmp >= 53 else "냉성" if tmp >= 42 else "한랭성"
        )
        el_name, el_adj = el_story.get(dom_el, (dom_el, ""))
        sentences = [temp_story.get(t_key, "")]
        sentences.append(f"사주 주성분은 {el_adj} {el_name}이야. {str_story.get(str_label, '')}")
        if wk_el and wk_el not in ("-", dom_el):
            wk_pct_f = float(wk_pct_val or 0)
            wk_txt = wk_story.get(wk_el, "")
            if wk_pct_f < 8:
                sentences.append(f"그런데 있잖아, {el_story.get(wk_el, (wk_el,''))[0]}은 거의 없다시피 해. {wk_txt}")
            elif wk_pct_f < 16:
                sentences.append(f"다만 {el_story.get(wk_el, (wk_el,''))[0]}이 좀 부족하네. {wk_txt}")
            elif wk_pct_f < 24:
                sentences.append(f"{el_story.get(wk_el, (wk_el,''))[0]}이 살짝 아쉬운 편이야. {wk_txt}")
        return " ".join(sentences)

    diag_plain = _make_plain(dominant_el, weak_el, float(weak_pct), temp, strength_label)

    # ── 오행 텍스트 사전 ─────────────────────────────────────
    element_nature = {
        "木": ("성장·추진", "마치 봄에 새싹이 올라오듯, 앞으로 뻗어나가고 싶어 하는 기운이야. 새로운 걸 시작하고 도전하는 힘이 강하거든."),
        "火": ("열정·표현", "활활 타오르는 불처럼, 주변을 환하게 밝히는 기운이야. 드러내고 표현하고 이끌어가는 힘이 있어."),
        "土": ("안정·조율", "땅이 만물을 품듯, 묵직하게 자리 잡고 버티는 기운이야. 중심을 잡고 균형을 맞추는 힘이 강하거든."),
        "金": ("결단·정밀", "쇳덩이처럼 단단하고 날카로운 기운이야. 기준을 딱 세우고 확실하게 끝내는 힘이 있어."),
        "水": ("지혜·유연", "물이 흘러가듯, 부드럽게 스며드는 기운이야. 상황에 따라 유연하게 적응하고 깊이 생각하는 힘이 강하거든."),
    }
    element_weak_note = {
        "木": "새로운 걸 시작하거나 밀고 나가는 힘이 좀 달릴 수 있어. 작은 도전을 하나씩 늘려가다 보면 점점 살아나.",
        "火": "자기 자신을 드러내거나 표현하는 게 좀 쑥스럽게 느껴질 수 있어. 조금씩 소통하는 연습을 하다 보면 나아져.",
        "土": "중심 잡기가 어렵거나 불안한 느낌이 들 수 있어. 규칙적인 생활 루틴이 아주 큰 도움이 돼.",
        "金": "딱 결정하고 끝내는 게 쉽지 않을 수 있어. 하나를 끝까지 마무리하는 습관을 들이면 훨씬 편해져.",
        "水": "쉬어가는 여유나 깊이 생각하는 시간이 부족할 수 있어. 혼자 조용히 앉아있는 시간을 꼭 챙겨.",
    }
    ...
    axis_high = {
        ...
        "흐름과 연결": "사람·일·생각이 자연스럽게 연결되는 편이야. 막히지 않고 잘 흘러가는 사주거든.",
        "현실작동력": "생각한 걸 행동으로 옮기는 힘이 살아 있어. 아이디어가 헛되지 않고 실제로 드러나는 편이야.",
    }
    axis_low = {
        "기초체력": "재료가 좀 부족한 편이야. 무리하게 달리면 금방 바닥날 수 있으니, 쉬엄쉬엄 가는 게 중요해.",
        "흐름과 연결": "가끔 막히고 끊기는 느낌이 들 수 있어. 내 편이 되어주는 사람, 내 흐름을 도와주는 환경이 아주 중요하거든.",
        "현실작동력": "좋은 생각이 있어도 실제로 드러나는 데 계기가 필요한 팔자야. 아주 작은 것 하나부터 먼저 시작해봐.",
    }
    strength_plain = {
        "극신약": "타고난 힘이 많이 약한 편이야. 좋은 사람, 좋은 환경이 곁에 있을 때 진짜 빛나거든. 도움을 잘 받는 게 이 사주의 운용 방식이야.",
        "신약":   "주변이 잘 받쳐줄 때 훨씬 잘 피어나는 사주야. 좋은 인연과 좋은 자리가 중요하거든. 혼자 버티기보다 손 내밀 줄 알아야 해.",
        "중화":   "혼자 미는 힘과 도움받는 힘이 균형 잡혀 있어. 상황 봐가며 유연하게 조율하는 게 이 사주의 특기야.",
        "신강":   "스스로 방향 잡고 밀어붙이는 힘이 강해. 너무 혼자 다 끌고 가면 지칠 수 있으니, 가끔은 멈추고 주변도 돌아봐.",
        "극신강": "타고난 힘이 아주 강한 편이야. 밀어붙이는 힘은 대단한데 — 방향을 잘 잡는 게 중요해. 에너지가 엉뚱한 데 쏠리지 않도록 출구를 잘 만들어줘야 해.",
    }
    ...
    def _make_grandma_intro() -> str:
        ...
        _name = str(payload.get("name", "이 사주") or "이 사주")
        _safe = html.escape(_name)

        # 1단락: 체질·온도 설명
        _temp_para = {
            "과열성": (
                f"{_safe} 사주, 첫인상부터 말할게. 불기운이 확실히 강해 — 과열 체질이야. "
                "몸도 기운도 쉽게 달아오르고, 열정이 넘쳐서 주변 사람들이 같이 뜨거워지거든. "
                "근데 이게 장점이기도 하고 단점이기도 해. 열정적으로 달려가는 건 좋은데, "
                "자칫 과소진이 빠른 구조야. 식히고 정리하는 시간이 이 사주한테는 진짜 보약이야 — "
                "가끔은 쉬어가야 오래 잘 달릴 수 있어."
            ),
            "열성": (
                f"{_safe} 사주는 따뜻한 기운이 강해. 열성 체질이야. "
                "어딜 가나 온기를 뿜어내고, 주변 사람들이 네 옆에 있으면 따뜻하다고 느끼거든. "
                "에너지가 살아 있고 표현력도 좋아. 다만 너무 달아오르면 과열되기 쉬우니까, "
                "중간중간 혼자 조용히 쉬는 시간을 챙기는 게 중요해."
            ),
            "온성": (
                f"{_safe} 사주는 온도가 딱 균형권이야. 온성 체질이라고 봐. "
                "차갑지도 뜨겁지도 않은 — 사실 이게 가장 안정적인 체질 중 하나야. "
                "어떤 환경에서도 적응을 잘 하고, 극단으로 치닫는 일이 적은 편이거든. "
                "이 균형을 유지하는 게 핵심이야. 다만 조후 흐름, 즉 계절 에너지에 따라 "
                "체감이 달라질 수 있으니까 환경 선택을 잘 하면 돼."
            ),
            "냉성": (
                f"{_safe} 사주는 서늘한 냉성 체질이야. "
                "차분하고 냉정하게 세상을 보는 눈이 있거든 — 감정에 휩쓸리지 않고 "
                "판단하는 힘이 강한 편이야. 이게 이 사주의 큰 강점이야. "
                "다만 너무 차가워지면 고립되기 쉬워. 따뜻한 사람들 곁에 의식적으로 있는 게 "
                "이 체질의 처방이야 — 온기는 환경에서 보충하는 거야."
            ),
            "한랭성": (
                f"{_safe} 사주는 한랭 체질이야. 겨울 새벽처럼 차가운 기운이 강하거든. "
                "깊이 생각하는 힘은 정말 좋아. 근데 움직임이 무거워지거나 "
                "감정이 얼어붙는 느낌이 들 때가 있어. 따뜻한 음식, 따뜻한 사람, 햇볕 — "
                "이게 이 체질한테 가장 필요한 처방이야. 온기를 적극적으로 채워봐."
            ),
        }
        _t_key = (
            "과열성" if temp >= 73 else
        ...
        # 2단락: 오행 주성분 + 강약
        _el_para = {
            "木": "이 사주의 주성분은 나무 기운(木)이야. 새싹이 땅을 뚫고 올라오듯 앞으로 뻗어나가려는 힘이 강하거든. 새로운 걸 시작하고 도전하는 에너지가 있어.",
            "火": "이 사주의 주성분은 불 기운(火)이야. 활활 타오르면서 주변을 환하게 밝히는 힘이거든. 드러내고 표현하고 이끌어가는 에너지가 있어.",
            "土": "이 사주의 주성분은 흙 기운(土)이야. 대지처럼 묵직하게 자리 잡고 버티는 힘이거든. 중심을 잡고 균형을 맞추는 에너지가 있어.",
            "金": "이 사주의 주성분은 쇠 기운(金)이야. 쇳덩이처럼 단단하고 날카롭게 결정하는 힘이거든. 기준을 딱 세우고 확실하게 끝내는 에너지가 있어.",
            "水": "이 사주의 주성분은 물 기운(水)이야. 물이 흘러가듯 부드럽게 스며드는 힘이거든. 유연하게 적응하고 깊이 생각하는 에너지가 있어.",
        }
        _str_para = {
            "극신강": (
                "타고난 힘이 극도로 강한 사주야. 혼자서도 거뜬히 밀고 나가는 힘이 있거든. "
                "근데 이 강한 힘이 자칫 주변과 충돌을 만들 수 있어. "
                "힘 빼는 법, 유연하게 받아주는 법을 익히면 이 사주의 힘이 두 배가 돼."
            ),
            "신강": (
                "타고난 힘이 강한 사주야. 스스로 방향을 잡고 밀어붙이는 뚝심이 있거든. "
                "혼자 개척하는 힘이 있어서 주도적으로 사는 게 잘 맞아. "
                "지나치게 고집 세게 굴지만 않으면 충분히 잘 해."
            ),
            "중화": (
                "타고난 힘이 균형 잡혀 있어. 혼자 미는 힘과 도움받는 힘이 적당히 맞아. "
                "어느 상황에서도 잘 적응하는 편이거든. 어떤 환경에 놔도 제 몫을 하는 사주야."
            ),
            "신약": (
                "타고난 힘이 약한 편이야. 혼자 뚝심으로 밀기보다 "
                "좋은 환경, 좋은 사람이 곁에 있을 때 훨씬 잘 피어나거든. "
                "이 사주는 환경을 잘 고르는 게 핵심이야. 힘이 없는 게 아니야 — 발현 방식이 다른 거야."
            ),
            "극신약": (
                "타고난 힘이 많이 부족한 편이야. 혼자 돌파하기보다 "
                "도움을 잘 받고 잘 활용하는 게 이 사주의 운용 방식이야. "
                "도움받는 걸 잘 하는 사람이 오히려 더 멀리 가는 경우가 많아 — 부끄럽게 생각할 거 없어."
            ),
        }
        _el_txt = _el_para.get(str(dominant_el), f"주성분은 {dominant_el} 기운이야.")
        _str_txt = _str_para.get(str(strength_label), "")
        p2 = _el_txt
        if _str_txt:
            p2 += f" {_str_txt}"
        # 약한 오행 추가
        _wk_story = {
            "木": "나무 기운이 좀 부족하니까, 새로운 걸 시작하거나 밀고 나가는 힘이 달릴 수 있어. 작은 도전을 하나씩 늘려가봐.",
            "火": "불 기운이 좀 적으니까, 자기 자신을 드러내거나 표현하는 게 쑥스러울 수 있어. 조금씩 소통하는 연습을 해봐.",
            "土": "흙 기운이 부족하니까, 중심 잡기가 어렵거나 불안한 느낌이 들 수 있어. 규칙적인 생활 루틴이 큰 도움이 돼.",
            "金": "쇠 기운이 좀 적으니까, 딱 결정하고 끝내는 게 쉽지 않을 수 있어. 하나를 끝까지 마무리하는 습관이 중요해.",
            "水": "물 기운이 부족하니까, 쉬어가는 여유나 깊이 생각하는 시간이 부족할 수 있어. 혼자 조용히 앉아 있는 시간을 꼭 챙겨.",
        }
        if weak_el and str(weak_el) not in ("-", str(dominant_el)):
            _wk_f = float(weak_pct or 0)
            _wk_prefix = (
                "있잖아, 아쉽게도" if _wk_f < 8 else
                "그리고" if _wk_f < 16 else
                "한 가지만 더"
            )
            _wk_txt = _wk_story.get(str(weak_el), "")
            if _wk_txt:
                p2 += f"<br>{_wk_prefix} {_wk_txt}"

        # 3단락: 용신/기신 + 처방
        _fw_primary_list = list((useful.get("primary") or []))
        _fw_burden_list  = list((useful.get("burden")  or []))
        _EL_KO = {"木": "나무(木)", "火": "불(火)", "土": "흙(土)", "金": "쇠(金)", "水": "물(水)"}
        _primary_ko = "·".join(_EL_KO.get(e, e) for e in _fw_primary_list) if _fw_primary_list else ""
        _burden_ko  = "·".join(_EL_KO.get(e, e) for e in _fw_burden_list)  if _fw_burden_list  else ""
        _SEASON_KO = {"木": "봄", "火": "여름", "土": "환절기", "金": "가을", "水": "겨울"}
        _COLOR_KO  = {"木": "초록·파랑", "火": "빨강·주황", "土": "노랑·갈색", "金": "흰색·회색", "水": "검정·진파랑"}
        _DIR_KO    = {"木": "동쪽", "火": "남쪽", "土": "중앙", "金": "서쪽", "水": "북쪽"}
        _season_hint = "·".join(_SEASON_KO.get(e, "") for e in _fw_primary_list if _SEASON_KO.get(e))
        _color_hint  = "·".join(_COLOR_KO.get(e, "")  for e in _fw_primary_list if _COLOR_KO.get(e))
        p3 = ""
        if _primary_ko:
            p3 = (
                f"이 사주한테 도움이 되는 기운은 <b>{_primary_ko}</b>야. "
                f"{'이 기운이 강한 ' + _season_hint + ' 계절, ' if _season_hint else ''}"
                f"{'색깔로는 ' + _color_hint + ', ' if _color_hint else ''}"
                "이쪽 방향의 환경이나 사람 곁에 있으면 체감이 좋아지거든. "
            )
            if _burden_ko:
                p3 += (
                    f"반대로 <b>{_burden_ko}</b> 기운이 넘치면 원국이 버거워져. "
                    "기신이라고 무조건 나쁜 건 아니야 — 이 방향이 강해지는 시기엔 "
                    "무리하지 않고 아끼면서 가는 게 현명해."
                )
        elif "중화" in str(strength_label):
            p3 = (
                "이 사주는 강약보다 흐름과 온도 균형이 더 중요해. "
                "어떤 환경에서 어떤 사람과 함께하느냐가 핵심이거든. "
    ...
    _score_label = (
        "매우 좋음" if _total_score >= 80 else
        "좋은 편"   if _total_score >= 65 else
        "보통"      if _total_score >= 50 else
    ...
    def _row(label, value, value_color="#f0e0ea"):
        return (
            ...
            f"color:#c8a0b8;border:1px solid rgba(120,120,220,0.18);white-space:nowrap;'>{label}</td>"
            f"<td style='padding:9px 12px;font-size:14px;font-weight:600;"
            f"color:{value_color};border:1px solid rgba(120,120,220,0.18);line-height:1.7;'>{value}</td>"
            f"</tr>"
        )
    ...
    # 진단서 HTML
    _cert_html = f"""
<div style='background:linear-gradient(160deg,#16161e 0%,#1a0d18 100%);
            border:2px solid rgba(120,120,220,0.45);
  ...
  <div style='background:linear-gradient(90deg,rgba(123,30,61,0.85),rgba(42,21,32,0.9));
              padding:14px 20px;border-bottom:1px solid rgba(120,120,220,0.30);
  ...
  <div style='padding:4px 16px 16px 16px;'>
    ...
    <div style='margin-top:14px;border-top:1px solid rgba(120,120,220,0.18);padding-top:12px;'>
...
"""
    ...
    <div style='margin-top:12px;border-top:1px solid rgba(120,120,220,0.18);padding-top:10px;'>
    ...
    <div style='margin-top:10px;border-top:1px solid rgba(120,120,220,0.18);padding-top:10px;'>
...
"""
    ...
    if _grandma_intro_html:
        st.html(
            f"<div style='background:#16161e;border-left:4px solid #818cf8;"
    ...
    _tc, _tbg = _thermo_colors.get(_t_key, ("#94a3b8", "#16161e"))
    ...
    for _an, _pct, _st in _axis_results:
        _sc, _sbg = _state_colors.get(_st, ("#94a3b8", "#16161e"))
    ...
    _ELEMENT_RX = {
        "木": {
            "color": "🟢 초록·파랑", "direction": "동쪽",
            "food": "신맛 (식초·레몬·사과)",
            "activity": "산책·등산·새 프로젝트 시작",
            "tip": "아침에 일찍 일어나 햇빛을 받으며 조금씩 움직이는 것이 도움이 돼.",
        },
        "火": {
            "color": "🔴 빨강·주홍", "direction": "남쪽",
            "food": "쓴맛 (커피·녹차·쑥)",
            "activity": "사람 만나기·발표·야외 운동",
            "tip": "밝은 공간에서 사람들과 어울리고, 하고 싶은 말을 표현하는 연습이 큰 도움이 돼.",
        },
        "土": {
            "color": "🟡 황토·노랑", "direction": "중앙·사방",
            "food": "단맛 (고구마·곡물·꿀)",
            "activity": "규칙적 생활·명상·정원 가꾸기",
            "tip": "매일 같은 시간에 자고 일어나는 것처럼 규칙적인 루틴이 중심을 잡아줘.",
        },
        "金": {
            "color": "⚪ 흰색·금색", "direction": "서쪽",
            "food": "매운맛 (마늘·생강·고추)",
            "activity": "정리정돈·결정하기·마감 있는 목표 세우기",
            "tip": "하루에 하나씩, 작은 것부터 확실히 끝내는 습관이 쌓이면 에너지가 살아나.",
        },
        "水": {
            "color": "🔵 검정·남색", "direction": "북쪽",
            "food": "짠맛 (해산물·된장·미역)",
            "activity": "혼자 쉬기·독서·수영·글쓰기",
            "tip": "하루에 잠깐이라도 혼자 조용히 앉아 생각하는 시간을 챙기면 기운이 회복돼.",
        },
    }
    _TEMP_RX = {
        "과열성": "🌊 찬 기운으로 식히기 — 수영이나 물 관련 활동, 서늘한 공간, 차가운 음식이 도움이 돼. 마음도 한 박자 쉬어가는 연습을 해봐.",
        "열성":   "💧 물 자주 마시고, 너무 흥분하거나 달아오르는 상황을 조금씩 조율해봐. 서늘한 환경이 균형을 잡아줘.",
        "온성":   "✅ 지금 온도가 딱 좋아. 현재 생활 패턴을 유지하면서 급격하게 바꾸지 않는 게 좋아.",
        "냉성":   "☀️ 따뜻한 기운 채우기 — 햇빛 쬐기, 따뜻한 음식, 사람들과 자주 어울리는 게 기운을 돋워줘.",
        "한랭성": "🔥 온기 보충이 급선무야 — 뜨거운 음식과 음료, 밝고 따뜻한 공간, 활동적인 사람들과 함께하는 시간이 정말 중요해.",
    }
    # 보충 처방
    _primary_els = display_dirs.get("primary", [])
    if _primary_els:
        _rx_el = str(_primary_els[0])
        _rx = _ELEMENT_RX.get(_rx_el, {})
        if _rx:
            st.markdown(
                f"<div style='background:#0d1e14;border-left:4px solid #4ade80;"
                f"padding:12px 14px;border-radius:6px;margin-bottom:8px;'>"
                f"<div style='font-size:12px;font-weight:700;color:#4ade80;margin-bottom:6px;'>"
                f"💊 보충 처방 — {_rx_el}기운 채우기</div>"
                f"<div style='font-size:13px;color:#ddd0d8;line-height:1.9;'>"
                f"· 색상 / 방향: {_rx['color']} / {_rx['direction']}<br>"
                f"· 음식: {_rx['food']}<br>"
                f"· 활동: {_rx['activity']}<br>"
                f"· 생활 팁: {_rx['tip']}"
                f"</div></div>",
                unsafe_allow_html=True,
            )
    ...
    if _t_rx:
        st.markdown(
            f"<div style='background:#1a1408;border-left:4px solid #f59e0b;"
            f"padding:12px 14px;border-radius:6px;margin-bottom:8px;'>"
            f"<div style='font-size:12px;font-weight:700;color:#d97706;margin-bottom:6px;'>"
            f"🌡️ 체온 처방 — {_t_key} 조절</div>"
            f"<div style='font-size:13px;color:#ddd0d8;line-height:1.8;'>{_t_rx}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    if _sh_hits:
        ...
        _SH_TONE = {
            ...
            "도화":     ("🩷", "중립 신호", "#1a0d18", "#818cf8"),
            "화개":     ("🟣", "중립 신호", "#14141e", "#7c3aed"),
            "공망":     ("⚪", "주의 신호", "#16161e", "#94a3b8"),
        }
        # 신살별 의사 텍스트
        _SH_DOC_TEXT = {
        ...
        for _hit in _sh_hits:
            ...
            st.markdown(
                ...
                f"<div style='font-size:13px;color:#ddd0d8;line-height:1.7;margin-bottom:4px;'>"
                f"{_summary}</div>"
        ...
        st.markdown(
            "<div style='background:#16161e;border:1px solid #3a2830;border-radius:8px;"
    ...
    with st.expander("🔍 기운의 흐름과 순환 구조 상세 보기", expanded=False):
        ...
        for _tn, _tm, _tv, _tlevel, _tlabel in _sorted_ten:
            ...
            _badge_bg = {"strong": "#3730a3", "mid": "#2a3a4a", "weak": "#1a2a1a"}.get(_tlevel, "#2a2a3a")
        ...
        st.markdown(
            f"<div style='background:#0d1828;border-left:4px solid #60a5fa;padding:12px 14px;"
            f"border-radius:6px;margin-bottom:10px;'>"
        ...
        st.markdown(
            f"<div style='background:#1e1820;border:1px solid #3a2830;border-radius:8px;"
            f"padding:11px 14px;margin-bottom:10px;'>"
            f"<div style='font-size:12px;color:#a0b4bc;font-weight:700;margin-bottom:5px;'>💡 용신·기신이 뭔지 알고 보면 달라</div>"
            f"<div style='font-size:13px;color:#ddd0d8;line-height:1.95;'>{_YONG_CONCEPT}</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
        if _logic_explain:
            st.markdown(
                f"<div style='background:#1a0d28;border-left:4px solid #c898f8;padding:10px 14px;"
                f"border-radius:6px;margin-bottom:10px;'>"
                f"<div style='font-size:12px;color:#7c3aed;font-weight:700;margin-bottom:4px;'>🧩 이 사주의 용신 선정 원리</div>"
                f"<div style='font-size:13px;color:#ddd0d8;line-height:1.95;'>{_logic_explain}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
        if _fw_primary:
            ...
            st.markdown(
                f"<div style='background:#0d1e14;border-left:4px solid #4ade80;padding:10px 14px;"
                f"border-radius:6px;margin-bottom:8px;'>"
                f"<div style='font-size:12px;color:#4ade80;font-weight:700;margin-bottom:4px;'>💚 용신 / 희신 (도움이 되는 기운)</div>"
                f"<div style='font-size:13px;color:#ddd0d8;line-height:1.9;'>"
                f"{_primary_items}"
                f"<span style='font-size:12px;color:#4ade80;line-height:1.9;'>"
        ...
        if _fw_burden:
            ...
            st.markdown(
                f"<div style='background:#200d0d;border-left:4px solid #f87171;padding:10px 14px;"
                f"border-radius:6px;margin-bottom:8px;'>"
        ...
        if _fw_bindings:
            st.markdown(
                "<div style='background:#0d1e14;border-left:4px solid #4ade80;padding:10px 14px;"
        ...
        if _fw_clashes:
            st.markdown(
                "<div style='background:#1a1408;border-left:4px solid #ea580c;padding:10px 14px;"
                "border-radius:6px;margin-bottom:8px;'>"
        ...
        for _fw in _fw_flows:
            ...
            if not _doc or _fn == "식신제살/상관패인":
                continue
            _fc, _fbg = _STRENGTH_COLOR.get(_fs, ("#6b7280", "#16161e"))
            # 원국 근거 구성
            _fsrc_items = []
            ...
            _fsrc_html = (
                "<div style='background:#14141c;border-left:2px solid rgba(244,114,182,.3);"
...
def render_single_summary(payload: Dict[str, object]) -> None:
    ...
    _pillar_html_cells = "".join(
        f"<div style='text-align:center;'><div style='font-size:12px;color:#c4c4e8;margin-bottom:2px;'>{_labels[i]}</div>"
        f"<div style='font-size:24px;font-weight:900;letter-spacing:2px;color:#d0d0e8;'>{p}</div></div>"
        for i, p in enumerate(_parts)
    )
    st.markdown(
        f"<div style='background:#16161e;border:2px solid #9898b8;border-radius:10px;"
    ...
    if view == "🏥 사주 진단서":
        ...
        expert_view = st.radio(
            "상세 항목",
            ["기운 정밀 분석", "운의 흐름 상세", "상세 명리 분석", "계산 참고 지표"],
            index=0,
            horizontal=True,
    ...
    for row in rows:
        ...
        <div class="ability-bar-row">
            <div class="ability-bar-head">
                <span class="ability-bar-title">{info['icon']} {info['primary']} <span style="font-size:.88rem;color:#9898b8;">({info['legacy']})</span></span>
                <span class="ability-bar-score">{score:.1f} / {max_score:.0f}</span>
            </div>
            <div style="font-size:.82rem;color:#9898b8;font-weight:760;margin-bottom:.32rem;">{info['short']} · {info['meaning']}</div>
...
def render_score_formula_diagram(result: Dict[str, object], prefix: str = "single") -> None:
    # v5.150: synced 값 사용 → 3대 능력치 합 = 총점
    ...
    st.markdown(f"""
    <div style="background:#16161e;border:1px solid rgba(140,140,210,0.45);border-radius:18px;padding:1rem 1rem 1.1rem 1rem;margin:0.35rem 0 1rem 0;">
        <div style="font-weight:900;color:#9898b8;font-size:1.04rem;margin-bottom:0.7rem;">총점 계산 한눈에 보기</div>
        <div style="display:flex;flex-wrap:wrap;align-items:center;gap:0.55rem;">
            <div style="flex:1 1 170px;background:#16161e;border:1px solid #f2ced8;border-radius:14px;padding:0.8rem;text-align:center;"><div style="font-size:1.02rem;color:#9898b8;font-weight:950;">기초체력</div><div style="font-size:.88rem;color:#9898b8;font-weight:800;">(기운의 크기)</div><div style="font-size:1.35rem;color:#d95b84;font-weight:900;">{size_score:.1f}</div><div style="font-size:.88rem;color:#9898b8;">오행·월령·통근·일간</div></div>
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">+</div>
            <div style="flex:1 1 170px;background:#16161e;border:1px solid #f2ced8;border-radius:14px;padding:0.8rem;text-align:center;"><div style="font-size:1.02rem;color:#9898b8;font-weight:950;">흐름과 연결</div><div style="font-size:.88rem;color:#9898b8;font-weight:800;"></div><div style="font-size:1.35rem;color:#d95b84;font-weight:900;">{flow_score:.1f}</div><div style="font-size:.88rem;color:#9898b8;">조후·십성 흐름</div></div>
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">+</div>
            <div style="flex:1 1 170px;background:#16161e;border:1px solid #f2ced8;border-radius:14px;padding:0.8rem;text-align:center;"><div style="font-size:1.02rem;color:#9898b8;font-weight:950;">현실작동력</div><div style="font-size:.88rem;color:#9898b8;font-weight:800;">(기운의 발현)</div><div style="font-size:1.35rem;color:#d95b84;font-weight:900;">{expr_score:.1f}</div><div style="font-size:.88rem;color:#9898b8;">합충·현실 작동</div></div>
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">+</div>
            <div style="flex:1 1 150px;background:#16161e;border:1px solid #f2ced8;border-radius:14px;padding:0.8rem;text-align:center;"><div style="font-size:0.94rem;color:#9898b8;font-weight:950;">신살·공망</div><div style="font-size:1.18rem;color:#d95b84;font-weight:900;">{shinsal:+.1f}</div><div style="font-size:.88rem;color:#9898b8;">체감 보정</div></div>
            <div style="font-size:1.2rem;font-weight:900;color:#b98195;">=</div>
            <div style="flex:1 1 180px;background:linear-gradient(135deg,#fff0f5,#fffafc);border:2px solid #ebb2c4;border-radius:16px;padding:0.85rem;text-align:center;"><div style="font-size:0.92rem;color:#9898b8;font-weight:800;">대표 총점</div><div style="font-size:1.6rem;color:#3730a3;font-weight:950;">{total:.1f}점</div></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_origin_identity_table(chart: Chart) -> None:
    ...
    def _hidden_lines(p: Pillar) -> str:
        ...
        for hs, ratio in BRANCHES[p.branch]["hidden"]:
            ...
            lines.append(f"{hs} {role}<br><span style='font-size:.66em;color:#9898b8;'>{ko} · {pct}%</span>")
    ...
    def _pillar_col(label: str, p: Pillar | None, highlight: bool = False) -> str:
        if p is None:
            return (
                ...
                "<div class='manse-big stem' style='background:#0d1828;color:#9898b8!important;'>?</div>"
                "<div class='manse-ko'>시주 미상</div>"
                "<div class='manse-role'>-</div>"
                "<div class='manse-big branch' style='background:#0d1828;color:#9898b8!important;'>?</div>"
...
def _mini_bar_html(label: str, value: float, max_value: float = 100.0, caption: str = "") -> str:
    ...
    caption_html = f"<div style='font-size:.88rem;color:#9898b8;margin-top:.18rem;'>{html.escape(str(caption))}</div>" if caption else ""
    ...
    return (
        ...
        f"<div style='font-weight:900;color:#9898b8;'>{html.escape(str(label))}</div>"
        f"<div style='font-weight:900;color:#818cf8;'>{value_label}</div>"
        f"</div>"
        f"<div style='height:10px;background:#221018;border-radius:99px;overflow:hidden;border:1px solid rgba(140,140,210,.35);'>"
...
def _compatibility_polygon_svg(axis_data: List[Tuple[str, float, str]]) -> str:
    ...
    for idx, ((label, val, _caption), a) in enumerate(zip(axis_data, angles)):
        ...
        if idx == 0:
            ...
            ly += 5
        labels.append(
            f"<text x='{lx:.1f}' y='{ly:.1f}' text-anchor='{anchor}' dominant-baseline='middle' font-size='13' font-weight='900' fill='#9898b8'>"
            f"{html.escape(label)}</text>"
        )

    poly = f"<polygon points='{' '.join(points)}' fill='rgba(99,102,241,.22)' stroke='#3730a3' stroke-width='3'/>"
    dots = ""
    for pnt in points:
        x, y = pnt.split(",")
        dots += f"<circle cx='{x}' cy='{y}' r='4.5' fill='#3730a3'/>"
...
def render_element_distribution_gauge(result: Dict[str, object]) -> None:
    ...
    st.markdown("<div style='background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:18px;padding:.9rem 1rem;'>" + "".join(html_rows) + "</div>", unsafe_allow_html=True)
...
def render_strength_scale(result: Dict[str, object]) -> None:
    ...
    st.markdown(f"""
    <div style="background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:18px;padding:1rem;margin:.35rem 0;">
        <div style="display:flex;justify-content:space-between;font-weight:900;color:#9898b8;"><span>신약</span><span>중화</span><span>신강</span></div>
        <div style="position:relative;height:18px;background:linear-gradient(90deg,#c7ddff,#fff4c7,#ffc6d9);border-radius:99px;margin:.7rem 0 .45rem 0;border:1px solid rgba(145,115,129,.20);">
            <div style="position:absolute;left:{pos:.1f}%;top:50%;transform:translate(-50%,-50%);width:22px;height:22px;border-radius:50%;background:#3730a3;border:3px solid #fff;box-shadow:0 3px 8px rgba(0,0,0,.18);"></div>
        </div>
        <div style="font-weight:900;color:#818cf8;">{label} · 강약 지수 {idx:.1f}</div>
    </div>
    """, unsafe_allow_html=True)
...
def render_climate_gauge(result: Dict[str, object]) -> None:
    ...
    st.markdown("#### 🌡️ 조후 감각")
    climate_html = (
        "<div style='background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:18px;padding:.9rem 1rem;'>"
...
def render_useful_signal(useful: Dict[str, object], result: Dict[str, object] | None = None) -> None:
    ...
    st.markdown("<div style='display:flex;flex-wrap:wrap;gap:.65rem;'>" + "".join(
        f"<div style='flex:1 1 180px;background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:16px;padding:.82rem;'><div style='font-weight:950;color:#9898b8;'>{html.escape(k)}</div><div style='color:#c090a8;font-weight:800;margin-top:.35rem;'>{html.escape(v)}</div></div>" for k, v in rows
...
def render_interaction_signal(result: Dict[str, object]) -> None:
    ...
    html_block = "<div style='background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:18px;padding:.9rem 1rem;'>"
    ...
    if detail_html:
        with st.expander("📖 이 사주에서 잡힌 합·충·형, 어떻게 이루어지고 어떤 의미인지 보기", expanded=False):
            st.markdown(
...
def render_luck_timeline(luck_flow: Dict[str, object] | None) -> None:
    ...
    for r in rows:
        ...
        cards.append(f"<div style='flex:1 1 120px;background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:16px;padding:.82rem;'><div style='font-weight:950;color:#9898b8;'>{safe_year}</div><div style='font-weight:900;color:#3730a3;margin:.25rem 0;'>{safe_tag}</div><div style='height:8px;background:#221018;border-radius:99px;overflow:hidden;'><div style='height:100%;width:{pct:.1f}%;background:linear-gradient(90deg,#f8b3ca,#bda7ff);'></div></div><div style='font-size:.88rem;color:#9898b8;margin-top:.25rem;'>{safe_sewun} · {score:.1f}점</div></div>")
...
def render_compatibility_axis_gauge(compatibility: Dict[str, object]) -> None:
    ...
    html_block = "<div style='background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:18px;padding:.9rem 1rem;margin:.5rem 0;'>"
    ...
    PATCH_NOTES = [
        ...
        },
        {
            ...
            "items": [
                ...
                ("🩺 진단서 텍스트 강화", "체질·신살·처방전 설명이 4~5문장으로 확장되어 각 항목의 이유까지 풀어줍니다."),
                ("🔍 올해 세운 상세 분석 추가", "올해 세운 글자의 용신·기신 여부와 합·충 관계를 깊이 보기 메뉴에서 확인할 수 있습니다."),
                ("⚖️ 사주 흐름 구조 설명 추가", "신약·신강 판단 근거, 용신·기신·희신, 식상생재·재생관·관인상생 작동 여부를 항목별로 설명합니다."),
                ("📸 공유 이미지 개편", "공유 카드가 4기둥 8글자 중심으로 재구성되었습니다."),
                ("✨ 말투 개선", "전문 용어를 쉬운 표현으로 교체하여 가독성을 높였습니다."),
                ("📊 점수 표기 추가", "진단서·처방전 각 항목에 점수가 표시됩니다."),
                ("✨ 신살 카드 추가", "천을귀인·문창귀인·역마·도화·화개·공망 신살이 카드 형태로 표시됩니다."),
                ("⚠️ 겁재·형살 오늘 기운 반영", "오늘 기운 화면에서 겁재·형살 신호 감지 시 주의 표시와 설명이 추가됩니다."),
            ],
        },
        {
            "version": "v5.141",
        ...
        },
        {
            ...
            "tag_color": "#7c3aed",
            "items": [
                ("🎨 사주풍경화 실험", "AI 기반 사주 이미지 생성 기능을 실험적으로 추가했습니다. (현재 안정판에서 제외)"),
                ("📈 점수 모델 개선", "3대 능력치(기초체력/흐름과연결/현실작동력) 배분 구조를 35/30/35로 조정했습니다."),
            ],
        },
    ]

    for note in PATCH_NOTES:
        tag_html = f"<span style='background:{note['tag_color']};color:#fff;font-size:.84rem;font-weight:900;padding:.18rem .55rem;border-radius:99px;margin-left:.5rem;'>{note['tag']}</span>"
        st.markdown(
            f"<div style='background:#16161e;border:1.5px solid rgba(99,102,241,.18);border-radius:16px;padding:1rem 1.2rem;margin:.65rem 0;box-shadow:0 4px 12px rgba(99,102,241,.07);'>"
            f"<div style='display:flex;align-items:center;gap:.35rem;margin-bottom:.55rem;'>"
            f"<span style='font-size:1.05rem;font-weight:950;color:#d0d0e8;'>{note['version']}</span>"
            f"{tag_html}"
            f"<span style='margin-left:auto;font-size:.88rem;color:#9b7280;'>{note['date']}</span>"
            f"</div>",
            unsafe_allow_html=True,
        )
        for icon_title, desc in note["items"]:
            st.markdown(
                ...
                f"<span style='font-weight:900;color:#818cf8;font-size:.88rem;'>{icon_title}</span> "
                f"<span style='font-size:.85rem;color:#9898b8;'>{desc}</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
...
def render_calculation_steps() -> None:
    ...
    for i, step in enumerate(steps, start=1):
        html_block += f"<div style='flex:1 1 160px;background:#16161e;border:1px solid rgba(140,140,210,.28);border-radius:14px;padding:.72rem;'><div style='font-weight:950;color:#3730a3;'>STEP {i}</div><div style='font-weight:850;color:#9898b8;'>{html.escape(step)}</div></div>"
    html_block += "</div>"
    st.markdown(html_block, unsafe_allow_html=True)
...
def render_detail_tabs(payload: Dict[str, object], prefix: str | None = None) -> None:
    ...
    if selected_detail == "핵심 요약":
        quest_title("🧭 핵심 요약", "참고점이 어떻게 만들어졌는지부터 원국의 캐릭터와 흐름까지 순서대로 봅니다.", ["총점", "원국", "캐릭터", "운의 파도"])
        ...
        if ability_view == "핵심 요약":
            ...
            quest_title("🔄 흐름과 연결 ", "조후, 십성 순환, 식상생재, 재생관, 관인상생을 봅니다.", ["조후", "십성", "식상생재", "재생관"])
            render_ability_axis_detail(result, "순환")
        ...
        for row in info_rows:
            info_html += (
                f"<div style='background:#16161e;border:1px solid rgba(140,140,210,.32);border-radius:16px;padding:.82rem .9rem;'>"
                f"<div style='font-weight:950;color:#9898b8;font-size:.88rem;'>{html.escape(str(row['항목']))}</div>"
                f"<div style='font-weight:850;color:#d0d0e8;line-height:1.55;margin-top:.28rem;white-space:normal;overflow:visible;text-overflow:clip;'>{html.escape(str(row['요약']))}</div>"
                f"</div>"
            )
        info_html += "</div>"
    ...
    support_view = st.radio(
        "하단 참고 메뉴",
        ["선택 안 함", "패치노트", "만세력 검문소", "용어 해설서", "안전·법적 고지", "검증 요약", "팔자 리포트"],
        key=f"{prefix}_support_selector",
    )

    if support_view == "패치노트":
        render_patch_notes()
    elif support_view == "만세력 검문소":
        ...
        if len(roster) >= 2:
            with col_b:
                ...
                if st.button("최신 2명 배틀", use_container_width=True, key="roster_battle"):
                    ...
                    st.session_state.show_details = False
            with col_c:
                if st.button("보관함 케미 분석", use_container_width=True, key="roster_multi"):
...
if st.session_state.payload is None and st.session_state.selected_main_mode is None:
    ...
    st.markdown(
        ...
        <div style='display:flex;align-items:center;gap:8px;margin-bottom:8px;'>
          <span style='background:#3730a3;color:#fff;border-radius:4px;padding:2px 8px;
          font-size:13px;font-weight:700;'>🆕 {APP_VERSION} 업데이트</span>
          <span style='font-size:13px;color:#c090a8;'>2026년 5월</span>
        </div>
        <div style='font-size:13px;color:#9898b8;line-height:1.9;'>
          · 모임 케미 기능이 추가되었어
        </div>
        </div>
        """,
...
if input_mode == "혼자 보기":
    ...
    if single_view_mode != "생년월일시 자동 산출":
        st.caption("원국 직접 입력 방식은 생년월일시 원문이 없으므로 대운·세운·오늘의 처방 일부가 제한될 수 있습니다.")
...
if input_mode == "혼자 보기" and single_view_mode == "원국 직접 입력":
    ...
    # v5.51: 보관함 재사용은 기본 UX에서 제외. 케미 분석은 매번 직접 입력한다.
    ...
    if analyze_clicked:
        ...
        try:
            ...
            if len(participants) < 2 or len(participants) > 5:
                st.error("케미 분석은 2~5명을 입력해야 합니다.")
                st.stop()
            if len(participants) == 2:
                battle_payload = {
                    "mode": "케미 분석",
...
if analyze_clicked:
    ...
    try:
        ...
        _mc = locals().get("multi_chem_payload")
        if _mc:
            for _p in _mc.get("participants", []):
                if _p and _p.get("auto_check"):
                    ac = _p["auto_check"]
                    loading_notes.append(f"{_p.get('name', '참가자')} 자동 산출 안정도: {ac.get('stability', '미확인')}")
        if multi_payload:
    ...
    if meal_payload:
        ...
        st.session_state.payload = None
        st.session_state.result_revealed = False
    elif "multi_chem_payload" in dir() and multi_chem_payload:
        log_event("chemistry_run", "multi_chem", {
            "participant_count": len(multi_chem_payload.get("participants", [])),
        })
        st.session_state.payload = {"multi_chem": True, **multi_chem_payload}
        st.session_state.pending_payload = None
        st.session_state.result_revealed = True
        st.rerun()
    elif battle_payload:
...
if payload.get("meal"):
    ...
    <div class="mini-card">
        ...
        <div style="color:rgba(50,50,80,0.78); margin-top:0.7rem; line-height:1.65;">
            {summary['summary']}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if len(participants) >= 3:
        st.markdown("#### 🌐 다자간 국면")
        st.caption("각 참가자가 서로에게 어떤 영향을 주는지 관계도로 볼 수 있어.")
        power_map = diplomacy_narrative(participants)
